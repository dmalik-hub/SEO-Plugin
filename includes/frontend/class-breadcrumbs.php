...
class Breadcrumbs {
	...
	private function add_crumbs_product_tag() {
		...
		$this->add_crumb( sprintf( __( 'Products tagged &ldquo;%s&rdquo;', 'gohigh-seo' ), $this->get_breadcrumb_title( 'term', $term, $term->name ) ), get_term_link( $term ) );
	}
	...
	private function maybe_add_page_crumb() {
		...
		$this->add_crumb( sprintf( esc_html__( 'Page %s', 'gohigh-seo' ), $current_page ), '', true );
	}
