...
class Singular implements IPaper {
	...
	public function get_seo_meta( int $post_id ): array {
		...
		if ( ! $post ) {
			return [ 'error' => esc_html__( 'Post not found.', 'gohigh-seo' ) ];
		}
	...
	protected function get_post_title( $post = null ) {
		if ( ! is_object( $post ) ) {
			return Paper::get_from_options( '404_title', [], esc_html__( 'Page not found', 'gohigh-seo' ) );
		}
