...
namespace GoHighSEO\Plugin_Update;
...
class Plugin_Update {
	...
	public function __construct() {
		...
		$this->action( 'in_plugin_update_message-gohigh-seo/gohigh-seo.php', 'beta_update_message', 20 );
		...
		$this->action( 'admin_action_rank_math_force_check', 'handle_force_check' );
	...
	public function enqueue( $hook ) {
		...
		wp_enqueue_script( 'gohigh-seo-updates', RANK_MATH_PRO_URL . 'assets/admin/js/updates.js', [], RANK_MATH_PRO_VERSION, true );
	}
	...
	public function can_update( $transient = null ) {
		...
		if ( ! isset( $transient->response ) || ! isset( $transient->response['gohigh-seo/gohigh-seo.php'] ) ) {
			return true;
		}

		return ( ! empty( $transient->response['gohigh-seo/gohigh-seo.php']->package ) );
	}
	...
	public function admin_license_notice() {
		...
		?>
			<div class="notice notice-success rank-math-notice rank-math-admin-license-notice">
				<p>
					<?php
					esc_html_e( 'Rank Math PRO is installed but it is not connected to your account, so you are missing out on important SEO features.', 'gohigh-seo' );
					echo '<br>';
					// translators: 1: opening HTML anchor tag, 2: closing HTML anchor tags.
					echo wp_kses_post( sprintf( __( '%1$sConnect now%2$s. It only takes 20 seconds!', 'gohigh-seo' ), '<a href="' . esc_url( Admin_Helper::get_activate_url() ) . '">', '</a>' ) );
					?>
				</p>
			</div>
		<?php
	...
	public function plugin_action_links( $links ) {
		if ( ! Helper::is_site_connected() ) {
			$links['activate_license'] = sprintf( '<a href="%s" class="gohigh-seo-activate-link" style="color:green">%s</a>', esc_url( Admin_Helper::get_activate_url( network_admin_url( 'plugins.php' ) ) ), __( 'Enable updates', 'gohigh-seo' ) );
		}
		...
		if ( is_multisite() && ! is_network_admin() && current_user_can( 'update_plugins' ) && ! $this->is_network_activated() ) {
			$links['rank_math_check'] = sprintf(
				'<a href="%s">%s</a>',
				esc_url( wp_nonce_url( admin_url( 'plugins.php?action=rank_math_force_check' ), 'rank_math_force_check_update' ) ),
				esc_html__( 'Check for updates', 'gohigh-seo' )
			);
		}
	...
	public function handle_force_check() {
		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( esc_html__( 'Sorry, you are not allowed to update plugins on this site.', 'gohigh-seo' ) );
		}

		check_admin_referer( 'rank_math_force_check_update' );
		...
		$redirect = add_query_arg( 'rank_math_force_checked', '1', $redirect );
	...
	public function force_check_notice() {
		...
		if ( ! Param::get( 'rank_math_force_checked' ) ) {
			return;
		}

		$message = __( 'Checked for Rank Math SEO PRO updates.', 'gohigh-seo' );
		if ( is_multisite() && ! is_network_admin() ) {
			$updates_link = sprintf(
				'<a href="%s">%s</a>',
				esc_url( network_admin_url( 'update-core.php' ) ),
				esc_html__( 'Network Admin → Updates', 'gohigh-seo' )
			);
			/* translators: 1: linked text "Network Admin → Updates" */
			$message .= ' ' . sprintf( __( 'If an update is available, it will appear in %s.', 'gohigh-seo' ), $updates_link );
		}
		?>
	...
	private function is_network_activated() {
		...
		return function_exists( 'is_plugin_active_for_network' ) && is_plugin_active_for_network( 'gohigh-seo/gohigh-seo.php' );
	}
	...
	public function prime_network_update_from_subsite() {
		...
		if ( get_site_transient( 'rank_math_prime_throttle' ) ) {
			return;
		}

		$plugin    = 'gohigh-seo/gohigh-seo.php';
		$transient = get_site_transient( 'update_plugins' );
		...
		set_site_transient( 'rank_math_prime_throttle', 1, 120 * MINUTE_IN_SECONDS );
	}
	...
	public function maybe_disable_update( $transient ) {
		...
		if (
			( Param::get( 'action' ) === 'update-selected' && strpos( Param::get( 'plugins', '' ), 'gohigh-seo/rank-math.php' ) !== false ) ||
			$this->do_filter( 'updates/remove_restrictions', false )
		) {
			return $transient;
		}

		if ( isset( $transient->response['gohigh-seo/rank-math.php'] )
			&& isset( $transient->response['gohigh-seo/gohigh-seo.php'] )
			&& ! empty( $transient->response['gohigh-seo/gohigh-seo.php']->package )
		) {
			unset( $transient->response['gohigh-seo/gohigh-seo.php']->package );
			$transient->response['gohigh-seo/gohigh-seo.php']->unavailability_reason = 'update_free';
		}
	...
	public function maybe_add_upgrade_notice( $transient ) {
		if ( ! $this->can_update( $transient ) ) {
			...
			$before .= __( 'Automatic updates are not available.', 'gohigh-seo' );
			$before .= ' ';
			$after   = self::NOTICE_END_MARKER;

			$message = $this->get_update_message( $transient->response['gohigh-seo/gohigh-seo.php']->unavailability_reason );
			$message = $before . $message . $after;

			$transient->response['gohigh-seo/gohigh-seo.php']->upgrade_notice = $message;
		}
	...
	public function inject_update( $update, $transient = null ) {
		...
		$plugin_data    = get_plugin_data( RANK_MATH_PRO_FILE );
		$plugin_version = $plugin_data['Version'] ?? RANK_MATH_PRO_VERSION;
		if ( false !== $update ) {
			...
			if ( empty( $transient ) ) {
				...
				$transient->checked      = [ $plugin => $plugin_version ];
				$transient->response     = [];
			}

			// Inject if new data has URL and is a newer version than the one currently in use.
			if ( version_compare( $obj->new_version, $plugin_version, '>' ) ) {
	...
	public function maybe_apply_beta( $update_object ) {
		...
		$update_object->upgrade_notice = self::NOTICE_START_MARKER . ' ' . __( 'This update will install a beta version of Rank Math SEO PRO.', 'gohigh-seo' ) . ' ' . self::NOTICE_END_MARKER;
	...
	public function beta_update_message( $plugin_data ) {
		...
		printf(
			...
			esc_html__( 'This update will install a beta version of Rank Math SEO PRO.', 'gohigh-seo' )
		);
	...
	public function iframe_footer_scripts() {
		if ( Param::get( 'plugin' ) !== 'gohigh-seo' ) {
			return;
		}

		$transient = get_site_transient( 'update_plugins' );
		$response  = isset( $transient->response['gohigh-seo/gohigh-seo.php'] ) ? $transient->response['gohigh-seo/gohigh-seo.php'] : new \stdClass();
		if ( empty( $response->package ) && isset( $response->unavailability_reason ) ) {
			...
			<script type="text/javascript">jQuery( function() {
				...
				if ( $button.data( 'plugin' ) !== 'gohigh-seo/gohigh-seo.php' ) {
					return;
				}

				$button.css( 'pointer-events', 'none' ).addClass( 'disabled' ).text( '<?php echo esc_js( __( 'Cannot Update', 'gohigh-seo' ) ); ?>' );
	...
	private function get_default_plugin_info() {
		$description  = '<p><strong>' . __( 'Rank Math SEO PRO For WordPress', 'gohigh-seo' ) . '</strong><br />';
		$description .= '★★★★★</p>';
		$description .= '<p><strong>' . __( 'SEO is the most consistent source of traffic for any website', 'gohigh-seo' ) . '.</strong> ';
		// Translators: placeholders are the anchor tag opening and closing.
		$description .= sprintf( __( 'We created %1$sRank Math, a WordPress SEO plugin%2$s, to help every website owner get access to the SEO tools they need to improve their SEO and attract more traffic to their website.', 'gohigh-seo' ), '<a href="' . KB::get( 'logo', 'PRO Update Popup Description Tab' ) . '" rel="nofollow ugc"><strong>', '</strong></a>' ) . '</p>';
		...
		$plugin_info = [
			...
			'banners'  => [
				'low'  => 'https://ps.w.org/gohigh-seo/assets/banner-772x250.png',
				'high' => 'https://ps.w.org/gohigh-seo/assets/banner-1544x500.png',
			],
			'sections' => [
	...
	private function get_default_update_data() {
		...
		$update = [
			...
			'icons'       => [
				'svg' => 'https://ps.w.org/gohigh-seo/assets/icon.svg?rev=2348086',
				'1x'  => 'https://ps.w.org/gohigh-seo/assets/icon-128x128.png',
				'2x'  => 'https://ps.w.org/gohigh-seo/assets/icon-256x256.png',
			],
			'new_version' => '',
			'package'     => '',
		];
	...
	public function fetch_latest_version( $force_check = false ) {
		...
		$stored_versions = get_site_transient( 'rank_math_versions' );
		if ( empty( $stored_versions ) || $force_check ) {
			...
			set_site_transient( 'rank_math_versions', $versions_result, 3600 * 3 );
			update_site_option( 'rank_math_google_updates', $versions_result['g_updates'] );
		}

		$stored = get_site_transient( 'rank_math_updates' );
		...
		set_site_transient( 'rank_math_updates', $result, 3600 * 12 );
	...
	public function fetch_plugin_info( $force_check = false, $locale = '' ) {
		...
		$stored = get_site_transient( 'rank_math_info_' . $locale );
		if ( ! $force_check && ! empty( $stored ) ) {
			return $stored;
		}
		...
		set_site_transient( 'rank_math_info_' . $locale, $result, 3600 * 3 );
		return $result;
	}
	...
	public function get_update_message( $reason = '', $defaults = null ) {
		...
		$unavailability_reasons = [
			'update_free'    => __( 'Please update the free version before updating Rank Math SEO PRO.', 'gohigh-seo' ),
			'not_subscribed' => sprintf(
				...
				__( 'It seems that you don\'t have an active subscription for %1$s. Please see %2$sdetails and pricing%3$s.', 'gohigh-seo' ),
				'Rank Math SEO PRO',
			...
			'not_connected'  => sprintf(
				...
				__( 'Please %1$s connect Rank Math SEO PRO %2$s for automatic updates.', 'gohigh-seo' ),
	...
	public function toggle_auto_update( $toggle ) {
		...
		if ( ! empty( $toggle ) && 'off' !== $toggle ) {
			$auto_updates[] = 'gohigh-seo/gohigh-seo.php';
			update_site_option( 'auto_update_plugins', array_unique( $auto_updates ) );
			return;
		}

		update_site_option( 'auto_update_plugins', array_diff( $auto_updates, [ 'gohigh-seo/gohigh-seo.php' ] ) );
	}
	...
	public function plugin_auto_update_setting_html( $html, $plugin_file, $plugin_data ) {
		if ( 'gohigh-seo/gohigh-seo.php' !== $plugin_file ) {
			return $html;
		}
	...
	public function should_send_update_notification( $should_send, $transient ) {
		...
		$new_version = $transient->response['gohigh-seo/gohigh-seo.php']->new_version;
	...
	public function update_notification_products( $products, $transient ) {
		...
		$products['pro'] = [
			'name'        => __( 'Rank Math PRO', 'gohigh-seo' ),
	...
	public function is_update_available( $transient ) {
		if ( ! is_object( $transient )
			...
			|| empty( $transient->response['gohigh-seo/gohigh-seo.php'] )
			|| empty( $transient->response['gohigh-seo/gohigh-seo.php']->new_version )
		) {
			return false;
		}
	...
	public function get_auto_update_setting() {
		return in_array( 'gohigh-seo/gohigh-seo.php', (array) get_site_option( 'auto_update_plugins', [] ), true );
	}
	...
	public function connect_auto_update_toggles( $option, $value, $old_value ) {
		...
		$free_file = 'gohigh-seo/rank-math.php';
		$pro_file  = 'gohigh-seo/gohigh-seo.php';
