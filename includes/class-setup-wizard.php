...
class Setup_Wizard {
	...
	public function ajax_import_settings() {
		...
		if ( is_array( $settings ) && $this->do_import_data( $settings ) ) {
			$this->success( __( 'Import successful.', 'gohigh-seo' ) );
			exit();
		}

		$this->error( __( 'No settings found to be imported.', 'gohigh-seo' ) );
		exit();
	}
	...
	private function has_valid_import_file() {
		if ( empty( $_FILES['import-me'] ) ) {  // phpcs:ignore WordPress.Security -- Verficiation is handled in the Rest function.
			$this->error( __( 'No file selected.', 'gohigh-seo' ) );
			return false;
		}

		$file = Helper::handle_file_upload();
		if ( is_wp_error( $file ) ) {
			$this->error( __( 'Settings file could not be imported:', 'gohigh-seo' ) . ' ' . $file->get_error_message() );
			return false;
		}

		if ( isset( $file['error'] ) ) {
			$this->error( __( 'Settings could not be imported:', 'gohigh-seo' ) . ' ' . $file['error'] );
			return false;
		}

		if ( ! isset( $file['file'] ) ) {
			$this->error( __( 'Settings could not be imported: Upload failed.', 'gohigh-seo' ) );
			return false;
		}
