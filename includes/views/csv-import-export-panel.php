...
namespace GoHighSEO\Admin\CSV_Import_Export;
...
?>
<h2><?php esc_html_e( 'CSV File', 'gohigh-seo' ); ?></h2>

<p class="description">
	<?php esc_html_e( 'Import SEO meta data for posts, terms, and users from a CSV file.', 'gohigh-seo' ); ?>
</p>
...
<div id="csv-box" class="rank-math-box no-padding">
	<div class="rank-math-box-tabs wp-clearfix">
		<a href="#csv-panel-import" class="active-tab">
			...
			<span class="rank-math-tab-text"><?php esc_html_e( 'Import CSV', 'gohigh-seo' ); ?></span>
		</a>
		<a href="#csv-panel-export" class="">
			<i class="rm-icon rm-icon-export"></i>
			<span class="rank-math-tab-text"><?php esc_html_e( 'Export CSV', 'gohigh-seo' ); ?></span>
		</a>
	</div>
		...
		<div class="rank-math-box-inner">
			<form id="csv-panel-import" class="rank-math-import-form cmb2-form active-tab" action="#csv-box" method="post" enctype="multipart/form-data" accept-charset="<?php echo esc_attr( get_bloginfo( 'charset' ) ); ?>">
				<?php if ( ! $import_in_progress ) : ?>
					...
					<div><input type="checkbox" class="cmb2-option" name="no_overwrite" id="csv_no_overwrite" value="on" checked="checked"> <label for="csv_no_overwrite"><?php esc_html_e( 'Do not overwrite existing data', 'gohigh-seo' ); ?></label></div>
					<p class="description no-overwrite-description"><?php esc_html_e( 'Check this to import meta fields only if their current meta value is empty.', 'gohigh-seo' ); ?></p>
					<div id="csv-import-warning" class="rank-math-notice notice-warning hidden">
						<p class="description">
							...
							<?php printf( esc_html__( '%s It is recommended to save a database backup before using this option because importing malformed CSV can result in loss of data.', 'gohigh-seo' ), '<strong>' . esc_html__( 'Warning:', 'gohigh-seo' ) . '</strong> ' ); ?>
						</p>
					</div>
				<?php else : ?>
				...
				<footer>
					<?php wp_nonce_field( 'rank_math_csv_import' ); ?>
					<input type="hidden" name="object_id" value="csv-import-plz">
					<input type="hidden" name="action" value="wp_handle_upload">
					<?php if ( $import_in_progress ) : ?>
						<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( [ 'rank_math_cancel_csv_import' => 1 ] ), 'rank_math_cancel_csv_import' ) ); ?>" id="csv-import-cancel" class="button button-link-delete csv-import-cancel"><?php esc_html_e( 'Cancel Import', 'gohigh-seo' ); ?></a>
						<span class="input-loading" style="visibility: visible;"></span>
					<?php else : ?>
						<button type="submit" class="button button-primary"><?php esc_html_e( 'Import', 'gohigh-seo' ); ?></button>
					<?php endif; ?>
				</footer>
			</form>
			<form id="csv-panel-export" class="rank-math-export-form cmb2-form" action="" method="post">
						...
						<li>
							...
							<?php if ( 'post' === $object_type ) : ?>
								<div class="csv-advanced-options">
									<p class="description csv-advanced-options-description"><?php esc_html_e( 'Post types:', 'gohigh-seo' ); ?></p>
							...
							<?php elseif ( 'term' === $object_type ) : ?>
								<div class="csv-advanced-options">
									<p class="description csv-advanced-options-description"><?php esc_html_e( 'Taxonomies:', 'gohigh-seo' ); ?></p>
									<ul class="cmb2-checkbox-list no-select-all cmb2-list csv-advanced-options-list">
							...
							<?php elseif ( 'user' === $object_type ) : ?>
								<div class="csv-advanced-options">
									<p class="description csv-advanced-options-description"><?php esc_html_e( 'User Roles:', 'gohigh-seo' ); ?></p>
									<ul class="cmb2-checkbox-list no-select-all cmb2-list csv-advanced-options-list">
				...
				<div class="csv-advanced-options readonly-columns">
					<p>
						<input type="checkbox" class="cmb2-option" name="readonly_columns" id="readonly_columns" value="1"> <label for="readonly_columns"><?php esc_html_e( 'Include read-only columns (SEO score and link counts)', 'gohigh-seo' ); ?></label>
					</p>
				</div>
				<p class="description"><?php esc_html_e( 'Choose the object types to export.', 'gohigh-seo' ); ?></p>

				<input type="checkbox" class="cmb2-option" name="use_advanced_options" id="csv-advanced-options-toggle" value="1"> <label for="csv-advanced-options-toggle"><?php esc_html_e( 'Use advanced options', 'gohigh-seo' ); ?></label>

				<footer>
					<?php wp_nonce_field( 'rank_math_csv_export' ); ?>
					<input type="hidden" name="rank_math_csv_export" value="1">
					<?php wp_nonce_field( 'rank_math_csv_export', '_wpnonce', true, true ); ?>
					<button type="submit" class="button button-primary" id="export-csv"><?php esc_html_e( 'Export', 'gohigh-seo' ); ?></button>
