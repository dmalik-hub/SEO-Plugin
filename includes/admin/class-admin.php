...
class Admin {
	...
	public function init_components() {
		$components = [
			'bulk_actions'       => 'GoHighSEO\\Admin\\Bulk_Actions',
			'post_filters'       => 'GoHighSEO\\Admin\\Post_Filters',
			'media_filters'      => 'GoHighSEO\\Admin\\Media_Filters',
			'quick_edit'         => 'GoHighSEO\\Admin\\Quick_Edit',
			'trends_tool'        => 'GoHighSEO\\Admin\\Trends_Tool',
			'links'              => 'GoHighSEO\\Admin\\Links',
			'misc'               => 'GoHighSEO\\Admin\\Misc',
			'csv_import'         => 'GoHighSEO\\Admin\\CSV_Import_Export\\CSV_Import_Export',
			'licence_activation' => 'GoHighSEO\\Admin\\Licence_Activation',
		];

		if ( Helper::is_amp_active() ) {
			$components['amp'] = 'GoHighSEO\\Admin\\Amp';
		}
	...
	/**
	 * Add Special separator into sitemap option panel
	 *
	...
	public function special_seprator( $tabs ) {
		if ( Helper::is_module_active( 'news-sitemap' ) || Helper::is_module_active( 'video-sitemap' ) || Helper::is_module_active( 'local-seo' ) ) {
			$tabs['special'] = [
				'title' => esc_html__( 'Special Sitemaps:', 'gohigh-seo' ),
				'type'  => 'separator',
			];
		}
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'gohigh-seo-general-options', RANK_MATH_PRO_URL . "assets/admin/js/{$settings_file}.js", [ 'wp-hooks' , 'lodash', 'jquery', 'wp-i18n', 'wp-api-fetch', 'wp-data', 'rank-math-components' ], rank_math()->version, true );
	...
	public function prevent_pro_notice( $output, $message, $options ) {
		if ( 'rank_math_notice' !== $options['id'] ) {
			return $output;
		}
