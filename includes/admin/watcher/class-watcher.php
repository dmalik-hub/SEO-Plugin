...
class Watcher implements Runner {
	...
	public function maybe_deactivate_plugins() {
		...
		if ( ! current_user_can( 'deactivate_plugins' ) ) {
			wp_die( esc_html__( 'Sorry, you are not allowed to deactivate plugins for this site.', 'gohigh-seo' ) );
		}
			...
			if ( $type === $plugin_type && is_plugin_active( $plugin ) ) {
				if ( ! current_user_can( 'deactivate_plugin', $plugin ) ) {
					$message = sprintf(
						...
						esc_html__( 'You are not allowed to deactivate this plugin: %s.', 'gohigh-seo' ),
						esc_html( $plugin )
					);
	...
	private static function set_notification( $group ) {
		$message = esc_html__( 'Please keep only one SEO plugin active, otherwise, you might lose your rankings and traffic.', 'gohigh-seo' );

		if ( 'sitemap' === $group ) {
			$message = esc_html__( 'Please keep only one Sitemap plugin active, otherwise, you might lose your rankings and traffic.', 'gohigh-seo' );
		}

		if ( 'redirections' === $group ) {
			$message = esc_html__( 'Please keep only one Redirection plugin active to avoid redirect conflicts or unexpected behavior.', 'gohigh-seo' );
		}

		$message .= ' <a href="###DEACTIVATE_PLUGINS###">' . __( 'Click here to Deactivate', 'gohigh-seo' ) . '</a>';
	...
	public function check_search_engine_visibility( $value ) {
		...
		GlobalHelper::add_notification(
			sprintf(
				...
				__( '<strong>SEO Notice</strong>: Your site is set to No Index and will not appear in search engines. You can change the Search engine visibility <a href="%1$s">from here</a>.', 'gohigh-seo' ),
