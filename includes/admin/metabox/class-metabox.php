...
namespace GoHighSEO\Admin\Metabox;
...
class Metabox implements Runner {
	...
	public function add_main_metabox() {
		...
		if ( ! empty( $object_types ) ) {
			add_meta_box(
				$this->metabox_id,
				esc_html__( 'Rank Math SEO', 'gohigh-seo' ),
	...
	public function render_taxonomy_metabox( $term, $taxonomy ) {
		...
		<div class="form-table rank-math-metabox-wrap rank-math-metabox-frame postbox">
			<div id="setting-panel-container-<?php echo esc_attr( $this->metabox_id ); ?>" class="rank-math-sidebar-panel rank-math-tabs">
				<h2 class="rank-math-metabox-frame-title"><?php esc_html_e( 'Rank Math SEO', 'gohigh-seo' ); ?></h2>
	...
	public function add_link_suggestion_metabox() {
		...
		add_meta_box(
			...
			esc_html__( 'Link Suggestions', 'gohigh-seo' ),
	...
	public function render_link_suggestion_metabox( $post ) {
		...
		echo wp_kses_post( Admin_Helper::get_tooltip( esc_html__( 'Click on the button to copy URL or insert link in content. You can also drag and drop links in the post content.', 'gohigh-seo' ) ) );
		echo '</div>';

		$suggestions = rank_math()->admin->get_link_suggestions( $post );
		if ( empty( $suggestions ) ) {
			echo '<em><small>' . esc_html__( 'We can\'t show any link suggestions for this post. Try selecting categories and tags for this post, and mark other posts as Pillar Content to make them show up here.', 'gohigh-seo' ) . '</small></em>';
			return;
		}
	...
	private function enqueue_translation() {
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'rank-math-analyzer', 'gohigh-seo', rank_math()->plugin_dir() . 'languages/' );
			wp_set_script_translations( 'rank-math-app', 'gohigh-seo', rank_math()->plugin_dir() . 'languages/' );
		}
	}
