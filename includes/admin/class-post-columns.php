...
class Post_Columns implements Runner {
	...
	public function add_columns( $columns ) {
		...
		$columns['rank_math_seo_details'] = esc_html__( 'SEO Details', 'gohigh-seo' );

		if ( Helper::get_settings( 'titles.pt_' . $current_pt . '_bulk_editing', true ) ) {
			$columns['rank_math_title']       = esc_html__( 'SEO Title', 'gohigh-seo' );
			$columns['rank_math_description'] = esc_html__( 'SEO Desc', 'gohigh-seo' );
		}
	...
	public function add_media_columns( $columns ) {
		$columns['rank_math_image_title'] = esc_html__( 'Title', 'gohigh-seo' );
		$columns['rank_math_image_alt']   = esc_html__( 'Alternative Text', 'gohigh-seo' );
	...
	public function get_column_title( $object_id, $object_type = 'post' ) {
		...
		<div class="rank-math-column-edit">
			<a href="#" class="rank-math-column-save"><?php esc_html_e( 'Save', 'gohigh-seo' ); ?></a>
			<a href="#" class="button-link-delete rank-math-column-cancel"><?php esc_html_e( 'Cancel', 'gohigh-seo' ); ?></a>
		</div>
		<?php
	}
	...
	public function get_column_seo_details( $post_id ) {
		...
		if ( ! self::is_post_indexable( $post_id ) ) {
			...
			echo '<strong>' . esc_html__( 'No Index', 'gohigh-seo' ) . '</strong>';
		...
		<span class="rank-math-column-display seo-score <?php echo esc_attr( $class ); ?> <?php echo ! $score ? 'disabled' : ''; ?>">
			...
			<?php if ( $is_pillar ) { ?>
				<img class="is-pillar" src="<?php echo esc_url( rank_math()->plugin_url() . 'assets/admin/img/pillar.svg' ); ?>" alt="<?php esc_html_e( 'Is Pillar', 'gohigh-seo' ); ?>" title="<?php esc_html_e( 'Is Pillar', 'gohigh-seo' ); ?>" width="25" />
			<?php } ?>
		</span>

			<label><?php esc_html_e( 'Focus Keyword', 'gohigh-seo' ); ?>:</label>
			<span class="rank-math-column-display">
				<strong title="Focus Keyword"><?php esc_html_e( 'Keyword', 'gohigh-seo' ); ?>:</strong>
				<span>
					<?php
						echo $keyword ? wp_kses_post( $this->do_filter( 'post/column/seo_details/focus_keyword', $keyword ) ) : esc_html__( 'Not Set', 'gohigh-seo' );
					?>
				</span>
			</span>
