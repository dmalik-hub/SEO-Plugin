...
use GoHighSEO\Helpers\Attachment;
...
abstract class Plugin_Importer {
	...
	public function get_choices() {
		...
		return array_intersect_key(
			[
				'settings'     => esc_html__( 'Import Settings', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Import plugin settings, global meta, sitemap settings, etc.', 'gohigh-seo' ) ),
				'postmeta'     => esc_html__( 'Import Post Meta', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Import meta information of your posts/pages like the focus keyword, titles, descriptions, robots meta, OpenGraph info, etc.', 'gohigh-seo' ) ),
				'termmeta'     => esc_html__( 'Import Term Meta', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Import data like category, tag, and CPT meta data from SEO.', 'gohigh-seo' ) ),
				'usermeta'     => esc_html__( 'Import Author Meta', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Import meta information like titles, descriptions, focus keyword, robots meta, etc., of your author archive pages.', 'gohigh-seo' ) ),
				'redirections' => esc_html__( 'Import Redirections', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Import all the redirections you have already set up in Yoast Premium.', 'gohigh-seo' ) ),
				'blocks'       => esc_html__( 'Import Blocks', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Import and convert all compatible blocks in post contents.', 'gohigh-seo' ) ),
				'locations'    => esc_html__( 'Import Locations', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Import Locations Settings from Yoast plugin.', 'gohigh-seo' ) ),
				'news'         => esc_html__( 'Import News Settings', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Import News Settings from Yoast News Add-on.', 'gohigh-seo' ) ),
				'video'        => esc_html__( 'Import Video Sitemap Settings', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Import Video Sitemap Settings from Yoast Video Add-on.', 'gohigh-seo' ) ),
			],
			array_combine(
				$this->choices,
				$this->choices
	...
	public function run_import( $perform ) {
		if ( ! method_exists( $this, $perform ) ) {
			throw new Exception( esc_html__( 'Unable to perform action this time.', 'gohigh-seo' ) );
		}
	...
	private function recalculate() {
		$this->set_pagination( \GoHighSEO\Tools\Update_Score::get()->find( false ) );

		$return = $this->get_pagination_arg();
		$data   = \GoHighSEO\Tools\Update_Score::get()->update_seo_score();
