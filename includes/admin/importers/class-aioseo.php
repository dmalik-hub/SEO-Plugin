...
class AIOSEO extends Plugin_Importer {
	...
	public function get_choices() {
		$choices = [
			'settings' => esc_html__( 'Import Settings', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Import AIO SEO plugin settings, global meta, sitemap settings, etc.', 'gohigh-seo' ) ),
			'postmeta' => esc_html__( 'Import Post Meta', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Import meta information of your posts/pages like the titles, descriptions, robots meta, OpenGraph info, etc.', 'gohigh-seo' ) ),
			'usermeta' => esc_html__( 'Import Author Meta', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Import Social URLs of your author archive pages.', 'gohigh-seo' ) ),
		];

		if ( DB::check_table_exists( 'aioseo_terms' ) ) {
			$choices['termmeta'] = esc_html__( 'Import Term Meta', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Import meta information of your terms like the titles, descriptions, robots meta, OpenGraph info, etc.', 'gohigh-seo' ) );
		}

		if ( DB::check_table_exists( 'aioseo_redirects' ) ) {
			$choices['redirections'] = esc_html__( 'Import Redirections', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Import all the redirections you have already set up in AIO SEO Premium.', 'gohigh-seo' ) );
		}

		if ( ! empty( $this->get_location_ids( true ) ) ) {
			$choices['locations'] = esc_html__( 'Import Locations', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Import Locations Settings.', 'gohigh-seo' ) );
		}
		...
		if ( ! empty( $settings['pageUrl'] ) ) {
			...
			if ( ! empty( $page ) ) {
				...
				$this->sitemap['html_sitemap_page'] = wp_insert_post(
					[
						'post_title'   => __( 'HTML Sitemap', 'gohigh-seo' ),
						'post_content' => '',
						'post_status'  => 'publish',
						'post_type'    => 'page',
