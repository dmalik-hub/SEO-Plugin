...
class Page {
	...
	public function __construct( $id, $title, $config = [] ) {
		...
		// Early bail!
		if ( ! $id ) {
			wp_die( esc_html__( '$id variable required', 'gohigh-seo' ), esc_html__( 'Variable Required', 'gohigh-seo' ) );
		}

		if ( ! $title ) {
			wp_die( esc_html__( '$title variable required', 'gohigh-seo' ), esc_html__( 'Variable Required', 'gohigh-seo' ) );
		}
