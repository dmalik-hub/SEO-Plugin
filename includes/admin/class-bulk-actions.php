...
class Bulk_Actions {
	...
	public function handle_post_bulk_actions( $redirect, $doaction, $object_ids ) {
		...
		switch ( $doaction ) {
			...
			case 'rank_math_bulk_robots_follow':
				...
				$message = sprintf( __( 'Robots meta edited for %1$d %2$s.', 'gohigh-seo' ), $edited, ( $edited > 1 ? $post_type_object->labels->name : $post_type_object->labels->singular_name ) );
				break;
			...
			case 'rank_math_bulk_stop_redirect':
				foreach ( $object_ids as $post_id ) {
					$redirection = \GoHighSEO\Redirections\Cache::get_by_object_id( $post_id, 'post' );
					if ( $redirection ) {
						\GoHighSEO\Redirections\DB::change_status( $redirection->redirection_id, 'trashed' );
						++$edited;
					}
				}
				// Translators: placeholder is the number of redirections deleted.
				$message = sprintf( _n( '%d redirection moved to Trash.', '%d redirections moved to Trash.', $edited, 'gohigh-seo' ), $edited );
				break;
			...
			case 'rank_math_bulk_schema_none':
				...
				// Translators: 1 The number of posts edited. 2 The post type name.
				$message = sprintf( __( 'Schema edited for %1$d %2$s.', 'gohigh-seo' ), $edited, ( $edited > 1 ? $post_type_object->labels->name : $post_type_object->labels->singular_name ) );
				break;
			...
			case 'rank_math_bulk_remove_canonical':
				...
				// Translators: 1 The number of posts edited. 2 The post type name.
				$message = sprintf( __( 'Custom Canonical URL removed from %1$d %2$s.', 'gohigh-seo' ), $edited, ( $edited > 1 ? $post_type_object->labels->name : $post_type_object->labels->singular_name ) );
				break;
			case 'rank_math_bulk_determine_search_intent':
				...
				if ( empty( $focus_keywords ) ) {
					$message = esc_html__( 'Focus keywords are not added for the selected posts. Please add the Focus keywords and try again.', 'gohigh-seo' );
				} else {
					...
					if ( isset( $response['error'] ) ) {
						...
						$message = sprintf( _n( 'Search Intent updated for %d Focus Keyword.', 'Search Intent updated for %d Focus Keywords.', $count, 'gohigh-seo' ), $count );
					}
				}
				break;
		}
	...
	public function delete_schema( $post_id ) {
		...
		DB_Helper::query( "DELETE FROM {$wpdb->postmeta} {$where}" );
	}
	...
	public function handle_tax_bulk_actions( $redirect, $doaction, $object_ids ) {
		...
		switch ( $doaction ) {
			...
			case 'rank_math_bulk_robots_follow':
				...
				$message = sprintf( __( 'Robots meta edited for %1$d %2$s.', 'gohigh-seo' ), $edited, ( $edited > 1 ? $tax_object->labels->name : $tax_object->labels->singular_name ) );
				break;
			...
			case 'rank_math_bulk_stop_redirect':
				foreach ( $object_ids as $term_id ) {
					$redirection = \GoHighSEO\Redirections\Cache::get_by_object_id( $term_id, 'term' );
					if ( $redirection ) {
						\GoHighSEO\Redirections\DB::change_status( $redirection->redirection_id, 'trashed' );
						++$edited;
					}
	...
	public function save_post_primary_term( $post_id ) {
		if ( Param::get( 'rank_math_quick_edit_nonce' ) || Param::get( 'rank_math_quick_edit_nonce_bulk' ) ) {
			return;
		}
