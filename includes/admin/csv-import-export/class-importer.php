...
class Importer {
	...
	public function start( $file, $settings = [] ) {
		...
		if ( empty( $line_indices ) ) {
			...
			$status['errors']      = [ esc_html__( 'The CSV file is empty or invalid.', 'gohigh-seo' ) ];
	...
	public function get_row_data( $line_number ) {
		...
		if ( ! $file ) {
			$this->add_error( esc_html__( 'Missing import file.', 'gohigh-seo' ), 'missing_file' );
		...
		if ( empty( $headers ) ) {
			$this->add_error( esc_html__( 'Missing CSV headers.', 'gohigh-seo' ), 'missing_headers' );
			return false;
		}

		$required_columns = [ 'id', 'object_type', 'slug' ];
		if ( count( array_intersect( $headers, $required_columns ) ) !== count( $required_columns ) ) {
			$this->add_error( esc_html__( 'Missing one or more required columns.', 'gohigh-seo' ), 'missing_required_columns' );
			return false;
		}
		...
		if ( empty( $raw_data ) ) {
			...
			if ( $line_number !== $total_lines ) {
				$this->add_error( esc_html__( 'Empty column data.', 'gohigh-seo' ), 'missing_data' );
		...
		if ( count( $headers ) !== count( $decoded ) ) {
			$this->add_error( esc_html__( 'Columns number mismatch.', 'gohigh-seo' ), 'columns_number_mismatch' );
			$this->row_failed( $line_number );
			return false;
		}
		...
		if ( ! in_array( $data['object_type'], array_keys( CSV_Import_Export::get_possible_object_types() ), true ) ) {
			$this->add_error( esc_html__( 'Unknown object type.', 'gohigh-seo' ), 'unknown_object_type' );
			$this->row_failed( $line_number );
			return false;
		}
