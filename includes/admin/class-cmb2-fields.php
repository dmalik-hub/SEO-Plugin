...
class CMB2_Fields implements Runner {
	...
	public function render_switch( $field, $escaped_value, $object_id, $object_type, $field_type_object ) {
		...
		if ( empty( $field->args['options'] ) ) {
			$field->args['options'] = [
				'off' => esc_html( $field->get_string( 'off', __( 'Off', 'gohigh-seo' ) ) ),
				'on'  => esc_html( $field->get_string( 'on', __( 'On', 'gohigh-seo' ) ) ),
			];
		}
		$field->set_options();
	...
	public function render_advanced_robots( $field, $escaped_value, $object_id, $object_type, $field_type_object ) {
		...
		$strings = [
			'max-snippet'       => __( 'Snippet', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Specify a maximum text-length, in characters, of a snippet for your page.', 'gohigh-seo' ) ),
			'max-video-preview' => __( 'Video Preview', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Specify a maximum duration in seconds of an animated video preview.', 'gohigh-seo' ) ),
			'max-image-preview' => __( 'Image Preview', 'gohigh-seo' ) . Admin_Helper::get_tooltip( esc_html__( 'Specify a maximum size of image preview to be shown for images on this page.', 'gohigh-seo' ) ),
		];
	...
	private function get_image_sizes( $size = 'large' ) {
		$values  = [
			'large'    => __( 'Large', 'gohigh-seo' ),
			'standard' => __( 'Standard', 'gohigh-seo' ),
			'none'     => __( 'None', 'gohigh-seo' ),
		];
		$options = '';
