...
use GoHighSEO\Runner;
use GoHighSEO\Traits\Hooker;
use GoHighSEO\Helpers\Arr;
use GoHighSEO\Helpers\Param;
use GoHighSEO\Wizard\Search_Console;
use GoHighSEO\Admin\Sanitize_Settings;
...
class Option_Center implements Runner {
	...
	public function register_general_settings() {
		$tabs = [
			'links'       => [
				...
				'desc'  => sprintf( esc_html__( 'Change how some of the links open and operate on your website. %s.', 'gohigh-seo' ), '<a href="' . KB::get( 'link-settings', 'Options Panel Links Tab' ) . '" target="_blank">' . esc_html__( 'Learn More', 'gohigh-seo' ) . '</a>' ),
			],
			'breadcrumbs' => [
				'icon'      => 'rm-icon rm-icon-direction',
				'title'     => esc_html__( 'Breadcrumbs', 'gohigh-seo' ),
				'classes'   => 'rank-math-advanced-option',
				/* translators: Link to kb article */
				'desc'      => sprintf( esc_html__( 'Here you can set up the breadcrumbs function. %s', 'gohigh-seo' ), '<a href="' . KB::get( 'breadcrumbs', 'Options Panel Breadcrumbs Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>. <br/>' ),
				'after_row' => current_theme_supports( 'rank-math-breadcrumbs' ) ? '' : '<div class="notice notice-alt notice-warning warning inline rank-math-notice"><p>' . esc_html__( 'Use the following code in your theme template files to display breadcrumbs.', 'gohigh-seo' ) . ' <a href="' . KB::get( 'breadcrumbs-install', 'Options Panel Breadcrumbs Tab' ) . '" target="_blank">' . esc_html__( 'Learn More', 'gohigh-seo' ) . '</a><br /><code>&lt;?php if (function_exists(\'rank_math_the_breadcrumbs\')) rank_math_the_breadcrumbs(); ?&gt;</code> OR <code>[rank_math_breadcrumb]</code></p></div>',
			],
			'webmaster'   => [
				'icon'  => 'rm-icon rm-icon-toolbox',
				'title' => esc_html__( 'Webmaster Tools', 'gohigh-seo' ),
				/* translators: Link to kb article */
				'desc'  => sprintf( esc_html__( 'Enter verification codes for third-party webmaster tools. %s', 'gohigh-seo' ), '<a href="' . KB::get( 'webmaster-tools', 'Options Panel Webmaster Tools Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>.<br />' ),
			],
			'others'      => [
				'icon'    => 'rm-icon rm-icon-misc',
				'title'   => esc_html__( 'Others', 'gohigh-seo' ),
				/* translators: Link to kb article */
				'desc'    => sprintf( esc_html__( 'Change some uncommon but essential settings here. %s.', 'gohigh-seo' ), '<a href="' . KB::get( 'other-settings', 'Options Panel Others Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>' ),
				'classes' => 'rank-math-advanced-option',
			],
		];
			...
			Arr::insert(
				...
				[
					'htaccess' => [
						...
						'title'   => esc_html__( 'Edit .htaccess', 'gohigh-seo' ),
						/* translators: Link to kb article */
						'desc'    => sprintf( esc_html__( 'Edit the contents of your .htaccess file easily. %s.', 'gohigh-seo' ), '<a href="' . KB::get( 'edit-htaccess', 'Options Panel htaccess Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>' ),
						'classes' => 'rank-math-advanced-option',
						'json'    => [
		...
		new Register_Options_Page(
			[
				...
				'title'      => esc_html__( 'SEO Settings', 'gohigh-seo' ),
				'menu_title' => esc_html__( 'General Settings', 'gohigh-seo' ),
	...
	public function register_title_settings() {
		...
		if ( 'page' === get_option( 'show_on_front' ) ) {
			...
			$homepage_notice = '<a href="' . admin_url( 'post.php?post=' . $home_page_id . '&action=edit' ) . '">' . esc_html__( 'Edit Page: ', 'gohigh-seo' ) . get_the_title( $home_page_id ) . '</a>';
		}
		$tabs = [
			'global'   => [
				'icon'  => 'rm-icon rm-icon-settings',
				'title' => esc_html__( 'Global Meta', 'gohigh-seo' ),
				/* translators: Link to KB article */
				'desc'  => sprintf( esc_html__( 'Change Global meta settings that take effect across your website. %s.', 'gohigh-seo' ), '<a href="' . KB::get( 'titles-meta', 'Options Panel Meta Global Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>' ),
				'json'  => [
					'overlayImages' => array_merge( [ '' => __( 'Off', 'gohigh-seo' ) ], Helper::choices_overlay_images( 'names' ) ),
				],
			],
			'local'    => [
				...
				'desc'      => sprintf( wp_kses_post( __( 'Optimize for local searches and Knowledge Graph using these settings. %s.', 'gohigh-seo' ) ), '<a href="' . KB::get( 'local-seo-settings', 'Options Panel Meta Local Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>' ),
				'after_row' => '<div class="notice notice-alt notice-info info inline rank-math-notice"><p>' . __( 'Use the <code>[rank_math_contact_info]</code> shortcode to display contact information in a nicely formatted way. You should also claim your business on Google if you have not already.', 'gohigh-seo' ) . '</p></div>',
			],
			'social'   => [
				'icon'  => 'rm-icon rm-icon-social',
				'title' => esc_html__( 'Social Meta', 'gohigh-seo' ),
				/* translators: Link to social setting KB article */
				'desc'  => sprintf( esc_html__( "Add social account information to your website's Schema and Open Graph. %s.", 'gohigh-seo' ), '<a href="' . KB::get( 'social-meta-settings', 'Options Panel Meta Social Tab' ) . '" target="_blank">' . esc_html__( 'Learn More', 'gohigh-seo' ) . '</a>' ),
			],
			'homepage' => [
				'icon'  => 'rm-icon rm-icon-home',
				'title' => esc_html__( 'Homepage', 'gohigh-seo' ),
				'desc'  => sprintf(
					/* translators: Link to KB article */
					esc_html__( 'Add SEO meta and OpenGraph details to your homepage. %s.', 'gohigh-seo' ),
					'<a href="' . KB::get( 'homepage-settings', 'Options Panel Meta Home Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>'
				),
				'json'  => [
			...
			'author'   => [
				...
				'title' => esc_html__( 'Authors', 'gohigh-seo' ),
				/* translators: Link to KB article */
				'desc'  => sprintf( esc_html__( 'Change SEO options related to the author archives. %s.', 'gohigh-seo' ), '<a href="' . KB::get( 'author-settings', 'Options Panel Meta Author Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>' ),
				'json'  => [
			...
			'misc'     => [
				'icon'  => 'rm-icon rm-icon-misc',
				'title' => esc_html__( 'Misc Pages', 'gohigh-seo' ),
				/* translators: Link to KB article */
				'desc'  => sprintf( esc_html__( 'Customize SEO meta settings of pages like search results, 404s, etc. %s.', 'gohigh-seo' ), '<a href="' . KB::get( 'misc-settings', 'Options Panel Meta Misc Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>' ),
			],
		];
		...
		new Register_Options_Page(
			[
				...
				'title'      => esc_html__( 'SEO Titles &amp; Meta', 'gohigh-seo' ),
				'menu_title' => esc_html__( 'Titles &amp; Meta', 'gohigh-seo' ),
	...
	public function title_post_type_settings( $tabs ) {
		...
		$links = [
			'post'       => '<a href="' . KB::get( 'post-settings', 'Options Panel Meta Posts Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>.',
			'page'       => '<a href="' . KB::get( 'page-settings', 'Options Panel Meta Pages Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>.',
			'product'    => '<a href="' . KB::get( 'product-settings', 'Options Panel Meta Products Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>.',
			'attachment' => '<a href="' . KB::get( 'media-settings', 'Options Panel Meta Attachments Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>.',
		];
		...
		$tabs['p_types'] = [
			'title' => esc_html__( 'Post Types:', 'gohigh-seo' ),
			'type'  => 'separator',
		...
		foreach ( Helper::get_accessible_post_types() as $post_type ) {
			...
			$tabs[ 'post-type-' . $obj->name ] = [
				'title'     => $is_attachment ? esc_html__( 'Attachments', 'gohigh-seo' ) : $obj->label,
				'icon'      => isset( $icons[ $obj->name ] ) ? $icons[ $obj->name ] : $icons['default'],
				/* translators: 1. post type name 2. link */
				'desc'      => sprintf( esc_html__( 'Change Global SEO, Schema, and other settings for %1$s. %2$s', 'gohigh-seo' ), $obj_name, $link ),
				'post_type' => $obj->name,
				...
				'json'      => [
					...
					$post_type            => [
						'title'                => $is_attachment ? esc_html__( 'Attachments', 'gohigh-seo' ) : $obj->label,
						'name'                 => $is_attachment ? esc_html__( 'Media', 'gohigh-seo' ) : $obj->labels->singular_name,
						'schemaTypes'          => Helper::choices_rich_snippet_types( esc_html__( 'None (Click here to set one)', 'gohigh-seo' ), $post_type ),
	...
	public function title_taxonomy_settings( $tabs ) {
		...
		$hash_link = [
			'category'    => '<a href="' . KB::get( 'category-settings', 'Options Panel Meta Categories Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>.',
			'post_tag'    => '<a href="' . KB::get( 'tag-settings', 'Options Panel Meta Tags Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>.',
			'product_cat' => '<a href="' . KB::get( 'product-categories-settings', 'Options Panel Meta Product Categories Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>.',
			'product_tag' => '<a href="' . KB::get( 'product-tags-settings', 'Options Panel Meta Product Tags Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>.',
		];
		...
		foreach ( $taxonomies_data as $attached => $taxonomies ) {
			...
			foreach ( $taxonomies as $taxonomy ) {
				...
				$tabs[ 'taxonomy-' . $taxonomy->name ] = [
					...
					'desc'     => sprintf( esc_html__( 'Change Global SEO, Schema, and other settings for %1$s. %2$s', 'gohigh-seo' ), $taxonomy_name, $link ),
		...
		if ( isset( $tabs['taxonomy-post_format'] ) ) {
			...
			$tab['title']      = esc_html__( 'Post Formats', 'gohigh-seo' );
			$tab['page_title'] = esc_html__( 'Post Formats Archive', 'gohigh-seo' );
	...
	public static function maybe_update_analytics( $settings, $updated ) {
		...
		if ( in_array( 'searchConsole', $updated, true ) && ! empty( $search_console ) ) {
			...
			$response = \GoHighSEO\Analytics\AJAX::get()->do_save_analytic_profile( $search_console );
		...
		if ( in_array( 'analyticsData', $updated, true ) && ! empty( $google_analytics ) ) {
			...
			$response = \GoHighSEO\Analytics\AJAX::get()->do_save_analytic_options( $google_analytics );
			if ( is_wp_error( $response ) ) {
				return [
					'error' => $response->get_error_message(),
				];
	...
	public function enqueue_settings_translations() {
		...
		wp_set_script_translations( 'rank-math-settings-chunk', 'gohigh-seo', rank_math()->plugin_dir() . 'languages/' );
		wp_set_script_translations( 'rank-math-options', 'gohigh-seo', rank_math()->plugin_dir() . 'languages/' );
		wp_set_script_translations( 'rank-math-components', 'gohigh-seo', rank_math()->plugin_dir() . 'languages/' );
	...
	private static function maybe_update_htaccess( $settings ) {
		...
		if ( ! is_super_admin() || ! Helper::has_cap( 'general' ) || ! Helper::has_cap( 'edit_htaccess' ) ) {
			return [
				'error' => esc_html__( 'You do not have permission to edit the .htaccess file.', 'gohigh-seo' ),
			];
		}
		...
		if ( ! self::do_htaccess_backup() ) {
			return [
				'error' => esc_html__( 'Failed to backup .htaccess file. Please check file permissions.', 'gohigh-seo' ),
			];
		}
		if ( ! self::do_htaccess_update( $content ) ) {
			return [
				'error' => esc_html__( 'Failed to update .htaccess file. Please check file permissions.', 'gohigh-seo' ),
			];
		}

		return [
			'success' => esc_html__( '.htaccess file updated successfully.', 'gohigh-seo' ),
		];
	}
