...
class Admin_Bar_Menu {
	...
	private function add_root_menu() {
		...
		$this->items['main'] = [
			...
			'title'    => '<span class="rank-math-icon">' . $this->get_icon() . '</span><span class="rank-math-text">' . esc_html__( 'Rank Math SEO', 'gohigh-seo' ) . '</span>',
			'href'     => Helper::get_admin_url( $first_menu ),
			'meta'     => [ 'title' => esc_html__( 'Rank Math Dashboard', 'gohigh-seo' ) ],
			'priority' => 10,
		];
		...
		if ( current_user_can( 'manage_options' ) ) {
			$this->add_sub_menu(
				'dashboard',
				[
					'title'    => esc_html__( 'Dashboard', 'gohigh-seo' ),
					'href'     => $this->items['main']['href'],
					'meta'     => [ 'title' => esc_html__( 'Dashboard', 'gohigh-seo' ) ],
					'priority' => 20,
				]
			);
		}
	...
	private function add_home_menu() {
		$this->add_sub_menu(
			'home',
			[
				'title'    => esc_html__( 'Homepage SEO', 'gohigh-seo' ),
				'href'     => Helper::get_settings_url( 'titles', 'homepage' ),
				'meta'     => [ 'title' => esc_html__( 'Edit Homepage SEO Settings', 'gohigh-seo' ) ],
				'priority' => 35,
			]
		);
	}
	...
	private function add_post_type_menu() {
		...
		if ( is_home() ) {
			$post_type = 'page';
			$name      = esc_html__( 'Pages', 'gohigh-seo' );
		}
		...
		$this->add_sub_menu(
			...
			[
				...
				'title'    => sprintf( esc_html__( 'SEO Settings for %s', 'gohigh-seo' ), $name ),
				'href'     => Helper::get_settings_url( 'titles', 'post-type-' . $post_type ),
				'meta'     => [ 'title' => esc_html__( 'Edit default SEO settings for this post type', 'gohigh-seo' ) ],
				'priority' => 35,
			]
		);
	}
	...
	private function add_taxonomy_menu() {
		...
		$this->add_sub_menu(
			...
			[
				...
				'title'    => sprintf( esc_html__( 'SEO Settings for %s', 'gohigh-seo' ), $labels->name ),
				'href'     => Helper::get_settings_url( 'titles', 'taxonomy-' . $term->taxonomy ),
				'meta'     => [ 'title' => esc_html__( 'Edit SEO settings for this archive page', 'gohigh-seo' ) ],
				'priority' => 35,
			]
		);
	}
	...
	private function add_date_menu() {
		$this->add_sub_menu(
			'date',
			[
				'title'    => esc_html__( 'SEO Settings for Date Archives', 'gohigh-seo' ),
	...
	private function add_search_menu() {
		$this->add_sub_menu(
			'search',
			[
				'title'    => esc_html__( 'SEO Settings for Search Page', 'gohigh-seo' ),
				'href'     => Helper::get_settings_url( 'titles', 'global' ),
				'meta'     => [ 'title' => esc_html__( 'Edit SEO settings for the search results page', 'gohigh-seo' ) ],
				'priority' => 35,
			]
		);
	}
	...
	private function add_mark_page_menu() {
		$this->add_sub_menu(
			'mark-me',
			[
				'title'    => esc_html__( 'Mark this page', 'gohigh-seo' ),
				'href'     => '#',
				'priority' => 100,
			]
		);
		...
		if ( is_singular( Helper::get_accessible_post_types() ) ) {
			...
			$this->add_sub_menu(
				'pillar-content',
				[
					'title' => $is_pillar_content . esc_html__( 'As Pillar Content', 'gohigh-seo' ),
					'href'  => '#pillar_content',
		...
		if ( Paper::get() ) {
			...
			$this->add_sub_menu(
				'no-index',
				[
					'title' => $noindex_check . esc_html__( 'As NoIndex', 'gohigh-seo' ),
					'href'  => '#noindex',
					'meta'  => [ 'class' => 'mark-page-as' ],
				],
				'mark-me'
			...
			$this->add_sub_menu(
				'no-follow',
				[
					'title' => $nofollow_check . esc_html__( 'As NoFollow', 'gohigh-seo' ),
					'href'  => '#nofollow',
					'meta'  => [ 'class' => 'mark-page-as' ],
				],
				'mark-me'
	...
	private function add_seo_tools() {
		$this->add_sub_menu(
			'third-party',
			[
				'title'    => esc_html__( 'External Tools', 'gohigh-seo' ),
				'href'     => '#',
				'priority' => 200,
			]
		);
		...
		$items = [
			'google-pagespeed'           => [
				'title' => esc_html__( 'Google PageSpeed', 'gohigh-seo' ),
				'href'  => 'https://developers.google.com/speed/pagespeed/insights/?url=' . $url,
				'meta'  => [ 'title' => esc_html__( 'Google PageSpeed Insights', 'gohigh-seo' ) ],
			],

			'google-richresults-mobile'  => [
				'title' => esc_html__( 'Google Rich Results (Mobile)', 'gohigh-seo' ),
				'href'  => 'https://search.google.com/test/rich-results?url=' . $url . '&user_agent=1',
				'meta'  => [ 'title' => esc_html__( 'Google Rich Results Test - Googlebot Smartphone', 'gohigh-seo' ) ],
			],

			'google-richresults-desktop' => [
				'title' => esc_html__( 'Google Rich Results (Desktop)', 'gohigh-seo' ),
				'href'  => 'https://search.google.com/test/rich-results?url=' . $url . '&user_agent=2',
				'meta'  => [ 'title' => esc_html__( 'Google Rich Results Test - Googlebot Desktop', 'gohigh-seo' ) ],
			],

			'fb-debugger'                => [
				'title' => esc_html__( 'Facebook Debugger', 'gohigh-seo' ),
				'href'  => 'https://developers.facebook.com/tools/debug/sharing/?q=' . $url,
				'meta'  => [ 'title' => esc_html__( 'Facebook Sharing Debugger', 'gohigh-seo' ) ],
			],
		];
