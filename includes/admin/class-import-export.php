...
class Import_Export implements Runner {
	...
	public function clean_plugin() {
		...
		if ( $result['status'] ) {
			...
			$this->success( sprintf( esc_html__( 'Cleanup of %s data successfully done.', 'gohigh-seo' ), $result['importer']->get_plugin_name() ) );
		}

		/* translators: Plugin name */
		$this->error( sprintf( esc_html__( 'Cleanup of %s data failed.', 'gohigh-seo' ), $result['importer']->get_plugin_name() ) );
	}
	...
	public function import_plugin() {
		...
		if ( ! $this->is_action_allowed( $perform ) ) {
			$this->error( esc_html__( 'Action not allowed.', 'gohigh-seo' ) );
		}
