...
class Dashboard_Widget {
	...
	public function add_dashboard_widgets() {
		...
		wp_add_dashboard_widget(
			...
			$icon . esc_html__( 'Rank Math Overview', 'gohigh-seo' ),
	...
	public function dashboard_widget_feed() {
		$posts = $this->get_feed();
		?>
		<h3 class="rank-math-blog-title"><?php esc_html_e( 'Latest Blog Posts from Rank Math', 'gohigh-seo' ); ?></h3>
		<?php if ( empty( $posts ) ) : ?>
			<p><?php esc_html_e( 'Error: the Rank Math blog feed could not be downloaded.', 'gohigh-seo' ); ?></p>
			<?php
			return;
		endif;
	...
	private function get_item_label( $posts ) {
		...
		if ( $is_new && empty( $label ) ) {
			$label = esc_html__( 'NEW', 'gohigh-seo' );
		}
