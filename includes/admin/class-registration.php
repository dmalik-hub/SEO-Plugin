...
class Registration {
	...
	private function get_registration_url( $status ) {
		if ( 'cancel' === $status ) {
			...
			Helper::add_notification( __( 'Rank Math plugin could not be connected.', 'gohigh-seo' ), [ 'type' => 'error' ] );
			return Security::remove_query_arg_raw( [ 'rankmath_connect', 'rankmath_auth' ] );
		}

		if ( 'banned' === $status ) {
			// User or site banned.
			Helper::add_notification( __( 'Unable to connect Rank Math.', 'gohigh-seo' ), [ 'type' => 'error' ] );
			return Security::remove_query_arg_raw( [ 'rankmath_connect', 'rankmath_auth' ] );
		}
