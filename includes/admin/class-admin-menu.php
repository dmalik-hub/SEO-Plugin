...
class Admin_Menu implements Runner {
	...
	public function register_pages() {
		...
		new Page(
			'rank-math',
			esc_html__( 'Rank Math', 'gohigh-seo' ),
			[
				'position'   => 50,
	...
	public function fix_admin_menu() {
		...
		foreach ( $menu as $key => $item ) {
			if ( 'Rank Math' === $item[0] ) {
				$menu[ $key ][0] = esc_html__( 'Rank Math SEO', 'gohigh-seo' ); // phpcs:ignore -- This is required to change the menu name without changing its slug `rank-math`
				break;
			}
		}
		...
		if ( 'Rank Math' === $submenu['rank-math'][0][0] ) {
			if ( current_user_can( 'manage_options' ) ) {
				...
				$submenu['rank-math'][0][0] = esc_html__( 'Dashboard', 'gohigh-seo' ) . $notification; // phpcs:ignore -- This is required to change the menu name when the plugin is not configured.
			} else {
		...
		$submenu['rank-math'][] = [ esc_html__( 'Help &amp; Support', 'gohigh-seo' ) . '<i class="dashicons dashicons-external" style="font-size:12px;vertical-align:-2px;height:10px;"></i>', 'manage_options', KB::get( 'knowledgebase', 'Sidebar Help Link' ) ]; // phpcs:ignore -- A custom link to our KB article.
	...
	private function get_active_offer() {
		...
		$dates        = [
			'halloween'    => [
				'start' => '2025-10-29',
				'end'   => '2025-11-05',
				'text'  => esc_html__( 'Halloween Sale', 'gohigh-seo' ),
			],
			'anniversary'  => [
				'start' => '2025-11-05',
				'end'   => '2025-11-12',
				'text'  => esc_html__( 'Anniversary Sale', 'gohigh-seo' ),
			],
			'black-friday' => [
				'start' => '2025-11-12',
				'end'   => '2025-11-29',
				'text'  => esc_html__( 'Black Friday Sale', 'gohigh-seo' ),
			],
			'cyber-monday' => [
				'start' => '2025-11-29',
				'end'   => '2025-12-03',
				'text'  => esc_html__( 'Cyber Monday Sale', 'gohigh-seo' ),
			],
			'christmas'    => [
				'start' => '2025-12-24',
				'end'   => '2025-12-31',
				'text'  => esc_html__( 'Christmas Sale', 'gohigh-seo' ),
			],
			'new-year'     => [
				'start' => '2025-12-31',
				'end'   => '2026-01-07',
				'text'  => esc_html__( 'New Year Sale', 'gohigh-seo' ),
			],
		];
