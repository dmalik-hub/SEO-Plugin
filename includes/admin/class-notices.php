...
class Notices implements Runner {
	...
	private function react_settings_ui_notice() {
		...
		if ( ! Helper::is_react_enabled() ) {
			$message = sprintf(
				...
				__( 'The React Settings UI is currently disabled, and the classic settings interface is active. Note: The PHP-based settings interface will be removed in an upcoming release. %1$sEnable the React Settings UI%2$s to switch back.', 'gohigh-seo' ),
	...
	public function notice_dismissible( $notification_id ) {
		if ( 'new_post_type' === $notification_id ) {
			...
			if ( Helper::is_module_active( 'sitemap' ) ) {
				\GoHighSEO\Sitemap\Cache::invalidate_storage();
			}
			return;
		}
	...
	private function new_post_type() {
		...
		$message = __( 'Rank Math has detected a new post type: %1$s. You may want to check the settings of the <a href="%2$s">Titles &amp; Meta page</a>.', 'gohigh-seo' );
		$count   = count( $new );
		if ( $count > 1 ) {
			/* Translators: placeholder is the post type names separated with commas. */
			$message = __( 'Rank Math has detected new post types: %1$s. You may want to check the settings of the <a href="%2$s">Titles &amp; Meta page</a>.', 'gohigh-seo' );
		}
	...
	private function display_wpml_notice() {
		...
		Helper::add_notification(
			__( 'Please activate the WPML String Translation plugin to convert Rank Math Setting values in different languages.', 'gohigh-seo' ),
			[
				'type' => 'error',
	...
	public function add_permalink_changes_warning() {
		...
		$message = __( '<b>Rank Math Warning:</b> Changing the permalinks on a live, indexed site may result in serious loss of traffic if done incorrectly. Consider adding a new redirection from the old URL format to the new one.', 'gohigh-seo' );
		Helper::add_notification(
			$message,
			[
				'type'    => 'warning',
