...
use GoHighSEO\Google\Permissions;
use GoHighSEO\Analytics\Url_Inspection;
use GoHighSEO\Helpers\Str;
use GoHighSEO\Google\Analytics;
...
$connections = [
	'reconnect'  => [
		...
		'text'  => esc_html__( 'Reconnect', 'gohigh-seo' ),
	],
	'disconnect' => [
		'link'  => '#',
		'class' => 'rank-math-disconnect-google',
		'text'  => esc_html__( 'Disconnect', 'gohigh-seo' ),
	],
];
...
if ( Helper::is_advanced_mode() && ( $is_profile_connected || $is_adsense_connected || $is_analytics_connected ) ) {
	$connections['test-connections'] = [
		...
		'text'  => esc_html__( 'Test Connections', 'gohigh-seo' ),
	];
}
...
<div class="<?php echo esc_attr( $console_classes ); ?>" tabindex="0">
	<header>
		<h3><span class="rank-math-connection-status-wrap"><span class="<?php echo esc_attr( $console_status_classes ); ?>" title="<?php echo esc_attr( $console_status ); ?>"></span></span> <?php esc_html_e( 'Search Console', 'gohigh-seo' ); ?></h3>
	</header>
	<div class="rank-math-accordion-content">
		...
		<div class="cmb-row cmb-type-select">
			<div class="cmb-row-col">
				<label for="site-console-profile"><?php esc_html_e( 'Site', 'gohigh-seo' ); ?></label>
		...
		<div class="cmb-row cmb-type-toggle">
			<div class="cmb-td">
				...
				<label for="enable-index-status"><?php esc_html_e( 'Enable the Index Status tab', 'gohigh-seo' ); ?></label>
				<div class="cmb2-metabox-description"><?php esc_html_e( 'Enable this option to show the Index Status tab in the Analytics module.', 'gohigh-seo' ); ?> <a href="<?php echo KB::get( 'url-inspection-api', 'SW Analytics Index Status Option' ); // phpcs:ignore ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Learn more.', 'gohigh-seo' ); ?></a></div>
			</div>
		</div>

		<?php if ( ! Console::is_valid_connection() ) : ?>
			<div class="rank-math-notice rank-math-notice--error">
				<p><?php esc_html_e( 'Data import will not work for this service as sufficient permissions are not given.', 'gohigh-seo' ); ?></p>
			</div>
		<?php endif; ?>
	</div>
</div>
...
<div class="<?php echo esc_attr( $analytic_classes ); ?>" tabindex="0">
	<header>
		<h3><span class="rank-math-connection-status-wrap"><span class="<?php echo esc_attr( $analytic_status_classes ); ?>" title="<?php echo esc_attr( $analytic_status ); ?>"></span></span><?php esc_html_e( 'Analytics', 'gohigh-seo' ); ?></h3>
	</header>
	<div class="rank-math-accordion-content rank-math-analytics-content">
		...
		<p class="warning yellow">
			<strong class="note"><?php echo esc_html__( 'Note', 'gohigh-seo' ); ?></strong>
			<?php
			printf(
				/* translators: %s: Link to KB article */
				esc_html__( 'Ready to switch to Google Analytics 4? %s', 'gohigh-seo' ),
				'<a href="' . KB::get( 'using-ga4', 'Analytics GA4 KB' ) . '" target="_blank">' . esc_html__( 'Click here to know how', 'gohigh-seo' ) . '</a>.' // phpcs:ignore
			);
			?>
		</p>
		...
		<div class="cmb-row cmb-type-select">
			<div class="cmb-row-col">
				<label for="site-analytics-account"><?php esc_html_e( 'Account', 'gohigh-seo' ); ?></label>
			...
			<div class="cmb-row-col">
				<label for="site-analytics-property"><?php esc_html_e( 'Property', 'gohigh-seo' ); ?></label>
			...
			<div class="cmb-row-col">
				...
				<?php echo esc_html__( 'Data Stream', 'gohigh-seo' ); ?>
				</label>
		...
		<div class="cmb-row cmb-type-toggle">
			<div class="cmb-td">
				...
				<label for="install-code"><?php esc_html_e( 'Install analytics code', 'gohigh-seo' ); ?></label>
				<div class="cmb2-metabox-description"><?php esc_html_e( 'Enable this option only if you are not using any other plugin/theme to install Google Analytics code.', 'gohigh-seo' ); ?></div>
			</div>
		</div>
		...
		<div class="cmb-row cmb-type-toggle <?php echo ! $is_pro_active ? 'cmb-redirector-element' : ''; ?>" <?php echo ! $is_pro_active ? 'data-url="' . KB::the( 'free-vs-pro', 'Anonymize IP' ) . '"' : ''; // phpcs:ignore ?>>
			<div class="cmb-td">
				...
				<label for="anonymize-ip">
					<?php esc_html_e( 'Anonymize IP addresses', 'gohigh-seo' ); ?>
					<?php if ( ! $is_pro_active ) : ?>
					<span class="gohigh-seo-badge">
						<a href="<?php KB::the( 'pro', 'Anonymize IP' ); ?>" target="_blank" rel="noopener noreferrer">
							<?php esc_html_e( 'PRO', 'gohigh-seo' ); ?>
						</a>
					</span>
					<?php endif; ?>
				</label>
		...
		<div class="cmb-row cmb-type-toggle <?php echo ! $is_pro_active ? 'cmb-redirector-element' : ''; ?>" <?php echo ! $is_pro_active ? 'data-url="' . KB::the( 'pro', 'Localjs IP' ) . '"' : ''; // phpcs:ignore ?>>
			<div class="cmb-td">
				...
				<label for="local-ga-js">
					<?php esc_html_e( 'Self-Hosted Analytics JS File', 'gohigh-seo' ); ?>
					<?php if ( ! $is_pro_active ) : ?>
					<span class="gohigh-seo-badge">
		...
		<div class="cmb-row cmb-type-toggle">
			<div class="cmb-td">
				...
				<label for="exclude-loggedin"><?php esc_html_e( 'Exclude Logged-in users', 'gohigh-seo' ); ?></label>
				<div class="rank-math-cmb-dependency hidden" data-relation="or">
					<span class="hidden" data-field="install-code" data-comparison="=" data-value="on"></span>
				</div>
			</div>
...
<div class="rank-math-box no-padding rank-math-accordion rank-math-connect-adsense disconnected" tabindex="0">
	<header>
		<h3>
			...
			</span><?php esc_html_e( 'AdSense', 'gohigh-seo' ); ?>
		</h3>
	</header>
	<div class="rank-math-accordion-content">
		<div class="cmb-row cmb-type-select">
			<div class="cmb-row-col">
				<label for="site-adsense-account"><?php esc_html_e( 'Account', 'gohigh-seo' ); ?></label>
				<select class="cmb2_select site-adsense-account notrack" name="site-adsense-account" id="site-adsense-account" data-selected="" disabled="disabled">
					<option value=""><?php esc_html_e( 'Select Account', 'gohigh-seo' ); ?></option>
				</select>
			</div>
		</div>
		<div id="gohigh-seo-cta" class="no-margin">
			<div class="rank-math-cta-text">
				<span class="gohigh-seo-badge">
					<a href="<?php KB::the( 'pro', 'AdSense Toggle' ); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'PRO', 'gohigh-seo' ); ?></a></span> <?php esc_html_e( "Google AdSense support is only available in Rank Math Pro's Advanced Analytics module.", 'gohigh-seo' ); ?>
			</div>
		</div>
	</div>
</div>
...
<div id="gohigh-seo-cta" class="rank-math-privacy-box width-100">
	<div class="rank-math-cta-table">
		<div class="rank-math-cta-body less-padding">
			...
			<?php
			/* translators: %s: Link to KB article */
			printf( esc_html__( 'We do not store any of the data from your Google account on our servers, everything is processed & stored on your server. We take your privacy extremely seriously and ensure it is never misused. %s', 'gohigh-seo' ), '<a href="' . KB::get( 'usage-policy', 'Analytics Privacy Notice' ) . '" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Learn more.', 'gohigh-seo' ) . '</a>' ); // phpcs:ignore
			?>
			</p>
		</div>
	</div>
...
if ( Helper::is_wizard() && ! RankMath\Analytics\Email_Reports::are_fields_hidden() ) {
	...
	<div class="cmb-row email-reports-header text-center" style="border-top:0;">
		<h1><?php esc_html_e( 'Email Reports', 'gohigh-seo' ); ?></h1>
		<div class="email-reports-desc text-center"><?php esc_html_e( 'Receive Analytics reports periodically in email.', 'gohigh-seo' ); ?> <a href="#" target="_blank"><?php esc_html_e( 'Learn more about Email Reports.', 'gohigh-seo' ); ?></a></div>
	</div>
	<div class="cmb-row cmb-type-toggle cmb2-id-console-email-reports" data-fieldtype="toggle">
		<div class="cmb-th">
			<label for="console_email_reports"><?php esc_html_e( 'Email Reports', 'gohigh-seo' ); ?></label>
		</div>
		<div class="cmb-td">
