...
class CMB2_Options {
	...
	public function register_option_page() {
		...
		foreach ( $tabs as $id => $tab ) {
			...
			$cmb->add_field(
				[
					'name'    => esc_html__( 'Panel', 'gohigh-seo' ),
					'id'      => 'setting-panel-' . $id,
					'type'    => 'tab',
					'open'    => true,
		...
		<?php if ( ! defined( 'RANK_MATH_PRO_FILE' ) ) : ?>
			<div class="rank-math-unlock-pro-notice" id="rank-math-unlock-pro-notice">
				<a href="<?php KB::the( 'pro', 'Unlock PRO Options Panel Notice' ); ?>" target="_blank" class="pro-link">
					<p>
						<?php esc_html_e( 'Take your SEO to the Next Level!', 'gohigh-seo' ); ?>
						<strong><?php esc_html_e( 'Get Rank Math PRO!', 'gohigh-seo' ); ?></strong>
						<span><?php esc_html_e( 'Click here to see all the exciting features.', 'gohigh-seo' ); ?></span>
					</p>
		...
		<div class="wrap rank-math-wrap rank-math-wrap-settings">
			...
			<form class="cmb-form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="POST" id="<?php echo esc_attr( $cmb->cmb_id ); ?>" enctype="multipart/form-data" encoding="multipart/form-data">
				...
				<footer class="form-footer rank-math-ui settings-footer wp-clearfix">
					...
					<input type="submit" name="submit-cmb" id="submit-cmb" title="<?php echo esc_html__( 'Ctrl/Cmd + Enter', 'gohigh-seo' ); ?>" class="button button-primary save-options" value="<?php esc_attr_e( 'Save Changes', 'gohigh-seo' ); ?>">
					<input type="submit" name="reset-cmb" id="rank-math-reset-cmb" value="<?php esc_attr_e( 'Reset Options', 'gohigh-seo' ); ?>" class="button button-secondary reset-options alignleft">
				</footer>
	...
	private function maybe_update_htaccess() {
		...
		if ( ! is_super_admin() || ! Helper::has_cap( 'general' ) || ! Helper::has_cap( 'edit_htaccess' ) ) {
			Helper::add_notification(
				esc_html__( 'You do not have permission to edit the .htaccess file.', 'gohigh-seo' ),
				[ 'type' => 'error' ]
			);
			return;
		}
		...
		if ( ! $this->do_htaccess_backup() ) {
			Helper::add_notification(
				esc_html__( 'Failed to backup .htaccess file. Please check file permissions.', 'gohigh-seo' ),
				[ 'type' => 'error' ]
			);
			return;
		}
		if ( ! $this->do_htaccess_update( $content ) ) {
			Helper::add_notification(
				esc_html__( 'Failed to update .htaccess file. Please check file permissions.', 'gohigh-seo' ),
				[ 'type' => 'error' ]
			);
			return;
		}

		Helper::add_notification( esc_html__( '.htaccess file updated successfully.', 'gohigh-seo' ) );
	}
