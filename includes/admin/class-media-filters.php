...
class Media_Filters {
	...
	public function enqueue_media() {
		wp_enqueue_script( 'media-library-seo-filter', RANK_MATH_PRO_URL . 'assets/admin/js/media.js', [ 'media-editor', 'media-views', 'media-models' ], rank_math()->version ); // phpcs:ignore
		wp_localize_script(
			'media-library-seo-filter',
			'RankMathProMedia',
			[
				'filters'    => $this->get_filters(),
				'filter_all' => __( 'Rank Math SEO Filters', 'gohigh-seo' ),
			]
		);
	}
	...
	public function add_seo_filter() {
		...
		<select id="media-attachment-seo-filter" name="seo-filter" class="attachment-filters">
			<option value="all"><?php echo esc_html__( 'Rank Math SEO Filters', 'gohigh-seo' ); ?></option>
	...
	private function get_filters() {
		return [
			'missing_alt'     => __( 'Missing alt tag', 'gohigh-seo' ),
			'missing_title'   => __( 'Missing or default title tag', 'gohigh-seo' ),
			'missing_caption' => __( 'Missing caption', 'gohigh-seo' ),
		];
	}
}
