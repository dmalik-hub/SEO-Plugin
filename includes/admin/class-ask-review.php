...
class Ask_Review {
	...
	public function hooks() {
		...
		if ( $this->current_time > $review_notice_date ) {
			if ( get_option( 'rank_math_review_notice_added' ) === false && ! Helper::has_notification( 'rank_math_notice' ) ) {
				$this->add_notice();
			}
	...
	public function review_notice_js() {
		...
		<style>
			...
			#rank_math_notice .rank-math-notice.is-dismissible a {
				color: #4f52d4;
			}
			...
			#rank_math_notice.is-dismissible:before {
				content: '';
				width: 50px;
				height: 100%;
			...
			#rank_math_notice.is-dismissible:after {
	...
	public function add_notice() {
		...
		if ( $show_wordpress ) {
			$selected_link = 'https://wordpress.org/support/plugin/gohigh-seo/reviews/?filter=5#new-post';
		} else {
		...
		$message .= sprintf( esc_html__( 'Hey, we noticed you\'ve been using %s for more than a week now – that\'s awesome!', 'gohigh-seo' ), '<strong>' . _x( 'Rank Math SEO', 'plugin name inside the review notice', 'gohigh-seo' ) . '</strong>' );
		$message .= '<br>';

		if ( $show_wordpress ) {
			$message .= esc_html__( 'We would love to get your feedback! It\'s essential for our continued development. Please consider taking a moment to leave us a review.', 'gohigh-seo' );
		} else {
			$message .= esc_html__( 'We would love to get your feedback! It\'s essential for our continued development. Please consider taking a moment to leave a review of your experience on Trustpilot.', 'gohigh-seo' );
		}

		$message .= '</p>
			<p><strong>Bhanu Ahluwalia</strong><br>' . esc_html__( 'Co-founder of Rank Math', 'gohigh-seo' ) . '</p>
			<p>
				<a href="' . esc_url( $selected_link ) . '" class="rank-math-dismiss-review-notice rank-math-review-action rank-math-review-out" target="_blank" rel="noopener noreferrer"><strong>' . esc_html__( 'Yes, you deserve it', 'gohigh-seo' ) . '</strong></a><br>
				<a href="#" class="rank-math-dismiss-review-notice rank-math-maybe-later-action">' . esc_html__( 'No, maybe later', 'gohigh-seo' ) . '</a><br>
				<a href="#" class="rank-math-dismiss-review-notice rank-math-already-reviewed-action">' . esc_html__( 'I already did', 'gohigh-seo' ) . '</a>
			</p>';
