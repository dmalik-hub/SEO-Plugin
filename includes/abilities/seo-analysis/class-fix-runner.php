...
namespace GoHighSEO\Abilities\SEO_Analysis;
...
class Fix_Runner {
		...
		if ( ! method_exists( $this, $handler ) ) {
			return $this->cannot_fix(
				sprintf(
					...
					__( 'No automatic fix is available for test "%s". Use the fix_text guidance to resolve it manually.', 'gohigh-seo' ),
					$test_id
				)
			);
		}
	...
	protected function fix_blog_public( array $input ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
		...
		return $this->ok( __( 'Site visibility set to public. Search engines can now index your site.', 'gohigh-seo' ) );
	}
	...
	protected function fix_permalink_structure( array $input ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
		...
		if ( '/%postname%/' === get_option( 'permalink_structure' ) ) {
			return $this->ok( __( 'Permalink structure is already set to /%postname%/.', 'gohigh-seo' ) );
		}
		...
		return $this->ok( __( 'Permalink structure set to /%postname%/ and rewrite rules flushed.', 'gohigh-seo' ) );
	}
	...
	protected function fix_site_description( array $input ) {
		...
		if ( empty( $value ) ) {
			return $this->cannot_fix(
				__( 'A "value" input is required to set the site tagline. Provide a short, unique description of your site.', 'gohigh-seo' )
			);
		}

		if ( 'just another wordpress site' === strtolower( $value ) ) { // phpcs:ignore WordPress.WP.CapitalPDangit.MisspelledInText
			return $this->cannot_fix(
				__( 'The provided tagline is the default WordPress value. Please supply a unique tagline.', 'gohigh-seo' )
			);
		}
		...
		return $this->ok(
			sprintf(
				...
				__( 'Site tagline updated to: "%s".', 'gohigh-seo' ),
				$value
			)
		);
	}
	...
	protected function fix_sitemaps( array $input ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
		...
		return $this->ok( __( 'Sitemap module enabled. Your XML sitemap is now active.', 'gohigh-seo' ) );
	}
	...
	protected function fix_schema( array $input ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
		...
		return $this->ok( __( 'Schema / rich snippet module enabled.', 'gohigh-seo' ) );
	}
	...
	protected function fix_noindex( array $input ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
		...
		return $this->ok( __( 'Global robots setting updated to allow indexing. Run a fresh audit to confirm the remote API test passes.', 'gohigh-seo' ) );
	}
	...
	protected function fix_opengraph( array $input ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
		return $this->cannot_fix(
			__( 'OpenGraph output is always enabled in Rank Math. If this test is failing, the remote audit API may need a fallback Open Graph image. Set one at Rank Math → Titles & Meta → Global Meta → Open Graph Thumbnail.', 'gohigh-seo' )
		);
	}
	...
	protected function fix_robots_txt( array $input ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
		...
		if ( isset( $general['robots_txt_content'] ) && '' !== trim( $general['robots_txt_content'] ) ) {
			return $this->ok( __( 'Robots.txt content is already configured in Rank Math settings.', 'gohigh-seo' ) );
		}

		$general['robots_txt_content'] = "User-agent: *\nDisallow: /wp-admin/\nAllow: /wp-admin/admin-ajax.php";
		update_option( 'rank-math-options-general', $general );

		return $this->ok( __( 'Default robots.txt content written to Rank Math settings.', 'gohigh-seo' ) );
	}
	...
	protected function fix_focus_keywords( array $input ) {
		...
		if ( empty( $types ) ) {
			return $this->cannot_fix( __( 'No indexable post types are configured in Rank Math.', 'gohigh-seo' ) );
		}
		...
		if ( empty( $posts ) ) {
			return $this->ok( __( 'All published posts already have a focus keyword set.', 'gohigh-seo' ) );
		}
		...
		$summary = sprintf(
			...
			__( 'Set focus keywords on %1$d posts (derived from post titles).', 'gohigh-seo' ),
			$updated
		);
		...
		if ( $truncated ) {
			$summary .= ' ' . sprintf(
				...
				__( 'Results were limited to %d posts — run again to continue.', 'gohigh-seo' ),
				$limit
			);
		}
	...
	protected function fix_post_titles( array $input ) {
		...
		if ( empty( $rows ) ) {
			return $this->ok( __( 'All posts with focus keywords already include them in their titles.', 'gohigh-seo' ) );
		}
		...
		$summary = sprintf(
			...
			__( 'Updated focus keywords on %d posts to match their titles.', 'gohigh-seo' ),
			$updated
		);
