...
class Variable {
	...
	public static function from( $id, $args ) {
		if ( empty( $id ) ) {
			throw new \InvalidArgumentException( esc_html__( 'The $id variable is required.', 'gohigh-seo' ) );
		}

		$variable          = new Variable();
		$variable->id      = $id;
		$variable->example = isset( $args['example'] ) ? $args['example'] : esc_html__( 'Example', 'gohigh-seo' );

		foreach ( self::$required as $key ) {
			if ( ! isset( $args[ $key ] ) ) {
				/* translators: variable name */
				throw new \InvalidArgumentException( sprintf( esc_html__( 'The $%1$s is required for variable %2$s.', 'gohigh-seo' ), esc_html( $key ), esc_html( $id ) ) );
			}
