...
use GoHighSEO\Admin\CSV_Import_Export\CSV_Import_Export;
...
class Rest extends WP_REST_Controller {
	...
	public function has_search_intent_permission( WP_REST_Request $request ) {
		return \GoHighSEO\Helper::has_cap( 'onpage_general' ) && ! empty( Admin_Helper::get_registration_data() );
	}
	...
	public function import_data_from_csv( WP_REST_Request $request ) {
		...
		if ( empty( $import_file ) || empty( $import_file['name'] ) ) {
			return new WP_REST_Response(
				[
					'success' => false,
					'message' => esc_html__( 'Please select a file to import.', 'gohigh-seo' ),
				]
			);
		}
		...
		if ( isset( $file_import['error'] ) ) {
			return new WP_REST_Response(
				[
					'success' => false,
					'message' => $file_import['error'] ?? esc_html__( 'Something went wrong! Please try again later.', 'gohigh-seo' ),
				]
			);
		}
	...
	private function get_ping_settings_args() {
		return [
			'apiKey'   => [
				'description'       => esc_html__( 'API Key.', 'gohigh-seo' ),
				'type'              => 'string',
				'required'          => true,
			...
			'username' => [
				'description'       => esc_html__( 'Username.', 'gohigh-seo' ),
				'type'              => 'string',
				'required'          => true,
				'sanitize_callback' => 'rest_sanitize_request_arg',
				'validate_callback' => 'rest_validate_request_arg',
			...
			'plan'     => [
				'description'       => esc_html__( 'Plan.', 'gohigh-seo' ),
				'type'              => 'string',
				'required'          => true,
				'sanitize_callback' => 'rest_sanitize_request_arg',
				'validate_callback' => 'rest_validate_request_arg',
			...
			'keywords' => [
				'description'       => esc_html__( 'Keywords.', 'gohigh-seo' ),
				'type'              => 'string',
				'required'          => true,
				'sanitize_callback' => 'rest_sanitize_request_arg',
				'validate_callback' => 'rest_validate_request_arg',
			...
			'settings' => [
				'description'       => esc_html__( 'Settings.', 'gohigh-seo' ),
				'type'              => 'string',
				'required'          => true,
				'sanitize_callback' => 'rest_sanitize_request_arg',
				'validate_callback' => 'rest_validate_request_arg',
	...
	private function get_search_intent_args() {
		return [
			'keyword' => [
				'description' => esc_html__( 'The keyword used to determine the intent.', 'gohigh-seo' ),
				'type'        => 'string',
				'required'    => true,
			],
		];
