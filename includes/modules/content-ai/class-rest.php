...
class Rest extends WP_REST_Controller {
	...
	public function __construct() {
		$this->namespace  = \GoHighSEO\Rest\Rest_Helper::BASE . '/ca';
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'content' => [
						'description' => esc_html__( 'The content of the new post.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
					],
					'title'   => [
						'description' => esc_html__( 'The title of the new post.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => false,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'outputs'    => [
						'description' => esc_html__( 'An array of AI-generated and existing outputs to be saved.', 'gohigh-seo' ),
						'type'        => 'array',
						'required'    => false,
					],
					'endpoint'   => [
						'description' => esc_html__( 'The API endpoint for which the output was generated.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => false,
					],
					'isChat'     => [
						'description' => esc_html__( 'Indicates if the request was for the Chat endpoint.', 'gohigh-seo' ),
						'type'        => 'boolean',
						'required'    => false,
					],
					'attributes' => [
						'description' => esc_html__( 'The parameters used to generate the AI output.', 'gohigh-seo' ),
						'type'        => 'object',
						'required'    => false,
					],
					'credits'    => [
						'description' => esc_html__( 'Credit usage details returned by the API.', 'gohigh-seo' ),
						'type'        => 'object',
						'required'    => false,
					],
					'usage'      => [
						'description' => esc_html__( 'Per-tool usage data returned by the API.', 'gohigh-seo' ),
						'type'        => 'object',
						'required'    => false,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'isChat' => [
						'description' => esc_html__( 'Indicates if the request to delete the output was for the Chat endpoint.', 'gohigh-seo' ),
						'type'        => 'boolean',
						'required'    => false,
					],
					'index'  => [
						'description' => esc_html__( 'The output index to delete, applicable only to the Chat endpoint.', 'gohigh-seo' ),
						'type'        => 'integer',
						'required'    => false,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'prompt' => [
						'description' => esc_html__( 'The selected prompt to be updated in the recent prompts.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'prompt' => [
						'description'       => esc_html__( 'The prompt data to be saved in the database.', 'gohigh-seo' ),
						'required'          => true,
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'prompts' => [
						'description' => esc_html__( 'A list of prompts received from the API to be saved in the database.', 'gohigh-seo' ),
						'type'        => 'array',
						'required'    => true,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'plan'        => [
						'description' => esc_html__( 'Content AI plan to update in the Database.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
					],
					'refreshDate' => [
						'description' => esc_html__( 'Content AI reset date to update in the Database', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'attachmentIds' => [
						'description' => esc_html__( 'List of attachment IDs for which to generate alt text.', 'gohigh-seo' ),
						'type'        => 'array',
						'required'    => true,
					],
				],
	...
	public function research_keyword( WP_REST_Request $request ) {
		...
		if ( ! in_array( $post_type, (array) Helper::get_settings( 'general.content_ai_post_types' ), true ) ) {
			return [
				'data' => esc_html__( 'Content AI is not enabled on this Post type.', 'gohigh-seo' ),
			];
		}
	...
	public function get_research_keyword_args() {
		return [
			'keyword'      => [
				'description' => esc_html__( 'The keyword to be researched.', 'gohigh-seo' ),
				'type'        => 'string',
				'required'    => true,
			],
			'country'      => [
				'description' => esc_html__( 'The country for which the keyword should be researched.', 'gohigh-seo' ),
				'type'        => 'string',
				'required'    => true,
			],
			'objectID'     => [
				'description' => esc_html__( 'The ID of the post initiating the keyword research request.', 'gohigh-seo' ),
				'type'        => 'integer',
				'required'    => true,
			],
			'force_update' => [
				'description' => esc_html__( 'If true, forces a fresh research request.', 'gohigh-seo' ),
				'type'        => 'boolean',
				'required'    => false,
			],
		];
	...
	private function get_researched_data( $keyword, $post_type, $country, $force_update = false ) {
		...
		if ( empty( $data['error'] ) && empty( $data['data']['details'] ) ) {
			return [
				'error' => esc_html__( 'No data found for the researched keyword.', 'gohigh-seo' ),
			];
		}
		...
		if ( $response_code === 410 ) {
			return wp_kses_post(
				sprintf(
					...
					__( 'There is a new version of Content AI available! %s the Rank Math SEO plugin to use this feature.', 'gohigh-seo' ),
					'<a href="' . esc_url( self_admin_url( 'update-core.php' ) ) . '">' . __( 'Please update', 'gohigh-seo' ) . '</a>'
				)
			);
		}
	...
	private function get_errored_data( $error ) {
		...
		if ( 'invalid_domain' === $error['code'] ) {
			return [
				'data' => esc_html__( 'This feature is not available on the localhost.', 'gohigh-seo' ),
			];
		}

		if ( 'domain_limit_reached' === $error['code'] ) {
			return [
				'data' => esc_html__( 'You have used all the free credits which are allowed to this domain.', 'gohigh-seo' ),
			];
		}
