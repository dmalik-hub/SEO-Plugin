...
const getErrorMessage = () => {
	...
	if ( ! rankMath.contentAI.isUserRegistered ) {
		return (
			<>
				{ __( 'Start using Content AI by connecting your RankMath account.', 'gohigh-seo' ) }
				<a href={ rankMath.connectSiteUrl }>{ __( 'Connect Now', 'gohigh-seo' ) }</a>
			</>
		)
	}
	...
	if ( ! rankMath.contentAI.plan ) {
		return (
			<>
				{ __( 'You do not have a Content AI plan.', 'gohigh-seo' ) }
				<a href="https://rankmath.com/kb/how-to-use-content-ai/?play-video=ioPeVIntJWw&utm_source=Plugin&utm_medium=Buy+Plan+Button&utm_campaign=WP">
					{ __( 'Choose your plan', 'gohigh-seo' ) }
				</a>
			</>
		)
	}
	...
	return (
		<>
			{ __( 'You have exhausted your Content AI Credits.', 'gohigh-seo' ) }
		...
		if ( hasApiError ) {
			return (
				<button
					...
					title={ __( 'Dismiss', 'gohigh-seo' ) }
					onClick={ handleDismissCommand }
					contentEditable={ false }
				>
					{ __( 'Dismiss', 'gohigh-seo' ) }
				</button>
			)
		}
			...
			return (
				<>
					<button
						...
						title={ __( 'Click or Press Enter', 'gohigh-seo' ) }
