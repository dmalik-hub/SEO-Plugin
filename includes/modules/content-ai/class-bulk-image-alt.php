...
class Bulk_Image_Alt extends \WP_Background_Process {
	...
	public function start( $data ) {
		Helper::add_notification(
			esc_html__( 'Bulk image alt generation started. It might take few minutes to complete the process.', 'gohigh-seo' ),
			[
				'type'    => 'success',
	...
	protected function complete() {
		...
		Helper::add_notification(
			...
			sprintf( _n( 'Image alt attributes successfully updated in %d post.', 'Image alt attributes successfully updated in %d posts.', count( $posts ), 'gohigh-seo' ), count( $posts ) ),
			[
				'type'    => 'success',
	...
	protected function task( $data ) {
		try {
			...
			if ( ! empty( $api_output['error'] ) ) {
				$notice = ! empty( $api_output['message'] ) ? $api_output['message'] : esc_html__( 'Bulk image alt generation failed.', 'gohigh-seo' );
				Helper::add_notification(
					$notice,
					[
						'type'    => 'error',
			...
			if ( Helper::is_feature_quota_exhausted( 'generate_image_alt_v2' ) ) {
				...
				Helper::add_notification(
					// Translators: placeholder is the number of modified posts.
					sprintf( esc_html__( 'Image alt attributes successfully updated in %d posts. The process was stopped as you have reached the monthly limit for this feature.', 'gohigh-seo' ), (int) $posts_processed ),
					[
						'type'    => 'success',
						'id'      => 'rank_math_content_ai_posts',
						'classes' => 'rank-math-notice',
