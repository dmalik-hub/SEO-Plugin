...
class Bulk_Actions {
	...
	public function bulk_actions( $actions ) {
		...
		$actions['rank_math_ai_options']                             = __( '&#8595; Rank Math Content AI', 'gohigh-seo' );
		$actions['rank_math_content_ai_fetch_seo_title']             = esc_html__( 'Write SEO Title with AI', 'gohigh-seo' );
		$actions['rank_math_content_ai_fetch_seo_description']       = esc_html__( 'Write SEO Description with AI', 'gohigh-seo' );
		$actions['rank_math_content_ai_fetch_seo_title_description'] = esc_html__( 'Write SEO Title & Description with AI', 'gohigh-seo' );
		$actions['rank_math_content_ai_fetch_image_alt']             = esc_html__( 'Write Image Alt Text with AI', 'gohigh-seo' );
	...
	public function handle_bulk_actions( $redirect, $doaction, $object_ids ) {
		...
		if ( ! empty( get_option( 'rank_math_content_ai_posts' ) ) ) {
			Helper::add_notification(
				esc_html__( 'Another bulk editing process is already running. Please try again later after the existing process is complete.', 'gohigh-seo' ),
				[
					'type'    => 'warning',
	...
	public function add_tools( $tools ) {
		...
		$tools['content_ai_cancel_bulk_edit_process'] = [
			'title'       => esc_html__( 'Cancel Content AI Bulk Editing Process', 'gohigh-seo' ),
			'description' => sprintf(
				// Translators: placeholders are the number of posts that were processed.
				esc_html__( 'Terminate the ongoing Content AI Bulk Editing Process to halt any pending modifications and revert to the previous state. The bulk metadata has been generated for %1$d out of %1$d posts so far.', 'gohigh-seo' ),
				$processed,
				count( $posts )
			),
			'button_text' => esc_html__( 'Terminate', 'gohigh-seo' ),
		];
	...
	public function cancel_bulk_edit_process() {
		...
		return __( 'Bulk Editing Process Successfully Cancelled', 'gohigh-seo' );
	}
