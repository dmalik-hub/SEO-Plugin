...
class Watcher {
	...
	public function handle_post_update( $post_id, $post, $before ) {
		...
		if ( $this->is_watched_transition( $transition, $post ) && $this->has_permalink_changed( $before_permalink, $after_permalink ) ) {
			...
			$message = sprintf(
				...
				__( 'SEO Notice: you just changed the slug of a %1$s and Rank Math has automatically created a redirection. You can edit the redirection by <a href="%2$s">clicking here</a>.', 'gohigh-seo' ),
	...
	private function add_invalid_notification( $url, $type ) {
		...
		$message = sprintf( wp_kses_post( __( '<strong>SEO Notice:</strong> A previously published %1$s has been moved to trash. You may redirect <code>%2$s</code> to <a href="%3$s">a new url</a>.', 'gohigh-seo' ) ), $type, $url, $admin_url );
