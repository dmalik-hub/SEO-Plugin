...
namespace GoHighSEO\Redirections;

use GoHighSEO\Traits\Hooker;
use GoHighSEO\Helper;
use GoHighSEO\Helpers\Param;
use GoHighSEO\Admin\List_Table;
use GoHighSEO\Redirections\Admin;
...
class Table extends List_Table {
	...
	public function __construct() {
		parent::__construct(
			[
				...
				'plural'   => 'redirections',
				'no_items' => $this->is_trashed_page() ? esc_html__( 'No redirections found in Trash.', 'gohigh-seo' ) : wp_kses_post( __( 'No redirections added yet. <a href="#" class="rank-math-add-new-redirection">Add New Redirection</a>', 'gohigh-seo' ) ),
			]
		);
	}
	...
	private function get_sources_html( $item ) {
		...
		// Show more button.
		$html .= ' <a href="#" class="rank-math-showmore" title="' . esc_html__( 'Show more', 'gohigh-seo' ) . '">[&hellip;]</a>';
		...
		$html .= join( '<br>', $parts );
		$html .= '<br><a href="#" class="rank-math-hidemore" title="' . esc_html__( 'Hide details', 'gohigh-seo' ) . '">[' . esc_html__( 'Hide', 'gohigh-seo' ) . ']</a>';
		$html .= '</div>';
	...
	public function column_actions( $item ) {
		...
		if ( $this->is_trashed_page() ) {
			return $this->row_actions(
				[
					'restore' => '<a href="' . $url . '" data-action="restore" class="rank-math-redirection-action">' . esc_html__( 'Restore', 'gohigh-seo' ) . '</a>',
					'delete'  => '<a href="' . $url . '" data-action="delete" class="rank-math-redirection-action">' . esc_html__( 'Delete Permanently', 'gohigh-seo' ) . '</a>',
				]
			);
		}
		...
		return $this->row_actions(
			[
				'edit'       => '<a data-redirection="' . esc_attr( wp_json_encode( $redirection ) ) . '" href="' . $edit_url . '" class="rank-math-redirection-edit">' . esc_html__( 'Edit', 'gohigh-seo' ) . '</a>',
				'deactivate' => '<a href="' . $url . '" data-action="deactivate" class="rank-math-redirection-action">' . esc_html__( 'Deactivate', 'gohigh-seo' ) . '</a>',
				'activate'   => '<a href="' . $url . '" data-action="activate" class="rank-math-redirection-action">' . esc_html__( 'Activate', 'gohigh-seo' ) . '</a>',
				'trash'      => '<a href="' . $url . '" data-action="trash" class="rank-math-redirection-action">' . esc_html__( 'Trash', 'gohigh-seo' ) . '</a>',
				'view'       => '<a href="' . $view_url . '" rel="bookmark">' . esc_html__( 'View', 'gohigh-seo' ) . '</a>',
			]
		);
	}
	...
	public function get_columns() {
		...
		return apply_filters(
			...
			[
				...
				'sources'       => esc_html__( 'From', 'gohigh-seo' ),
				'url_to'        => esc_html__( 'To', 'gohigh-seo' ),
				'header_code'   => esc_html__( 'Type', 'gohigh-seo' ),
				'hits'          => esc_html__( 'Hits', 'gohigh-seo' ),
				'created'       => esc_html__( 'Created', 'gohigh-seo' ),
				'last_accessed' => esc_html__( 'Last Accessed', 'gohigh-seo' ),
			]
		);
	}
	...
	public function get_bulk_actions() {
		if ( $this->is_trashed_page() ) {
			$actions = [
				'restore' => esc_html__( 'Restore', 'gohigh-seo' ),
				'delete'  => esc_html__( 'Delete Permanently', 'gohigh-seo' ),
			];
		} else {
			$actions = [
				'activate'   => esc_html__( 'Activate', 'gohigh-seo' ),
				'deactivate' => esc_html__( 'Deactivate', 'gohigh-seo' ),
				'trash'      => esc_html__( 'Move to Trash', 'gohigh-seo' ),
			];
		}
	...
	public function get_views() {
		...
		$labels  = [
			'all'      => esc_html__( 'All', 'gohigh-seo' ),
			'active'   => esc_html__( 'Active', 'gohigh-seo' ),
			'inactive' => esc_html__( 'Inactive', 'gohigh-seo' ),
			'trashed'  => esc_html__( 'Trash', 'gohigh-seo' ),
		];
	...
	public function extra_tablenav( $which ) {
		...
		submit_button( esc_html__( 'Empty Trash', 'gohigh-seo' ), '', 'delete_all', false );
		echo '</div>';
	}
