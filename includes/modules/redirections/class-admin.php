...
use GoHighSEO\Admin\Page;
use GoHighSEO\Monitor\DB as Monitor_DB;
...
class Admin extends Base {
	...
	public function __construct() {
		...
		add_action( 'rank_math/redirection/clean_trashed', 'GoHighSEO\\Redirections\\DB::periodic_clean_trash' );
	}
	...
	public function register_admin_page() {
		...
		$this->page = new Page(
			...
			esc_html__( 'Redirections', 'gohigh-seo' ),
			[
				'position'   => 40,
				'parent'     => 'rank-math',
				...
				'help'       => [
					...
					'redirections-screen-content' => [
						'title' => esc_html__( 'Screen Content', 'gohigh-seo' ),
						'view'  => $dir . 'help-tab-screen-content.php',
					],
					'redirections-actions'        => [
						'title' => esc_html__( 'Available Actions', 'gohigh-seo' ),
						'view'  => $dir . 'help-tab-actions.php',
					],
					'redirections-bulk'           => [
						'title' => esc_html__( 'Bulk Actions', 'gohigh-seo' ),
		...
		Arr::insert(
			...
			[
				'redirections' => [
					...
					'desc'  => sprintf( esc_html__( 'Easily create redirects without fiddling with tedious code. %s.', 'gohigh-seo' ), '<a href="' . KB::get( 'redirections-settings', 'Options Panel Redirections Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>' ),
	...
	public function init() {
		...
		if ( empty( $ids ) ) {
			Helper::add_notification( __( 'No valid ID found.', 'gohigh-seo' ) );
			return;
		}
	...
	public function handle_ajax() {
		...
		if ( ! $id ) {
			$this->error( esc_html__( 'No valid ID found.', 'gohigh-seo' ) );
		}
		...
		$this->error( esc_html__( 'No valid action found.', 'gohigh-seo' ) );
	}
	...
	private function perform_action( $action, $ids ) {
		...
		$message = [
			'activate'   => esc_html__( 'Redirection successfully activated.', 'gohigh-seo' ),
			'deactivate' => esc_html__( 'Redirection successfully deactivated.', 'gohigh-seo' ),
			'trash'      => esc_html__( 'Redirection successfully moved to Trash.', 'gohigh-seo' ),
			'restore'    => esc_html__( 'Redirection successfully restored.', 'gohigh-seo' ),
		];
		...
		if ( 'delete' === $action ) {
			...
			if ( $count > 0 ) {
				...
				return sprintf( esc_html__( '%d redirection(s) successfully deleted.', 'gohigh-seo' ), $count );
			}
		}
