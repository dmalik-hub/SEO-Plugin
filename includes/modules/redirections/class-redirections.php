...
use GoHighSEO\Redirections\DB;
use GoHighSEO\Helpers\Param;
use GoHighSEO\Admin\Admin_Helper;
use GoHighSEO\Redirections\Redirection;
...
class Redirections {
	...
	public function add_export_tab_content() {
		...
		<div class="rank-math-redirections-csv-export">
			<h4><?php esc_html_e( 'Sync to .htaccess', 'gohigh-seo' ); ?></h4>
			<p class="description">
				<?php esc_html_e( 'Copy all active redirections to the .htaccess file.', 'gohigh-seo' ); ?>
			</p>
			<div class="csv-export-footer">
				<?php wp_nonce_field( 'rank_math_htaccess_sync_redirections', '_wpnonce_htaccess', true, true ); ?>
				<button type="submit" class="button button-secondary" id="export-redirections-csv" name="rank-math-redirections-export" value="htaccess-sync"><?php esc_html_e( 'Sync to .htaccess', 'gohigh-seo' ); ?></button>
	...
	public function maybe_sync_htaccess() {
		...
		if ( ! wp_verify_nonce( Param::post( '_wpnonce_htaccess' ), 'rank_math_htaccess_sync_redirections' ) ) {
		...
		if ( false === $htaccess || ! $htaccess['writable'] || ! Helper::is_filesystem_direct() ) {
			wp_die( esc_html__( 'The redirections could not be synced because the .htaccess file does not exist or it is not writable.', 'gohigh-seo' ) );
		}

		$rules = $this->get_htaccess_rules();
		if ( ! $rules ) {
			Helper::add_notification( __( 'No valid redirection found.', 'gohigh-seo' ) );
			return;
		}
		...
		Helper::add_notification( __( 'Redirections successfully synced to the .htaccess file.', 'gohigh-seo' ) );
	}
	...
	public function admin_scripts() {
		...
		wp_enqueue_style( 'gohigh-seo-redirections', $url . 'css/redirections.css', [], RANK_MATH_PRO_VERSION );
		wp_enqueue_script( 'gohigh-seo-redirections', $url . 'js/redirections.js', [ 'lodash' ], RANK_MATH_PRO_VERSION, true );
		wp_set_script_translations( 'gohigh-seo-redirections', 'gohigh-seo', RANK_MATH_PRO_PATH . 'languages/' );
	}
