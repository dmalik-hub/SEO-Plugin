...
class Import_Export {
	...
	public function export_tab() {
		...
		if ( $count['count'] >= $this->limit ) {
			...
			<div class="inline notice notice-warning notice-alt rank-math-notice" style="margin: 10px 10px 0;">
				<p>
					...
					printf( esc_html__( 'Warning: you have more than %d active redirections. Exporting them to your .htaccess file may cause performance issues.', 'gohigh-seo' ), absint( $this->limit ) );
					?>
				</p>
			</div>
			<?php
		...
		<div class="rank-math-redirections-export-options">
			<a class="button button-primary" href="<?php echo esc_url( wp_nonce_url( Helper::get_admin_url( 'redirections', 'export=apache' ), 'rank-math-export-redirections' ) ); ?>"><?php esc_html_e( 'Export to .htaccess', 'gohigh-seo' ); ?></a>
			<a class="button button-primary" href="<?php echo esc_url( wp_nonce_url( Helper::get_admin_url( 'redirections', 'export=nginx' ), 'rank-math-export-redirections' ) ); ?>"><?php esc_html_e( 'Export to Nginx config file', 'gohigh-seo' ); ?></a>
		</div>
		<?php
	}
