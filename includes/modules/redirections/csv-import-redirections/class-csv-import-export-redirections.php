...
namespace GoHighSEO\Redirections\CSV_Import_Export_Redirections;
...
class CSV_Import_Export_Redirections {
	...
	public function add_help_tab() {
		...
		$content .= sprintf( esc_html__( 'Use the following columns in the CSV file (the order does not matter): %s', 'gohigh-seo' ), '<code>id, source, matching, destination, type, category, status, ignore</code>' );
		$content .= '</li><li>';
		// Translators: placeholders are column names.
		$content .= sprintf( esc_html__( 'Only the %1$s and the %2$s columns are required, the others are optional.', 'gohigh-seo' ), '<code>source</code>', '<code>destination</code>' );
		$content .= '</li><li>';
		// Translators: placeholder 1 is the column name, placeholder 2 is the possible column value ("case").
		$content .= sprintf( esc_html__( 'The %1$s column may contain the value %2$s, or nothing.', 'gohigh-seo' ), '<code>ignore</code>', '<code>case</code>' );
		$content .= '</li><li>';
		// Translators: placeholder is the column name.
		$content .= sprintf( esc_html__( 'If the numeric ID is specified in the %s column, then the redirection will be edited. If it is not set or empty, then a new redirection will be created.', 'gohigh-seo' ), '<code>id</code>' );
		$content .= '</li><li>';
		// Translators: placeholder is the filter name.
		$content .= sprintf( esc_html__( 'If an imported redirection differs from an existing redirection (or another imported redirection) only by the source value, then those redirections will be merged into a single redirection with multiple sources. You can change this behavior with the %s filter hook.', 'gohigh-seo' ), '<code>rank_math/admin/csv_import_redirection_update</code>' );
		$content .= '</li><li>';
		// Translators: 1 is the command name, 2 is the column name.
		$content .= sprintf( esc_html__( 'Use %1$s (case-sensitive) as the value for the %2$s column to delete a redirection.', 'gohigh-seo' ), '<code>DELETE</code>', '<code>destination</code>' );
		$content .= '</li><li>';
		// Translators: placeholder is a link to the KB article.
		$content .= sprintf( esc_html__( 'For more information, please see %s.', 'gohigh-seo' ), '<a href="https://rankmath.com/kb/how-to-manage-redirects-via-csv/" target="_blank">' . __( 'our Knowledge Base article', 'gohigh-seo' ) . '</a>' );
		$content .= '</li></ul>';

		$screen->add_help_tab(
			[
				'id'      => 'csv_import_redirections',
				'title'   => __( 'CSV Import', 'gohigh-seo' ),
				'content' => $content,
			]
		);
	}
	...
	public function add_import_tab_content() {
		...
		<?php if ( ! $import_in_progress ) : ?>
					<p><label for="csv-import-me"><strong><?php esc_html_e( 'CSV File', 'gohigh-seo' ); ?></strong></label><p>
					<input type="file" name="csv-redirections-import-me" id="csv-redirections-import-me" value="" accept=".csv">
					<br>
					<span class="validation-message"><?php esc_html_e( 'Please select a CSV file to import.', 'gohigh-seo' ); ?></span>
					<p class="description">
						<?php // Translators: placeholder is a comma-separated list of columns. ?>
						<?php printf( esc_html__( 'Import a CSV file to create or update redirections. The file must include at least the following columns: %s', 'gohigh-seo' ), '<code>source, destination</code>' ); ?>
						<button type="button" id="rank-math-contextual-help-link" class="button button-small show-settings"><?php esc_html_e( 'More details', 'gohigh-seo' ); ?></button>
					</p>
				<?php else : ?>
			...
			<?php wp_nonce_field( 'rank_math_csv_import_redirections' ); ?>
					<input type="hidden" name="object_id" value="csv-import-redirections-plz">
					<input type="hidden" name="action" value="wp_handle_upload">
					<?php if ( $import_in_progress ) : ?>
						<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( [ 'rank_math_cancel_csv_import_redirections' => 1 ] ), 'rank_math_cancel_csv_import_redirections' ) ); ?>" id="csv-import-cancel" class="button button-link-delete csv-import-redirections-cancel"><?php esc_html_e( 'Cancel Import', 'gohigh-seo' ); ?></a>
					<?php else : ?>
						<button type="submit" class="button button-primary"><?php esc_html_e( 'Import CSV', 'gohigh-seo' ); ?></button>
					<?php endif; ?>
			</div>
		</div>
		<?php
	...
	public function add_export_tab_content() {
		...
		<div class="rank-math-redirections-csv-export">
			<h4><?php esc_html_e( 'Export CSV', 'gohigh-seo' ); ?></h4>
			<input type="checkbox" class="cmb2-option" name="include_deactivated" id="include_deactivated" value="1" checked="checked"> <label for="include_deactivated"><?php esc_html_e( 'Include deactivated redirections', 'gohigh-seo' ); ?></label>

			<div class="csv-export-footer">
				<?php wp_nonce_field( 'rank_math_csv_export_redirections', '_wpnonce', true, true ); ?>
				<button type="submit" class="button button-secondary" id="export-redirections-csv" name="rank-math-redirections-export" value="csv"><?php esc_html_e( 'Export CSV', 'gohigh-seo' ); ?></button>
	...
	public function change_export_button_label( $buttons ) {
		$buttons['import_export']['label'] = __( 'Import & Export', 'gohigh-seo' );
		return $buttons;
	}
	...
	public function add_notice() {
		...
		Helper::add_notification( esc_html__( 'CSV import is in progress...', 'gohigh-seo' ), [ 'type' => 'success' ] );
	}
	...
	public function maybe_do_export() {
		...
		if ( ! wp_verify_nonce( isset( $_REQUEST['_wpnonce'] ) ? sanitize_key( $_REQUEST['_wpnonce'] ) : '', 'rank_math_csv_export_redirections' ) ) {
			wp_die( esc_html__( 'Invalid nonce.', 'gohigh-seo' ) );
		}
		if ( ! current_user_can( 'rank_math_redirections' ) ) {
			wp_die( esc_html__( 'Sorry, you are not allowed to export redirections on this site.', 'gohigh-seo' ) );
		}
	...
	public function maybe_do_import() {
		...
		if ( empty( $_FILES['csv-redirections-import-me'] ) || empty( $_FILES['csv-redirections-import-me']['name'] ) ) {
			wp_die( esc_html__( 'Please select a file to import.', 'gohigh-seo' ) );
		}
		if ( ! wp_verify_nonce( isset( $_REQUEST['_wpnonce'] ) ? sanitize_key( $_REQUEST['_wpnonce'] ) : '', 'rank_math_csv_import_redirections' ) ) {
			wp_die( esc_html__( 'Invalid nonce.', 'gohigh-seo' ) );
		}
		if ( ! current_user_can( 'import' ) || ! current_user_can( 'rank_math_redirections' ) ) {
			wp_die( esc_html__( 'Sorry, you are not allowed to import redirections on this site.', 'gohigh-seo' ) );
		}
	...
	public function validate_file( $file ) {
		if ( is_wp_error( $file ) ) {
			Helper::add_notification( esc_html__( 'CSV could not be imported:', 'gohigh-seo' ) . ' ' . $file->get_error_message(), [ 'type' => 'error' ] );
			return false;
		}

		if ( isset( $file['error'] ) ) {
			Helper::add_notification( esc_html__( 'CSV could not be imported:', 'gohigh-seo' ) . ' ' . $file['error'], [ 'type' => 'error' ] );
			return false;
		}

		if ( ! isset( $file['file'] ) ) {
			Helper::add_notification( esc_html__( 'CSV could not be imported: Upload failed.', 'gohigh-seo' ), [ 'type' => 'error' ] );
			return false;
		}

		if ( ! isset( $file['type'] ) || 'text/csv' !== $file['type'] ) {
			\wp_delete_file( $file['file'] );
			Helper::add_notification( esc_html__( 'CSV could not be imported: File type error.', 'gohigh-seo' ), [ 'type' => 'error' ] );
			return false;
		}
	...
	public static function maybe_cancel_import() {
		...
		if ( ! wp_verify_nonce( isset( $_REQUEST['_wpnonce'] ) ? sanitize_key( $_REQUEST['_wpnonce'] ) : '', 'rank_math_cancel_csv_import_redirections' ) ) {
			Helper::add_notification( esc_html__( 'Import could not be canceled: invalid nonce. Please try again.', 'gohigh-seo' ), [ 'type' => 'error' ] );
			wp_safe_redirect( remove_query_arg( 'rank_math_cancel_csv_import_redirections' ) );
			exit;
		}
		if ( ! current_user_can( 'import' ) ) {
			Helper::add_notification( esc_html__( 'Import could not be canceled: you are not allowed to import content to this site.', 'gohigh-seo' ), [ 'type' => 'error' ] );
			wp_safe_redirect( remove_query_arg( 'rank_math_cancel_csv_import_redirections' ) );
			exit;
		}
	...
	public static function cancel_import( $silent = false ) {
		...
		if ( ! $file_path ) {
			if ( ! $silent ) {
				Helper::add_notification( esc_html__( 'Import could not be canceled.', 'gohigh-seo' ), [ 'type' => 'error' ] );
			}
		...
		if ( ! $silent ) {
			Helper::add_notification(
				__( 'CSV import canceled.', 'gohigh-seo' ),
				[
					'type'    => 'success',
	...
	public static function import_progress_details() {
		...
		if ( $import_in_progress ) {
			...
			?>
			<p><?php esc_html_e( 'Import in progress...', 'gohigh-seo' ); ?></p>
			<p class="csv-import-redirections-status">
				<?php // Translators: placeholders represent count like 15/36. ?>
				<?php printf( esc_html__( 'Items processed: %1$s/%2$s', 'gohigh-seo' ), absint( min( $total_lines, $total_lines - $remaining_items + 1 ) ), absint( $total_lines ) ); ?>
			</p>
	...
	public static function get_import_complete_message() {
		...
		$message       = sprintf(
			...
			__( 'CSV import completed. Successfully imported %d rows.', 'gohigh-seo' ),
			count( $imported_rows )
		);

		if ( ! empty( $status['errors'] ) ) {
			$message  = __( 'CSV import completed.', 'gohigh-seo' ) . ' ';
			$message .= sprintf(
				// Translators: placeholder is the number of rows imported.
				__( 'Imported %d rows.', 'gohigh-seo' ) . ' ',
				count( $imported_rows )
			);

			if ( ! empty( $status['errors'] ) ) {
				$message .= __( 'One or more errors occured while importing: ', 'gohigh-seo' ) . '<br>';
				$message .= '<code>' . join( '</code><br><code>', $status['errors'] ) . '</code><br>';
			}
			if ( ! empty( $status['failed_rows'] ) ) {
				$message .= '<br>' . __( 'The following lines could not be imported: ', 'gohigh-seo' ) . '<br>';
	...
	public static function get_localized_action( $action ) {
		$actions = [
			'created' => __( 'Created', 'gohigh-seo' ),
			'updated' => __( 'Updated', 'gohigh-seo' ),
			'deleted' => __( 'Deleted', 'gohigh-seo' ),
			'merged'  => __( 'Merged', 'gohigh-seo' ),
		];
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'gohigh-seo-redirections', RANK_MATH_PRO_URL . 'includes/modules/redirections/assets/js/redirections.js', [ 'wp-components', 'rank-math-components' ], RANK_MATH_PRO_VERSION, true );
		wp_enqueue_style( 'gohigh-seo-redirections', RANK_MATH_PRO_URL . 'includes/modules/redirections/assets/css/redirections.css', [], RANK_MATH_PRO_VERSION );
	}
}
