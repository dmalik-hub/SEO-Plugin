...
class Categories {
	...
	public function register_categories() {
		...
		$tax_labels = [
			'name'              => _x( 'Redirection Categories', 'taxonomy general name', 'gohigh-seo' ),
			'singular_name'     => _x( 'Redirection Category', 'taxonomy singular name', 'gohigh-seo' ),
			'search_items'      => __( 'Search Redirection Categories', 'gohigh-seo' ),
			'all_items'         => __( 'All Redirection Categories', 'gohigh-seo' ),
			'parent_item'       => __( 'Parent Category', 'gohigh-seo' ),
			'parent_item_colon' => __( 'Parent Category:', 'gohigh-seo' ),
			'edit_item'         => __( 'Edit Category', 'gohigh-seo' ),
			'update_item'       => __( 'Update Category', 'gohigh-seo' ),
			'add_new_item'      => __( 'Add New Category', 'gohigh-seo' ),
			'new_item_name'     => __( 'New Category Name', 'gohigh-seo' ),
			'menu_name'         => __( 'Redirection Categories', 'gohigh-seo' ),
		];
	...
	public function bulk_actions( $actions ) {
		...
		$actions['bulk_add_redirection_category'] = __( 'Add to Category', 'gohigh-seo' );
		return $actions;
	}
	...
	public function handle_bulk_actions() {
		...
		if ( empty( $ids ) ) {
			Helper::add_notification( __( 'No valid ID provided.', 'gohigh-seo' ) );
			return;
		}

		foreach ( $ids as $id ) {
			wp_set_object_terms( $id, absint( $category_filter ), 'rank_math_redirection_category', apply_filters( 'rank_math/redirection/bulk_append_categories', true ) );
		}

		// Translators: placeholder is the number of updated redirections.
		Helper::add_notification( sprintf( __( '%d redirections have been assigned to the category.', 'gohigh-seo' ), count( $ids ) ) );
	}
	...
	public function add_category_column( $columns ) {
		$columns['category'] = __( 'Category', 'gohigh-seo' );
		return $columns;
	}
	...
	public function category_column_content( $defaults, $item ) {
		...
		if ( empty( $cats ) ) {
			$cats     = __( 'Uncategorized', 'gohigh-seo' );
	...
	public function category_filter( $which ) {
		...
		$dropdown_args = [
			...
			'show_option_none'  => __( 'Select Category', 'gohigh-seo' ),
	...
	public function back_to_redirections_link() {
		...
		echo '<p><a href="' . esc_url( $link ) . '">' . esc_html__( '&larr; Go Back to the Redirections', 'gohigh-seo' ) . '</a></p>';
	}
	...
	public function page_title_actions( $actions ) {
		...
		$actions['manage_categories'] = [
			...
			'label' => __( 'Manage Categories', 'gohigh-seo' ),
		];
