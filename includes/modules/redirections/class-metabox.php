	...
	public function save_advanced_meta( $cmb ) {
		if ( $this->can_delete( $cmb->data_to_save ) ) {
			...
			if ( ! empty( $cmb->data_to_save['redirection_id'] ) ) {
				...
				Helper::add_notification( esc_html__( 'Redirection successfully deleted.', 'gohigh-seo' ), [ 'type' => 'info' ] );
			}
			return [
				'action'  => 'delete',
				'message' => esc_html__( 'Redirection successfully deleted.', 'gohigh-seo' ),
			];
		}
		...
		if ( ! $this->can_update( $cmb->data_to_save ) ) {
			return [
				'action'  => 'cant_update',
				'message' => esc_html__( 'Can\'t update redirection.', 'gohigh-seo' ),
			];
		}
		...
		$response = [
			'action'  => 'update',
			'message' => esc_html__( 'Redirection updated successfully.', 'gohigh-seo' ),
		];

		if ( $redirection->is_new() ) {
			Helper::add_notification( esc_html__( 'New redirection created.', 'gohigh-seo' ) );
			$response = [
				'id'      => $redirection->get_id(),
				'action'  => 'new',
				'message' => esc_html__( 'New redirection created.', 'gohigh-seo' ),
			];
		}
