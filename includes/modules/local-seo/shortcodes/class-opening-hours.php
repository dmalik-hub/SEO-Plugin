...
class Opening_Hours {
	...
	public function get_data( $shortcode, $schema ) {
		if ( ! isset( $schema['openingHoursSpecification'] ) ) {
			return '<p>' . esc_html__( 'Open 24/7', 'gohigh-seo' ) . '</p>';
		}
		...
		ob_start();
		?>
		<h5><?php esc_html_e( 'Opening Hours:', 'gohigh-seo' ); ?></h5>
		<div class="rank-math-business-opening-hours">
			<?php
			foreach ( $days as $day => $hours ) {
				$time = ! empty( $hours['time'] ) ? implode( esc_html__( ' and ', 'gohigh-seo' ), $hours['time'] ) : esc_html__( 'Closed', 'gohigh-seo' );
	...
	private function get_opening_hours_note( $shortcode ) {
		return empty( $shortcode->atts['opening_hours_note'] ) ? esc_html__( 'Open now', 'gohigh-seo' ) : esc_html( $shortcode->atts['opening_hours_note'] );
	}
