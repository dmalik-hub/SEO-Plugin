...
use GoHighSEO\Frontend\Shortcodes;
...
class Address {
	...
	public function get_data( $shortcode, $schema ) {
		...
		$labels = [
			'telephone'        => [
				'key'   => 'telephone',
				'label' => esc_html__( 'Phone', 'gohigh-seo' ),
			],
			'secondary_number' => [
				'key'   => 'secondary_number',
				'label' => esc_html__( 'Secondary phone', 'gohigh-seo' ),
			],
			'fax'              => [
				'key'   => 'faxNumber',
				'label' => esc_html__( 'Fax', 'gohigh-seo' ),
			],
			'email'            => [
				'key'   => 'email',
				'label' => esc_html__( 'Email', 'gohigh-seo' ),
			],
			'url'              => [
				'key'   => 'url',
				'label' => esc_html__( 'URL', 'gohigh-seo' ),
			],
			'vat_id'           => [
				'key'   => 'vatID',
				'label' => esc_html__( 'VAT ID', 'gohigh-seo' ),
			],
			'tax_id'           => [
				'key'   => 'taxID',
				'label' => esc_html__( 'Tax ID', 'gohigh-seo' ),
			],
			'coc_id'           => [
				'key'   => 'coc_id',
				'label' => esc_html__( 'Chamber of Commerce ID', 'gohigh-seo' ),
			],
			'pricerange'       => [
				'key'   => 'priceRange',
				'label' => esc_html__( 'Price indication', 'gohigh-seo' ),
			],
		];
	...
	public function get_address( $schema, $atts = [] ) {
		...
		return '<h5>' . esc_html__( 'Address:', 'gohigh-seo' ) . '</h5><address>' . wp_kses_post( $data ) . '</address>';
	}
}
