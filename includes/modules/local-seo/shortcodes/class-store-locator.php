...
class Store_Locator {
		...
		?>
			<div class="rank-math-business-wrapper">
				<form id="rank-math-local-store-locator" method="post" action="#rank-math-local-store-locator">
					...
					<?php if ( ! empty( $terms ) ) { ?>
						<div class="rank-math-form-field">
							<select name="rank-math-location-category">
								<option value=""><?php echo esc_html__( 'Select Category', 'gohigh-seo' ); ?></option>
					...
					<div class="rank-math-form-field">
						<input type="text" name="rank-math-search-address" id="rank-math-search-address" placeholder="<?php echo esc_html__( 'Address, Suburb, Region, Zip or Landmark', 'gohigh-seo' ); ?>" value="<?php echo esc_attr( $address ); ?>" />
						<input type="hidden" name="lat" id="rank-math-lat" />
						<input type="hidden" name="lng" id="rank-math-lng" />
					</div>
					<?php $this->detect_location(); ?>
					<div class="rank-math-form-field">
						<button type="submit" name="rank-math-submit" value="search"><?php echo esc_html__( 'Search', 'gohigh-seo' ); ?></button>
					</div>
				</form>
		<?php
	...
	private function detect_location() {
		...
		echo '<a href="#" id="rank-math-current-location">' . esc_html__( 'Detect Location', 'gohigh-seo' ) . '</a>';
	}
	...
	public function get_directions( $location, $shortcode ) {
		...
		?>
			<div class="rank-math-directions-wrapper">
				<a href="#" class="rank-math-show-route" data-toggle-text="<?php echo esc_html__( 'Hide route', 'gohigh-seo' ); ?>">
					<?php echo esc_html( $shortcode->atts['route_label'] ); ?>
				</a>

				<div class="rank-math-directions-result">
					<h3><?php echo esc_html__( 'Route', 'gohigh-seo' ); ?></h3>
					<div class="rank-math-directions-form">
							<form method="post">
								<div class="rank-math-form-field">
									<label for="origin"><?php echo esc_html__( 'Your location:', 'gohigh-seo' ); ?></label>
									<input type="text" name="origin" id="rank-math-origin" value="<?php echo esc_attr( Param::post( 'rank-math-search-address' ) ); ?>" />
									<input type="submit" name="get-direction" value="<?php echo esc_html__( 'Show route', 'gohigh-seo' ); ?>" />
