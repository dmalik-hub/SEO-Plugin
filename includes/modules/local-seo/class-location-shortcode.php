...
use GoHighSEO\Traits\Shortcode;
...
class Location_Shortcode {
	...
	public function enqueue() {
		...
		wp_register_script( 'rank-math-google-maps', '//maps.googleapis.com/maps/api/js?&key=' . rawurlencode( $this->api_key ) . '&loading=async&callback=rankMathInitMap&libraries=marker', [], rank_math()->version, true );
		wp_register_script( 'rank-math-google-maps-cluster', 'https://unpkg.com/@googlemaps/markerclusterer/dist/index.min.js', [], rank_math()->version, true );
		wp_register_script( 'rank-math-local', RANK_MATH_PRO_URL . 'includes/modules/local-seo/assets/js/rank-math-local.js', [ 'jquery', 'lodash', 'rank-math-google-maps', 'rank-math-google-maps-cluster' ], rank_math()->version, true );
	}
	...
	public function local_shortcode( $atts ) {
		...
		if ( ! $this->api_key && is_user_logged_in() && in_array( $this->atts['type'], [ 'store-locator', 'map' ], true ) ) {
			return sprintf(
				...
				esc_html__( 'This page can\'t load Google Maps correctly. Please add %s.', 'gohigh-seo' ),
				'<a href="' . Helper::get_settings_url( 'titles', 'local' ) . '" target="_blank">' . esc_html__( 'API Key', 'gohigh-seo' ) . '</a>'
			);
		}

		wp_enqueue_style( 'rank-math-local-business', RANK_MATH_PRO_URL . 'includes/modules/local-seo/assets/css/local-business.css', null, rank_math()->version );
	...
	private function get_shortcode_data() {
		...
		if ( empty( $locations ) ) {
			return esc_html__( 'Sorry, no locations were found.', 'gohigh-seo' );
		}
