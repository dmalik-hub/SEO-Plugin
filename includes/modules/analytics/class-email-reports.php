...
class Email_Reports {
	...
	public function add_variables( $variables ) {
		...
		$variables['logo_alt'] = __( 'Logo', 'gohigh-seo' );
	...
	public function get_default_footer_text() {
		return join(
			...
			[
				...
				sprintf( esc_html__( 'This email was sent to you as a registered member of %s.', 'gohigh-seo' ), '<a href="###SITE_URL###">###SITE_URL_SIMPLE###</a>' ),

				// Translators: placeholder is a link to the settings, with "click here" as the anchor text.
				sprintf( esc_html__( 'To update your email preferences, %s. ###ADDRESS###', 'gohigh-seo' ), '<a href="###SETTINGS_URL###">' . esc_html__( 'click here', 'gohigh-seo' ) . '</a>' ),
			]
		);
	}
	...
	public function get_bg_preview() {
		...
		return '<div class="rank-math-preview-bg" data-title="' . esc_attr( __( 'Preview', 'gohigh-seo' ) ) . '" style="background: ' . esc_attr( Helper::get_settings( 'general.console_email_header_background', $this->get_header_bg_default() ) ) . '"></div>' . $script;
	}
	...
	public function get_subject_default() {
		return sprintf(
			...
			__( 'Rank Math [SEO Report] - %s', 'gohigh-seo' ),
