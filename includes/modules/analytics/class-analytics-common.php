...
use GoHighSEO\Analytics\Analytics;
...
class Analytics_Common {
	...
	public function dashboard_widget() {
		?>
		<h3>
			<?php esc_html_e( 'Analytics', 'gohigh-seo' ); ?>
			<span><?php esc_html_e( 'Last 30 Days', 'gohigh-seo' ); ?></span>
			<a href="<?php echo esc_url( Helper::get_admin_url( 'analytics' ) ); ?>" class="rank-math-view-report" title="<?php esc_html_e( 'View Report', 'gohigh-seo' ); ?>">
	...
	public function admin_bar_items( $menu ) {
		...
		$menu->add_sub_menu(
			'analytics',
			[
				'title'    => esc_html__( 'Analytics', 'gohigh-seo' ) . '<span class="rm-menu-new update-plugins" style="background: ' . $dot_color . ';margin-left: 5px;min-width: 10px;height: 10px;margin-bottom: -1px;display: inline-block;border-radius: 5px;"><span class="plugin-count"></span></span>',
				'href'     => Helper::get_admin_url( 'analytics' ),
				'meta'     => [ 'title' => esc_html__( 'Review analytics and sitemaps', 'gohigh-seo' ) ],
				'priority' => 20,
			]
		);
	}
	...
	public function analytics_clear_caches() {
		DB::purge_cache();
		return __( 'Analytics cache cleared.', 'gohigh-seo' );
	}
	...
	public function analytics_reindex_posts() {
		...
		( new \GoHighSEO\Analytics\Workflow\Objects() )->flat_posts();

		return __( 'Post re-index in progress.', 'gohigh-seo' );
	}
	...
	public function analytics_fix_collations() {
		...
		return $changed ? sprintf(
			...
			_n( '%1$d collation changed to %2$s.', '%1$d collations changed to %2$s.', $changed, 'gohigh-seo' ),
			$changed,
			'`' . $objects_coll . '`'
		) : __( 'No collation mismatch to fix.', 'gohigh-seo' );
	}
	...
	private function get_dashboard_widget_items() {
		...
		return [
			'search-traffic'    => [
				'label' => __( 'Search Traffic', 'gohigh-seo' ),
				'desc'  => __( 'This is the number of pageviews carried out by visitors from Search Engines.', 'gohigh-seo' ),
				'value' => $is_connected && defined( 'RANK_MATH_PRO_FILE' ),
				'data'  => isset( $data->pageviews ) ? $data->pageviews : '',
			],
			'total-impressions' => [
				'label' => __( 'Total Impressions', 'gohigh-seo' ),
				'desc'  => __( 'How many times your site showed up in the search results.', 'gohigh-seo' ),
				'value' => true,
				'data'  => $data->impressions,
			],
			'total-clicks'      => [
				'label' => __( 'Total Clicks', 'gohigh-seo' ),
				'desc'  => __( 'How many times your site was clicked on in the search results.', 'gohigh-seo' ),
				'value' => ! $is_connected || ( $is_connected && ! defined( 'RANK_MATH_PRO_FILE' ) ),
				'data'  => $data->clicks,
			],
			'total-keywords'    => [
				'label' => __( 'Total Keywords', 'gohigh-seo' ),
				'desc'  => __( 'Total number of keywords your site ranks for within top 100 positions.', 'gohigh-seo' ),
				'value' => true,
				'data'  => $data->keywords,
			],
			'average-position'  => [
				'label'  => __( 'Average Position', 'gohigh-seo' ),
				'desc'   => __( 'Average position of all the keywords ranking within top 100 positions.', 'gohigh-seo' ),
				'value'  => true,
				'revert' => true,
				'data'   => $data->position,
			],
