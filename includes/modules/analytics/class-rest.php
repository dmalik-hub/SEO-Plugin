...
use GoHighSEO\Google\PageSpeed;
use GoHighSEO\SEO_Analysis\SEO_Analyzer;
...
class Rest extends WP_REST_Controller {
	...
	public function __construct() {
		$this->namespace = \GoHighSEO\Rest\Rest_Helper::BASE . '/an';
	}
	...
	public function get_tracked_keyword_summary() {
		\GoHighSEO\Admin\Api::get()->get_settings();
	...
	public function add_track_keyword( WP_REST_Request $request ) {
		...
		$total_keywords = Keywords::get()->get_tracked_keywords_count();
		$response       = \GoHighSEO\Admin\Api::get()->keywords_info( $registered['username'], $registered['api_key'], $total_keywords );
		if ( $response ) {
	...
	public function delete_all_tracked_keywords() {
		...
		$response = \GoHighSEO\Admin\Api::get()->keywords_info( $registered['username'], $registered['api_key'], 0 );
		if ( $response ) {
			update_option( 'rank_math_keyword_quota', $response );
		}
	...
	private function get_page_analysis_score( $url ) {
		...
		if ( ! $analyzer->run_api_tests() ) {
			$message = $analyzer->api_error ?? esc_html__( 'An error occurred while running the SEO analysis.', 'gohigh-seo' );

			return new WP_Error(
				'seo_analysis_error',
				/* translators: %s: Error message */
				sprintf( esc_html__( 'SEO Analysis failed: %s', 'gohigh-seo' ), $message )
			);
		}

		$analyzer->build_results();
		if ( empty( $analyzer->results ) ) {
			$message = $analyzer->api_error ?? esc_html__( 'No results found for the SEO analysis.', 'gohigh-seo' );
	...
	public function get_tracked_keywords_rows_args() {
		...
		$query_params['search'] = [
			'description'       => esc_html__( 'Keyword to search.', 'gohigh-seo' ),
			'type'              => 'string',
			'default'           => '',
	...
	public function get_keyword_pages_args() {
		...
		$query_params['query'] = [
			'description'       => esc_html__( 'Query to search.', 'gohigh-seo' ),
			'type'              => 'string',
			'default'           => '',
			...
			'validate_callback' => function ( $param, $request, $key ) {
				if ( empty( $param ) ) {
					return new WP_Error(
						...
						sprintf( esc_html__( 'The %s parameter must not be empty.', 'gohigh-seo' ), $key ),
						[ 'status' => 400 ]
					);
				}
	...
	public function get_add_track_keyword_args() {
		...
		$query_params['keyword'] = [
			'description'       => esc_html__( 'Keyword to add.', 'gohigh-seo' ),
			'type'              => 'string',
			'default'           => '',
			'sanitize_callback' => 'rest_sanitize_request_arg',
			'validate_callback' => function ( $param, $request, $key ) {
	...
	public function get_auto_add_focus_keywords_args() {
		...
		$query_params['data'] = [
			'description'       => esc_html__( 'Data to add.', 'gohigh-seo' ),
			'type'              => 'object',
			'default'           => '',
			...
			'validate_callback' => function ( $param, $request, $key ) {
				...
				if ( ! empty( $param['enable_auto_import'] ) && ! is_numeric( $param['enable_auto_import'] ) ) {
					return new WP_Error(
						'param_value_invalid',
						/* translators: %s: parameter name */
						sprintf( esc_html__( 'The %s parameter must be numeric.', 'gohigh-seo' ), 'enable_auto_import' ),
						[ 'status' => 400 ]
					);
				}
				...
				if ( ! empty( $param['post_types'] ) ) {
					if ( ! is_array( $param['post_types'] ) ) {
						return new WP_Error(
							'param_value_invalid',
							/* translators: %s: parameter name */
							sprintf( esc_html__( 'The %s parameter must be array.', 'gohigh-seo' ), 'post_types' ),
							[ 'status' => 400 ]
						);
					}
					...
					if ( ! preg_match( '/^[a-zA-Z0-9-_]+$/', implode( '', $param['post_types'] ) ) ) {
						return new WP_Error(
							'param_value_invalid',
							/* translators: %s: parameter name */
							sprintf( esc_html__( 'The %s parameter must be valid post type.', 'gohigh-seo' ), 'post_types' ),
							[ 'status' => 400 ]
						);
					}
				}
				...
				if ( ! empty( $param['secondary_keyword'] ) && ! is_bool( $param['secondary_keyword'] ) ) {
					return new WP_Error(
						'param_value_invalid',
						/* translators: %s: parameter name */
						sprintf( esc_html__( 'The %s parameter must be boolean.', 'gohigh-seo' ), 'secondary_keyword' ),
						[ 'status' => 400 ]
					);
				}
	...
	public function get_pagespeed_args() {
		...
		$query_params['id'] = [
			'description'       => esc_html__( 'Record id.', 'gohigh-seo' ),
			'type'              => 'integer',
			'default'           => 0,
			'sanitize_callback' => 'rest_sanitize_request_arg',
			'validate_callback' => function ( $param, $request, $key ) {
				...
				if ( ! is_numeric( $param ) ) {
					return new WP_Error(
						'param_value_invalid',
						/* translators: %s: parameter name */
						sprintf( esc_html__( 'The %s parameter must be integer.', 'gohigh-seo' ), $key ),
						[ 'status' => 400 ]
					);
				}
		...
		$query_params['objectID'] = [
			'description'       => esc_html__( 'Post id.', 'gohigh-seo' ),
			'type'              => 'integer',
			'default'           => 0,
			'sanitize_callback' => 'rest_sanitize_request_arg',
			'validate_callback' => function ( $param, $request, $key ) {
		...
		$query_params['force'] = [
			'description'       => esc_html__( 'Force update pagespeed.', 'gohigh-seo' ),
			'type'              => 'boolean',
			'default'           => false,
			'sanitize_callback' => 'rest_sanitize_request_arg',
			'validate_callback' => function ( $param, $request, $key ) {
				if ( ! is_bool( $param ) ) {
					return new WP_Error(
						'param_value_invalid',
						/* translators: %s: parameter name */
						sprintf( esc_html__( 'The %s parameter must be boolean.', 'gohigh-seo' ), $key ),
						[ 'status' => 400 ]
					);
				}
		...
		$query_params['isAdminBar'] = [
			'description'       => esc_html__( 'Is admin bar.', 'gohigh-seo' ),
			'type'              => 'boolean',
			'default'           => false,
			'sanitize_callback' => 'rest_sanitize_request_arg',
			'validate_callback' => function ( $param, $request, $key ) {
	...
	public function get_collection_params() {
		...
		$query_params['order'] = [
			'description'       => esc_html__( 'Order sort attribute ascending or descending.', 'gohigh-seo' ),
			'type'              => 'string',
			'default'           => 'desc',
		...
		$query_params['orderby'] = [
			'description'       => esc_html__( 'Sort collection by post attribute.', 'gohigh-seo' ),
			'type'              => 'string',
			'default'           => 'date',
			'enum'              => [
				'author',
