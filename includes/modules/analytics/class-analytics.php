...
use GoHighSEO\Analytics\Workflow\Jobs;
use GoHighSEO\Analytics\Workflow\Workflow;
use GoHighSEO\Admin\Admin_Helper as ProAdminHelper;
use GoHighSEO\Analytics\DB;
use GoHighSEO\Google\Analytics as GoogleAnalytics;
use GoHighSEO\Analytics\Email_Reports as Email_Reports_Base;
...
class Analytics {
	...
	public function options_data( $data ) {
		...
		$data['analytics']['console_email_heading']           = Helper::get_settings( 'general.console_email_heading', 'SEO Report of Your Website' );
	...
	public function adsense_options( $html, $analytics, $all_services ) {
		...
		<div class="<?php echo esc_attr( $adsense_classes ); ?>" tabindex="0">
			<header>
				<h3><span class="rank-math-connection-status-wrap"><span class="<?php echo esc_attr( $adsense_status_classes ); ?>" title="<?php echo esc_attr( $adsense_status ); ?>"></span></span><?php esc_html_e( 'AdSense', 'gohigh-seo' ); ?></h3>
			</header>
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'rank-math-analytics-pro-stats', $uri . '/assets/css/admin-bar.css', [ 'rank-math-analytics-stats' ], rank_math()->version );
		wp_enqueue_script( 'rank-math-analytics-pro-stats', $uri . '/assets/js/admin-bar.js', [ 'rank-math-analytics-stats' ], rank_math()->version, true );
		wp_set_script_translations( 'rank-math-analytics-pro-stats', 'gohigh-seo', RANK_MATH_PRO_PATH . 'languages/' );
	 ...
	 *
	 * @param array $values Array of localized data.
	 *
	 * @return array
	 */
	...
	public function admin_bar_items( $menu ) {
		...
		if ( is_singular( $post_types ) && Helper::is_post_indexable( get_the_ID() ) ) {
			$menu->add_sub_menu(
				'post_analytics',
				[
					'title'    => esc_html__( 'Post Analytics', 'gohigh-seo' ),
					'href'     => Helper::get_admin_url( 'analytics#/single/' . get_the_ID() ),
					'meta'     => [ 'title' => esc_html__( 'Analytics Report', 'gohigh-seo' ) ],
					'priority' => 20,
				]
			);
		}
	...
	public function enqueue_analytics() {
		...
		wp_set_script_translations( 'gohigh-seo-analytics', 'gohigh-seo', RANK_MATH_PRO_PATH . 'languages/' );
	}
	...
	public function add_country_dropdown3() {
		...
		?>
		<div class="cmb-row-col">
			<label for="site-console-country"><?php esc_html_e( 'Country', 'gohigh-seo' ); ?></label>
	...
	public function add_country_dropdown2() {
		...
		<div class="cmb-row-col country-option">
			<label for="site-analytics-country"><?php esc_html_e( 'Country', 'gohigh-seo' ); ?></label>
	...
	public function send_summary() {
		...
		if ( $registered && isset( $registered['username'] ) && isset( $registered['api_key'] ) ) {
			...
			\GoHighSEO\Admin\Api::get()->send_summary(
				[
	...
	public function change_description() {
		return sprintf(
			...
			__( 'Enter the number of days to keep Analytics data in your database. The maximum allowed days are %d. Though, 2x data will be stored in the DB for calculating the difference properly.', 'gohigh-seo' ),
	...
	public function before_settings_save( $type, $settings ) {
		...
		\GoHighSEO\Admin\Api::get()->sync_setting( $settings['sync_global_setting'] );
		$this->send_summary();
	}
	...
	public function google_permission_info( $data ) {
		...
		$permissions .= esc_html__( 'AdSense: ', 'gohigh-seo' ) . Permissions::get_status_text( Permissions::has_adsense() ) . ', ';
		$permissions .= esc_html__( 'Analytics: ', 'gohigh-seo' ) . Permissions::get_status_text( Permissions::has_analytics() );
	...
	public function post_column_search_traffic( $post_id ) {
		...
		<span class="rank-math-column-display rank-math-search-traffic">
			<strong>
				<?php
					$analytics_connected
						? esc_html_e( 'Search Traffic:', 'gohigh-seo' )
						: esc_html_e( 'Search Impression:', 'gohigh-seo' );
				?>
			</strong>
