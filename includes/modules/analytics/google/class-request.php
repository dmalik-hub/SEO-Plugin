...
class Request {
		...
		if ( ! $this->refresh_token() || ! is_scalar( $this->token ) ) {
			if ( ! $this->is_notice_added ) {
				...
				$this->last_error      = sprintf(
					...
					wp_kses_post( __( 'There is a problem with the Google auth token. Please <a href="%1$s" class="button button-link rank-math-reconnect-google">reconnect your app</a>', 'gohigh-seo' ) ),
	...
	private function log_response( $http_verb = '', $url = '', $args = [], $response = [], $formatted_response = '', $params = [], $text = '' ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			esc_html__( 'Bad request. Please check the code.', 'gohigh-seo' ) : $formatted_response['error_description'];
		} else {
	...
	private function determine_success( $response, $formatted_response ) {
		...
		$message = esc_html__( 'Unknown error, call get_response() to find out what happened.', 'gohigh-seo' );
