...
class Console extends Analytics {
	...
	public function get_site_verification_token( $url ) {
		...
		return \GoHighSEO\CMB2::sanitize_webmaster_tags( $response['token'] );
	}
	...
	public function get_search_analytics( $args = [] ) {
		...
		if ( ! $this->is_success() ) {
			return new WP_Error( 'request_failed', __( 'The Google Search Console request failed.', 'gohigh-seo' ) );
		}
