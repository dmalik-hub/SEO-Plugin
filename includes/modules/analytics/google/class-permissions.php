...
class Permissions {
	...
	public static function get_status() {
		return [
			esc_html__( 'Search Console', 'gohigh-seo' ) => self::get_status_text( self::has_console() ),
		];
	}
	...
	public static function get_status_text( $check ) {
		return $check ? esc_html__( 'Given', 'gohigh-seo' ) : esc_html__( 'Not Given', 'gohigh-seo' );
	}
	...
	public static function print_warning() {
		...
		<p class="warning">
			<strong class="warning">
				<?php esc_html_e( 'Warning:', 'gohigh-seo' ); ?>
			</strong>
			<?php
				/* translators: %s is the reconnect link. */
				printf( wp_kses_post( __( 'You have not given the permission to fetch this data. Please <a href="%s">reconnect</a> with all required permissions.', 'gohigh-seo' ) ), esc_url( wp_nonce_url( admin_url( 'admin.php?reconnect=google' ), 'rank_math_reconnect_google' ) ) );
			?>
		</p>
		<?php
	}
