...
class Block_HowTo extends Block {
	...
	private function build_duration( $attrs ) {
		...
		if ( $days > 0 ) {
			...
			$elements[] = sprintf( _n( '%d day', '%d days', $days, 'gohigh-seo' ), $days );
		}

		if ( $hours > 0 ) {
			/* translators: %d is the number of hours. */
			$elements[] = sprintf( _n( '%d hour', '%d hours', $hours, 'gohigh-seo' ), $hours );
		}

		if ( $minutes > 0 ) {
			/* translators: %d is the number of minutes. */
			$elements[] = sprintf( _n( '%d minute', '%d minutes', $minutes, 'gohigh-seo' ), $minutes );
		}
