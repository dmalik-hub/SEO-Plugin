...
class Snippet_Shortcode {
	...
	public function rich_snippet( $atts ) {
		...
		if ( empty( $data ) || empty( $data['schema'] ) ) {
			return esc_html__( 'No schema found.', 'gohigh-seo' );
		}
	...
	public function get_opening_hours( $field_id ) {
		...
		echo '<div>';
		echo '<strong>' . esc_html__( 'Opening Hours', 'gohigh-seo' ) . '</strong><br />';
	...
	public function get_opening_hour( $opening_hour ) {
		$labels = [
			'dayOfWeek' => esc_html__( 'Days', 'gohigh-seo' ),
			'opens'     => esc_html__( 'Opening Time', 'gohigh-seo' ),
			'closes'    => esc_html__( 'Closing Time', 'gohigh-seo' ),
		];
	...
	public function show_ratings( $field_id = 'review.reviewRating.ratingValue' ) {
		...
		<div class="rank-math-total-wrapper">
			...
			<strong><?php echo $this->do_filter( 'review/text', esc_html__( 'Editor\'s Rating:', 'gohigh-seo' ) ); // phpcs:ignore ?></strong><br />
