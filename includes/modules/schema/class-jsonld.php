...
class JsonLD {
	...
	public function add_context_data( $data ) {
		...
		$snippets           = [
			'\\GoHighSEO\\Schema\\Publisher'     => ! isset( $data['publisher'] ) && $can_add_global,
			'\\GoHighSEO\\Schema\\Website'       => $can_add_global,
			'\\GoHighSEO\\Schema\\PrimaryImage'  => is_singular() && ! post_password_required() && $can_add_global,
			'\\GoHighSEO\\Schema\\Breadcrumbs'   => $this->can_add_breadcrumb(),
			'\\GoHighSEO\\Schema\\Webpage'       => $can_add_global,
			'\\GoHighSEO\\Schema\\Author'        => is_author() || ( is_singular() && $can_add_global ),
			'\\GoHighSEO\\Schema\\Products_Page' => $is_product_archive,
			'\\GoHighSEO\\Schema\\Singular'      => ! post_password_required() && is_singular(),
		];
