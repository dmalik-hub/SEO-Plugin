...
class Rest extends WP_REST_Controller {
	...
	public function get_video_data( WP_REST_Request $request ) {
		...
		global $wp_embed;
		return ( new \GoHighSEO\Schema\Video\Parser( get_post( $object_id ) ) )->get_metadata( $wp_embed->autoembed( $url ) );
	}
	...
	private function get_save_template_args() {
		return [
			...
			'schema' => [
				'type'              => 'object',
				'required'          => true,
				'description'       => esc_html__( 'Schema data.', 'gohigh-seo' ),
				'validate_callback' => [ '\\GoHighSEO\\Rest\\Rest_Helper', 'is_param_empty' ],
			],
		];
	}
	...
	public static function get_permissions_check( $request ) { // phpcs:ignore
		...
		if ( $post_id ) {
			$post = \GoHighSEO\Rest\Rest_Helper::get_post( $post_id );
			if ( is_wp_error( $post ) ) {
				return $post;
			}
			...
			if ( $post->post_type !== 'rank_math_schema' ) {
				return new WP_Error(
					'rest_forbidden',
					__( 'Cannot modify non-template posts.', 'gohigh-seo' ),
			...
			return new WP_Error(
				'rest_cannot_edit',
				__( 'Sorry, you are not allowed to edit this template.', 'gohigh-seo' ),
				[ 'status' => rest_authorization_required_code() ]
			);
		}
		...
		return new WP_Error(
			'rest_cannot_edit',
			__( 'Sorry, you are not allowed to save template.', 'gohigh-seo' ),
			[ 'status' => rest_authorization_required_code() ]
		);
	}
	...
	private function get_video_args() {
		return [
			...
			'url'      => [
				'required'          => true,
				'description'       => esc_html__( 'Video URL.', 'gohigh-seo' ),
				'validate_callback' => [ '\\GoHighSEO\\Rest\\Rest_Helper', 'is_param_empty' ],
			],
		];
	}
