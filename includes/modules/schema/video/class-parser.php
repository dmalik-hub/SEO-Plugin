...
class Parser {
	...
	private function get_video_metadata( $url, $generate_image = true ) {
		...
		foreach ( $networks as $network ) {
			...
			$data = \call_user_func( [ '\\GoHighSEO\\Schema\\' . $network, 'match' ], $args );
			if ( is_array( $data ) ) {
				break;
			}
		}
	...
	private function save_video_thumbnail( $data ) {
		...
		$image_title = __( 'Video Thumbnail', 'gohigh-seo' );
		if ( ! empty( $data['name'] ) ) {
		...
		$attachment_title = sprintf( __( 'Video Thumbnail: %s', 'gohigh-seo' ), $image_title );
