...
use GoHighSEO\Schema\DB;
use GoHighSEO\Traits\Hooker;
use GoHighSEO\Rest\Sanitize;
...
class Admin {
	...
	public function add_pro_schema_types( $types ) {
		$types = array_merge(
			$types,
			[
				'dataset'   => esc_html__( 'DataSet', 'gohigh-seo' ),
				'FactCheck' => esc_html__( 'Fact Check', 'gohigh-seo' ),
				'movie'     => esc_html__( 'Movie', 'gohigh-seo' ),
			]
		);

		unset( $types['off'] );
		ksort( $types, SORT_NATURAL | SORT_FLAG_CASE );
		return [ 'off' => esc_html__( 'None (Click here to set one)', 'gohigh-seo' ) ] + $types;
	}
	...
	public function overwrite_wplink() {
		...
		wp_register_script( 'wplink', RANK_MATH_PRO_URL . 'assets/admin/js/wplink.js', [ 'jquery', 'wp-a11y' ], rank_math()->version, true );

		wp_localize_script(
			'wplink',
			'wpLinkL10n',
			[
				'title'             => esc_html__( 'Insert/edit link', 'gohigh-seo' ),
				'update'            => esc_html__( 'Update', 'gohigh-seo' ),
				'save'              => esc_html__( 'Add Link', 'gohigh-seo' ),
				'noTitle'           => esc_html__( '(no title)', 'gohigh-seo' ),
				'noMatchesFound'    => esc_html__( 'No matches found.', 'gohigh-seo' ),
				'linkSelected'      => esc_html__( 'Link selected.', 'gohigh-seo' ),
				'linkInserted'      => esc_html__( 'Link inserted.', 'gohigh-seo' ),
				...
				'schemaMarkupLabel' => esc_html__( 'Use in Schema Markup', 'gohigh-seo' ),
				'linkTitle'         => esc_html__( 'Link Title', 'gohigh-seo' ),
			]
		);
	}
	...
	public function admin_scripts() {
		...
		$this->localize_data();
		wp_enqueue_style( 'rank-math-schema-pro', RANK_MATH_PRO_URL . 'includes/modules/schema/assets/css/schema.css', [ 'rank-math-schema' ], rank_math()->version );

		wp_enqueue_script(
			'gohigh-seo-schema-filters',
		...
		wp_set_script_translations( 'gohigh-seo-schema-filters', 'gohigh-seo', RANK_MATH_PRO_PATH . 'languages/' );
		...
		if ( $screen instanceof WP_Screen && 'rank_math_schema' === $screen->post_type ) {
			...
			wp_set_script_translations( 'rank-math-schema-pro', 'gohigh-seo', RANK_MATH_PRO_PATH . 'languages/' );

			return;
		}

		wp_enqueue_script( 'rank-math-schema-pro', RANK_MATH_PRO_URL . 'includes/modules/schema/assets/js/schema.js', [ 'rank-math-schema' ], rank_math()->version, true );
		wp_set_script_translations( 'rank-math-schema-pro', 'gohigh-seo', RANK_MATH_PRO_PATH . 'languages/' );
	}
	...
	public function deregister_scripts() {
		...
		if ( wp_script_is( 'gohigh-seo-editor', 'registered' ) ) {
			wp_deregister_script( 'gohigh-seo-editor' );
		}
	}
	...
	private function get_post_taxonomies() {
		...
		foreach ( $post_types as $post_type ) {
			...
			$data[ $post_type ] = [ 'all' => esc_html__( 'All Taxonomies', 'gohigh-seo' ) ] + $taxonomies;
		}
