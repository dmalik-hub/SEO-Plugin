...
if ( isset( $schema['timeRequired'] ) && Helper::get_formatted_duration( $schema['timeRequired'] ) ) {
	...
	$time_required[] = ! empty( $duration->h ) ? sprintf( esc_html__( '%d Hour', 'gohigh-seo' ), $duration->h ) : '';

	/* translators: %d Time in minutes */
	$time_required[] = ! empty( $duration->i ) ? sprintf( esc_html__( '%d Min', 'gohigh-seo' ), $duration->i ) : '';

	/* translators: %d Time in seconds */
	$time_required[] = ! empty( $duration->s ) ? sprintf( esc_html__( '%d Sec', 'gohigh-seo' ), $duration->s ) : '';
		...
		<p>
			...
			<span>
				<?php if ( ! empty( $season['seasonNumber'] ) ) { ?>
					<?php echo esc_html__( 'Season', 'gohigh-seo' ); ?> <?php echo esc_html( $season['seasonNumber'] ); ?>
				...
				<?php if ( ! empty( $schema['episodeNumber'] ) ) { ?>
					<?php echo esc_html__( 'Episode', 'gohigh-seo' ); ?> <?php echo esc_html( $schema['episodeNumber'] ); ?>
				<?php } ?>
			</span>
		</p>
		<!-- /wp:paragraph -->
		...
		<!-- wp:paragraph -->
			<p>
				...
				<?php if ( ! empty( $schema['author'] ) ) { ?>
					<?php echo esc_html__( 'By', 'gohigh-seo' ); ?> <?php echo esc_html( $schema['author']['name'] ); ?>
				<?php } ?>
			</p>
		<!-- /wp:paragraph -->
	</div>
