	...
	public function admin_bar_items( $menu ) {
		$menu->add_sub_menu(
			...
			[
				...
				'meta'     => [ 'title' => esc_html__( 'Site-wide analysis', 'gohigh-seo' ) ],
				'priority' => 50,
			]
		);
		...
		if ( ! is_admin() && ! is_404() ) {
			...
			$menu->add_sub_menu(
				'analyze',
				[
					'title' => $link ? esc_html__( 'Analyze this Page', 'gohigh-seo' ) : esc_html__( 'SEO Analyzer', 'gohigh-seo' ),
					'href'  => Helper::get_admin_url( 'seo-analysis' ) . ( $link ? '&u=' . rawurlencode( $link ) : '' ),
					'meta'  => [ 'title' => esc_html__( 'SEO Analysis for this page', 'gohigh-seo' ) ],
				],
				'seo-analysis'
			);
		}
