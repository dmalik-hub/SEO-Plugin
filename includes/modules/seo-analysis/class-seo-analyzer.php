...
class SEO_Analyzer {
	...
	public function enable_auto_update_in_stored_data() {
		...
		$results['auto_update']['message'] = __( 'Rank Math auto-update option is enabled on your site.', 'gohigh-seo' );
	...
	private function get_api_results() {
		...
		if ( 200 !== $status ) {
			...
			$this->api_error = sprintf( __( 'HTTP %d error.', 'gohigh-seo' ), $status );
			return false;
		}
		...
		if ( ! is_array( $response ) ) {
			$this->api_error = __( 'Unexpected API response.', 'gohigh-seo' );
			return false;
		}
	...
	private function get_serp_data() {
		...
		if ( empty( $description ) ) {
			$description = __( '(No Description)', 'gohigh-seo' );
		}
