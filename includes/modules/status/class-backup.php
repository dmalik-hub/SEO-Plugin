...
class Backup {
	...
	public static function create_backup() {
		...
		if ( is_null( $key ) ) {
			return [
				'type'    => 'error',
				'message' => esc_html__( 'Unable to create backup this time.', 'gohigh-seo' ),
			];
		}
		...
		return [
			'type'    => 'success',
			'message' => esc_html__( 'Backup created successfully.', 'gohigh-seo' ),
	...
	public static function restore_backup( $key ) {
		...
		if ( ! array_key_exists( $key, $backups ) ) {
			return [
				'type'    => 'error',
				'message' => esc_html__( 'Backup does not exist.', 'gohigh-seo' ),
			];
		}
		...
		return [
			'type'    => 'success',
			'message' => esc_html__( 'Backup restored successfully.', 'gohigh-seo' ),
		];
	}
	...
	public static function delete_backup( $key ) {
		...
		if ( ! isset( $backups[ $key ] ) ) {
			return [
				'type'    => 'error',
				'message' => esc_html__( 'No backup key found to delete.', 'gohigh-seo' ),
				'backups' => self::get_backups(),
			];
		}
		...
		return [
			'type'    => 'success',
			'message' => esc_html__( 'Backup successfully deleted.', 'gohigh-seo' ),
			'backups' => self::get_backups(),
		];
	}
}
