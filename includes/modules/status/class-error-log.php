...
class Error_Log {
	...
	private static function error_log_load_error() {
		...
		if (
			...
			return esc_html__( 'The error log cannot be retrieved.', 'gohigh-seo' );
		}

		// Error log must be smaller than 100 MB.
		$size = $wp_filesystem->size( $log_file );
		if ( $size > 100000000 ) {
			return esc_html__( 'The error log cannot be retrieved: Error log file is too large.', 'gohigh-seo' );
		}
