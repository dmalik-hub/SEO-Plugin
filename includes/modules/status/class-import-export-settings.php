...
namespace GoHighSEO\Status;

use WP_REST_Response;
use GoHighSEO\Helper;
use GoHighSEO\Helpers\Param;
use GoHighSEO\Status\Backup;
...
class Import_Export_Settings {
	...
	public static function import() {
		...
		if ( isset( $file['error'] ) ) {
			return [
				'error' => esc_html__( 'Settings could not be imported:', 'gohigh-seo' ) . ' ' . $file['error'],
			];
		}

		if ( ! isset( $file['file'] ) ) {
			return [
				'error' => esc_html__( 'Settings could not be imported: Upload failed.', 'gohigh-seo' ),
			];
		}
		...
		if ( is_null( $wp_filesystem ) || ! Helper::is_filesystem_direct() ) {
			return [
				'error' => esc_html__( 'Uploaded file could not be read.', 'gohigh-seo' ),
			];
		}
		...
		if ( is_array( $settings ) && self::do_import_data( $settings ) ) {
			return [
				'success' => esc_html__( 'Settings successfully imported. Your old configuration has been saved as a backup.', 'gohigh-seo' ),
			];
		}

		return [
			'error' => esc_html__( 'No settings found to be imported.', 'gohigh-seo' ),
		];
	}
	...
	private static function set_redirections( $redirections ) {
		foreach ( $redirections as $key => $redirection ) {
			$matched = \GoHighSEO\Redirections\DB::match_redirections_source( $redirection['sources'] );
	...
	public static function get_export_data( array $panels = [] ) {
		...
		if ( ! \GoHighSEO\Helpers\DB::check_table_exists( 'rank_math_redirections' ) ) {
		...
		foreach ( $panels as $panel ) {
			...
			if ( 'redirections' === $panel ) {
				$items = \GoHighSEO\Redirections\DB::get_redirections( [ 'limit' => 1000 ] );
