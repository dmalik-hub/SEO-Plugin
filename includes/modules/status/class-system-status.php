...
use GoHighSEO\Status\Error_Log;
use GoHighSEO\Status\System_Status as System_Status_Free;
...
class System_Status {
	...
	public function filter_status_info( $rankmath ) {
		$rankmath['fields']['version']['label'] = esc_html__( 'Free version', 'gohigh-seo' );
		array_splice(
			$rankmath['fields'],
			1,
			0,
			[
				[
					'label' => esc_html__( 'PRO version', 'gohigh-seo' ),
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'gohigh-seo-status', RANK_MATH_PRO_URL . 'includes/modules/status/assets/js/status.js', [ 'rank-math-status' ], rank_math()->version, true );
		wp_enqueue_script( 'rank-math-gallery-analysis', RANK_MATH_PRO_URL . 'assets/admin/js/product-analysis.js', [], rank_math()->version, true );
	}
	...
	public function add_csv_json_data( $data ) {
		...
		$json = [
			...
			'exportCsvNonce'   => wp_create_nonce( 'rank_math_csv_export' ),
