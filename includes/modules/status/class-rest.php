...
class Rest extends WP_REST_Controller {
	...
	public function __construct() {
		$this->namespace = \GoHighSEO\Rest\Rest_Helper::BASE . '/status';
	}
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			[
				'methods'             => 'POST',
				'callback'            => [ '\\GoHighSEO\\Status\\Import_Export_Settings', 'import' ],
		...
		register_rest_route(
			...
			[
				'methods'             => 'POST',
				'callback'            => [ '\\GoHighSEO\\Status\\Import_Export_Settings', 'export' ],
				'permission_callback' => [ $this, 'has_import_export_permission' ],
			]
		);
		register_rest_route(
	...
	public function has_import_export_permission() {
		if ( ! Helper::has_cap( 'general' ) ) {
			return new WP_Error(
				'rest_cannot_access',
				__( 'Sorry, you are not authorized to Import/Export the settings.', 'gohigh-seo' ),
	...
	public function run_backup( WP_REST_Request $request ) {
		...
		if ( ! in_array( $action, [ 'create', 'delete', 'restore' ], true ) ) {
			return new WP_REST_Response(
				[
					'type'    => 'error',
					'message' => esc_html__( 'Invalid action selected.', 'gohigh-seo' ),
				]
			);
		}
	...
	public function get_view_data( WP_REST_Request $request ) {
		...
		$hash = [
			'version_control' => '\GoHighSEO\Version_Control',
			'tools'           => '\GoHighSEO\Tools\Database_Tools',
			'status'          => '\GoHighSEO\Status\System_Status',
			'import_export'   => '\GoHighSEO\Admin\Import_Export',
		];
		if ( ! isset( $hash[ $view ] ) ) {
			return [];
		}
