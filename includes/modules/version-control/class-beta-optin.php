...
class Beta_Optin {
	...
	public function hooks() {
		...
		$this->action( 'in_plugin_update_message-gohigh-seo/rank-math.php', 'plugin_update_message' );
	...
	public function beta_plugin_information() {
		if ( 'gohigh-seo' !== Param::request( 'plugin' ) ) {
			return;
		}
	...
	public static function has_beta_update( $transient ) {
		return (
			...
			&& ! empty( $transient->response['gohigh-seo/rank-math.php'] )
			&& ! empty( $transient->response['gohigh-seo/rank-math.php']->is_beta )
		);
	}
	...
	public function fetch_trunk_version() {
		...
		$response = wp_remote_get( 'https://plugins.svn.wordpress.org/gohigh-seo/trunk/rank-math.php' );
	...
	public function transient_update_plugins( $value ) {
		...
		$new_version  = isset( $value->response['gohigh-seo/rank-math.php'] ) && ! empty( $value->response['gohigh-seo/rank-math.php']->new_version ) ? $value->response['gohigh-seo/rank-math.php']->new_version : 0;
	...
	public function inject_beta( $value, $beta_version ) {
		...
		$value->response['gohigh-seo/rank-math.php'] = new \stdClass();

		$plugin_data = Version_Control::get_plugin_data( $beta_version, 'https://downloads.wordpress.org/plugin/gohigh-seo.zip' );
		foreach ( $plugin_data as $prop_key => $prop_value ) {
			$value->response['gohigh-seo/rank-math.php']->{$prop_key} = $prop_value;
		}

		$value->response['gohigh-seo/rank-math.php']->is_beta        = true;
		$value->response['gohigh-seo/rank-math.php']->upgrade_notice = self::NOTICE_START_MARKER . ' ' . __( 'This update will install a beta version of Rank Math.', 'gohigh-seo' ) . ' ' . self::NOTICE_END_MARKER;

		if ( empty( $value->no_update ) ) {
			$value->no_update = [];
		}

		unset( $value->no_update['gohigh-seo/rank-math.php'] );
	...
	public function plugin_update_message( $plugin_data ) {
		...
		printf(
			...
			esc_html__( 'This update will install a beta version of Rank Math.', 'gohigh-seo' )
		);
	}
		...
		<script type="text/javascript">
			jQuery( document ).ready( function( $ ) {
				...
				$('.open-plugin-details-modal').each( function( index, element ) {
					if ( element.href.indexOf( 'plugin=gohigh-seo&section=changelog' ) !== -1 ) {
						// Found our link.
						$( element )
