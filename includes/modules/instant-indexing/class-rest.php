...
class Rest extends WP_REST_Controller {
	...
	public function __construct() {
		$this->namespace = \GoHighSEO\Rest\Rest_Helper::BASE . '/in';
	}
		...
		register_rest_route(
			...
			[
				[
					...
					'args'                => [
						'urls' => [
							'description' => __( 'The list of urls to submit to the Instant Indexing API.', 'gohigh-seo' ),
							'type'        => 'string',
							'required'    => true,
						],
					],
		...
		register_rest_route(
			...
			[
				[
					...
					'args'                => [
						'filter' => [
							'description' => __( 'Filter log by type.', 'gohigh-seo' ),
							'type'        => 'string',
		...
		register_rest_route(
			...
			[
				[
					...
					'args'                => [
						'filter' => [
							'description' => __( 'Clear log by type.', 'gohigh-seo' ),
							'type'        => 'string',
							'enum'        => [ 'all', 'manual', 'auto' ],
							'default'     => 'all',
						],
	...
	public function submit_urls( WP_REST_Request $request ) {
		...
		if ( empty( $urls ) ) {
			return new WP_Error( 'empty_urls', __( 'No URLs provided.', 'gohigh-seo' ) );
		}
		...
		if ( ! $urls ) {
			return new WP_Error( 'invalid_urls', __( 'Invalid URLs provided.', 'gohigh-seo' ) );
		}

		$result = Api::get()->submit( $urls );
		if ( ! $result ) {
			return new WP_Error( 'submit_failed', __( 'Failed to submit URLs. See details in the History tab.', 'gohigh-seo' ) );
		}
	...
	public function get_log( WP_REST_Request $request ) {
		...
		foreach ( $result as $key => $value ) {
			...
			$result[ $key ]['timeHumanReadable'] = sprintf( __( '%s ago', 'gohigh-seo' ), human_time_diff( $value['time'] ) );
