...
class Instant_Indexing extends Base {
	...
	public function post_bulk_actions( $actions ) {
		$actions['rank_math_indexnow'] = esc_html__( 'Instant Indexing: Submit Pages', 'gohigh-seo' );
		return $actions;
	}
	...
	public function post_row_actions( $actions, $post ) {
		...
		$actions['indexnow_submit'] = '<a href="' . esc_url( $link ) . '" class="rm-instant-indexing-action rm-indexnow-submit">' . __( 'Instant Indexing: Submit Page', 'gohigh-seo' ) . '</a>';
	...
	public function register_instant_indexing_settings() {
		$tabs = [
			'url-submission' => [
				...
				'title'   => esc_html__( 'Submit URLs', 'gohigh-seo' ),
				'desc'    => esc_html__( 'Send URLs directly to the IndexNow API.', 'gohigh-seo' ) . ' <a href="' . KB::get( 'instant-indexing', 'Indexing Submit URLs' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>',
				'classes' => 'rank-math-advanced-option',
				'file'    => __DIR__ . '/views/console.php',
			],
			'settings'       => [
				'icon'  => 'rm-icon rm-icon-settings',
				'title' => esc_html__( 'Settings', 'gohigh-seo' ),
				/* translators: Link to kb article */
				'desc'  => sprintf( esc_html__( 'Instant Indexing module settings. %s.', 'gohigh-seo' ), '<a href="' . KB::get( 'instant-indexing', 'Indexing Settings' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>' ),
				'file'  => __DIR__ . '/views/options.php',
			],
			'history'        => [
				'icon'    => 'rm-icon rm-icon-htaccess',
				'title'   => esc_html__( 'History', 'gohigh-seo' ),
				'desc'    => esc_html__( 'The last 100 IndexNow API requests.', 'gohigh-seo' ),
				'classes' => 'rank-math-advanced-option',
		...
		new Register_Options_Page(
			[
				...
				'menu_title' => esc_html__( 'Instant Indexing', 'gohigh-seo' ),
	...
	private function add_submit_message_notice( $success, $count ) {
		...
		$notification_message = __( 'Error submitting page to IndexNow.', 'gohigh-seo' );

		if ( $success ) {
			$notification_type    = 'success';
			$notification_message = sprintf(
				/* translators: %s: Number of pages submitted. */
				_n( '%s page submitted to IndexNow.', '%s pages submitted to IndexNow.', $count, 'gohigh-seo' ),
				$count
			);
		}
