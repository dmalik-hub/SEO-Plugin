...
use GoHighSEO\WooCommerce\Migrate_GTIN;
use GoHighSEO\Traits\Hooker;
use GoHighSEO\Schema\DB;
use GoHighSEO\Schema\Product_WooCommerce;
use GoHighSEO\Schema\Product;
...
class WooCommerce {
	...
	public function add_gtin_migration_tool( $tools ) {
		...
		$tools['migrate_gtin_values'] = [
			'title'       => esc_html__( 'GTIN Migration Tool for WooCommerce', 'gohigh-seo' ),
			'description' => esc_html__( 'Migrate GTIN values from the plugin into the native WooCommerce GTIN field.', 'gohigh-seo' ),
			'button_text' => esc_html__( 'Migrate', 'gohigh-seo' ),
		];
	...
	public function migrate_gtin_values() {
		...
		if ( empty( $products ) ) {
			return [
				'status'  => 'error',
				'message' => __( 'No products found to migrate.', 'gohigh-seo' ),
			];
		}

		Migrate_GTIN::get()->start( $products );

		return __( 'The GTIN values from the plugin are being transferred to the built-in WooCommerce GTIN field. This process runs in the background, and you\'ll receive a confirmation message once all product data has been successfully migrated. You can close this page.', 'gohigh-seo' );
	}
	...
	private function add_variable_offer( $variation, &$entity ) {
		...
		$entity['offers'] = [
			...
			'availability'    => Product_WooCommerce::get()->get_availability( $variation ),
