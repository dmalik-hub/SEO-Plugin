...
namespace GoHighSEO\Monitor;

use GoHighSEO\Helper;
use GoHighSEO\Traits\Hooker;
use GoHighSEO\Redirections\DB as RedirectionsDB;
use GoHighSEO\Redirections\Cache as RedirectionsCache;
use GoHighSEO\Admin\List_Table;
use GoHighSEO\Monitor\Admin;
...
class Table extends List_Table {
	...
	public function __construct() {
		parent::__construct(
			[
				...
				'singular' => 'event',
				'plural'   => 'events',
				'no_items' => esc_html__( 'The 404 error log is empty.', 'gohigh-seo' ),
			]
		);
	}
	...
	protected function extra_tablenav( $which ) {
		...
		<div class="alignleft actions">
			<input type="button" class="button button-link-delete action rank-math-clear-logs" value="<?php esc_attr_e( 'Clear Log', 'gohigh-seo' ); ?>">
		</div>
		<?php
	}
	...
	protected function column_uri( $item ) {
		$link = '<a href="' . esc_url( home_url( $item['uri'] ) ) . '" target="_blank" title="' . esc_attr__( 'View', 'gohigh-seo' ) . '">' . esc_html( $item['uri_decoded'] ) . '</a>';
	...
	public function column_actions( $item ) {
		...
		$actions['view'] = sprintf(
			'<a href="%s" target="_blank">' . esc_html__( 'View', 'gohigh-seo' ) . '</a>',
		...
		$actions['delete'] = sprintf(
			'<a href="%s" class="rank-math-404-delete">' . esc_html__( 'Delete', 'gohigh-seo' ) . '</a>',
	...
	private function add_redirection_actions( $item, &$actions ) {
		...
		if ( $redirection ) {
			...
			$actions['view_redirection'] = sprintf( '<a href="%s" target="_blank">' . esc_html__( 'View Redirection', 'gohigh-seo' ) . '</a>', $url );
			return;
		}
		...
		$actions['redirect'] = sprintf(
			...
			$url,
			esc_html__( 'Redirect', 'gohigh-seo' )
		);
	}
	...
	public function get_columns() {
		$columns = [
			...
			'uri'            => esc_html__( 'URI', 'gohigh-seo' ),
			'referer'        => esc_html__( 'Referer', 'gohigh-seo' ),
			'user_agent'     => esc_html__( 'User-Agent', 'gohigh-seo' ),
			'times_accessed' => esc_html__( 'Hits', 'gohigh-seo' ),
			'accessed'       => esc_html__( 'Access Time', 'gohigh-seo' ),
		];
	...
	public function get_bulk_actions() {
		$actions = [
			'redirect' => esc_html__( 'Redirect', 'gohigh-seo' ),
			'delete'   => esc_html__( 'Delete', 'gohigh-seo' ),
		];
