...
class Monitor_Pro extends CSV {
	...
	public function export_panel() {
		...
		<div class="rank-math-box rank-math-export-404-panel <?php echo Param::get( 'export-404' ) ? '' : 'hidden'; ?>">
			<h3><?php esc_html_e( 'Export 404 Logs', 'gohigh-seo' ); ?></h3>
			<p class="description">
				<?php esc_html_e( 'Export and download 404 logs from a selected period of time in the form of a CSV file. Leave the from/to fields empty to export all logs.', 'gohigh-seo' ); ?>
			</p>
			<div class="form-wrap">
				<form action="" method="get" autocomplete="off">
					...
					<div class="form-field">
						<label for="rank_math_export_404_date_from">
								<?php esc_html_e( 'From date', 'gohigh-seo' ); ?>
						</label>
					...
					<div class="form-field">
						<label for="rank_math_export_404_date_to">
							<?php esc_html_e( 'To date', 'gohigh-seo' ); ?>
						</label>
					...
					<div class="rank_math_export_404_submit_wrap form-field">
						<input type="submit" value="<?php esc_attr_e( 'Export', 'gohigh-seo' ); ?>" class="button button-primary">
					</div>
				</form>
			</div>
		</div>
	...
	public function maybe_export() {
		...
		if ( ! current_user_can( 'export' ) || ! Helper::has_cap( '404_monitor' ) ) {
			wp_die( esc_html__( 'Sorry, your user does not seem to have the necessary capabilities to export.', 'gohigh-seo' ) );
		}

		if ( wp_verify_nonce( Param::get( '_nonce' ), 'export_404' ) ) {
			wp_die( esc_html__( 'Nonce error. Please try again.', 'gohigh-seo' ) );
		}
	...
	public function enqueue( $hook ) {
		...
		wp_enqueue_script( 'gohigh-seo-404-monitor', $url . 'js/404-monitor.js', [ 'jquery-ui-core', 'jquery-ui-datepicker' ], RANK_MATH_PRO_VERSION, true );
		wp_enqueue_style( 'gohigh-seo-404-monitor', $url . 'css/404-monitor.css', [], RANK_MATH_PRO_VERSION );
	}
	...
	public function manage_columns( $columns ) {
		...
		$columns['total_hits'] = esc_html__( 'Hits', 'gohigh-seo' );
		return $columns;
	}
