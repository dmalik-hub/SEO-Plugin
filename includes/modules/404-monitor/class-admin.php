...
class Admin extends Base {
	...
	protected function do_delete() {
		...
		if ( $count > 0 ) {
			Helper::add_notification(
				...
				sprintf( esc_html__( '%d log(s) deleted.', 'gohigh-seo' ), $count ),
				[ 'type' => 'success' ]
			);
		}
	}
	...
	protected function do_clear_log() {
		...
		Helper::add_notification(
			/* translators: delete counter */
			sprintf( esc_html__( 'Log cleared - %d items deleted.', 'gohigh-seo' ), $count ),
			[ 'type' => 'success' ]
		);
	}
	...
	public function register_admin_page() {
		...
		$this->page = new Page(
			...
			esc_html__( '404 Monitor', 'gohigh-seo' ),
			[
				'position'   => 30,
				'parent'     => 'rank-math',
		...
		if ( $this->page->is_current_page() ) {
			Helper::add_json( 'logConfirmClear', esc_html__( 'Are you sure you wish to delete all 404 error logs?', 'gohigh-seo' ) );
		...
		Arr::insert(
			...
			[
				'404-monitor' => [
					...
					'desc'  => sprintf( esc_html__( 'Monitor broken pages that ruin user-experience and affect SEO. %s.', 'gohigh-seo' ), '<a href="' . KB::get( '404-monitor-settings', 'Options Panel 404 Monitor Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>' ),
