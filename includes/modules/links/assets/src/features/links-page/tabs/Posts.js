	...
	if ( ! score ) {
		return (
			<span className="rank-math-seo-score na">
				<strong>{ __( 'Not Set', 'gohigh-seo' ) }</strong>
			</span>
		)
	}
	let scoreClass = 'bad'
...
const Posts = () => {
	...
	const headers = useMemo( () => [
		{
			key: 'post_title',
			label: __( 'Post Title', 'gohigh-seo' ),
			isSortable: true,
			isHideable: false,
			render: ( value, row ) => (
				<>
		...
		},
		{
			...
			label: __( 'Internal Links', 'gohigh-seo' ),
			isSortable: true,
			render: ( value, row ) => {
				...
				return (
					...
					>
						{ count }
						<span>{ __( 'View internal links from this post', 'gohigh-seo' ) }</span>
					</button>
				)
			},
		},
		{
			...
			label: __( 'External Links', 'gohigh-seo' ),
			isSortable: true,
			render: ( value, row ) => {
				const count = parseInt( value, 10 ) || 0
				if ( count === 0 ) {
				...
				return (
					...
					>
						{ count }
						<span>{ __( 'View external links from this post', 'gohigh-seo' ) }</span>
					</button>
				)
			},
		},
		{
			...
			label: __( 'Incoming Links', 'gohigh-seo' ),
			isSortable: true,
			render: ( value, row ) => {
				const count = parseInt( value, 10 ) || 0
				if ( count === 0 ) {
					return (
						<span className="rank-math-orphan-warning rank-math-tooltip" title={ __( 'Orphan post — no incoming links', 'gohigh-seo' ) }>
							<i className="dashicons dashicons-warning" />
							<span>{ __( 'This post has no internal links. We recommend that you add links to this URL in other posts of your website.', 'gohigh-seo' ) }</span>
						</span>
					)
				}
				return (
					...
					>
						{ count }
						<span>{ __( 'View incoming links to this post', 'gohigh-seo' ) }</span>
					</button>
				)
			},
		},
	...
	const orphanOptions = [
		{ label: __( 'All Posts', 'gohigh-seo' ), value: '', statusType: 'all' },
		{ label: __( 'Orphan Posts', 'gohigh-seo' ), value: 'orphan', statusType: 'orphan' },
		{ label: __( 'With Incoming Links', 'gohigh-seo' ), value: 'linked', statusType: 'linked' },
	]

	// SEO score range filter toggle options — statusType drives colored pill colors
	const seoScoreOptions = [
		{ label: __( 'All Scores', 'gohigh-seo' ), value: '', statusType: 'all' },
		{ label: __( 'Great', 'gohigh-seo' ), value: 'great', statusType: 'great' },
		{ label: __( 'Good', 'gohigh-seo' ), value: 'good', statusType: 'good' },
		{ label: __( 'Bad', 'gohigh-seo' ), value: 'bad', statusType: 'bad' },
		{ label: __( 'Not Set', 'gohigh-seo' ), value: 'no-score', statusType: 'no-score' },
	]
	...
	const filterStagingFilters = useMemo( () => [
		{
			key: 'post_type',
			label: __( 'Post Type', 'gohigh-seo' ),
			multiSelect: true,
			allLabel: __( 'All Types', 'gohigh-seo' ),
	...
	const footerStats = useMemo( () => {
		...
		return [
			{ label: __( 'Total Posts', 'gohigh-seo' ), value: ( stats.total_posts || 0 ).toLocaleString() },
			{ label: __( 'Orphan Posts', 'gohigh-seo' ), value: ( stats.orphan_posts || 0 ).toLocaleString() },
		]
	}, [ stats ] )
