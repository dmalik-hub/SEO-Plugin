...
const LinkTypeBadge = ( { type } ) => (
	<span className={ `rank-math-link-type-badge ${ type }` }>
		{ type === 'internal' ? __( 'Internal', 'gohigh-seo' ) : __( 'External', 'gohigh-seo' ) }
	</span>
)
...
const Links = () => {
	...
	const headers = useMemo( () => [
		{
			key: 'source_title',
			label: __( 'Source Post', 'gohigh-seo' ),
			isSortable: true,
			isHideable: false,
			render: ( value, row ) => (
				<>
		...
		{
			key: 'url',
			label: __( 'Destination', 'gohigh-seo' ),
			isSortable: true,
			isHideable: false,
			render: ( value, row ) => (
				<>
	...
	const linkTypeOptions = [
		{ label: __( 'All', 'gohigh-seo' ), value: '' },
		{ label: __( 'Internal', 'gohigh-seo' ), value: '1', statusType: 'internal' },
		{ label: __( 'External', 'gohigh-seo' ), value: '0', statusType: 'external' },
	]

	// Link status toggle options (PRO-only — clicking any non-All value opens upgrade modal)
	const linkStatusOptions = [
		{ label: __( 'All', 'gohigh-seo' ), value: '', statusType: 'all' },
		{ label: __( 'Success', 'gohigh-seo' ), value: '2xx', statusType: 'success' },
		{ label: __( 'Broken', 'gohigh-seo' ), value: 'broken', statusType: 'error' },
		{ label: __( 'Redirects', 'gohigh-seo' ), value: '3xx', statusType: 'warning' },
		{ label: __( 'Robots Blocked', 'gohigh-seo' ), value: 'robots_blocked', statusType: 'blocked' },
		{ label: __( 'Marked Safe', 'gohigh-seo' ), value: 'marked_safe', statusType: 'markedSafe' },
		{ label: __( 'Unchecked', 'gohigh-seo' ), value: 'unchecked', statusType: 'unchecked' },
	]
	...
	const footerStats = useMemo( () => {
		...
		return [
			{
				label: __( 'Total Links', 'gohigh-seo' ),
