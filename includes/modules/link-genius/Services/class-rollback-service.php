...
class Rollback_Service {
		...
		if ( 'completed' !== $batch->status ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Only completed batches can be rolled back. This batch is currently in status: %s', 'gohigh-seo' ),
					$batch->status
				),
				[ 'status' => 400 ]
			);
	...
	private function get_processor_for_source_type( $source_type ) {
		switch ( $source_type ) {
			case 'bulk_update':
				return \GoHighSEO\Link_Genius\Features\BulkUpdate\Processor::get();

			case 'keyword_map':
				return \GoHighSEO\Link_Genius\Features\KeywordMaps\Keyword_Map_Processor::get();
			...
			default:
				return new WP_Error(
					...
					sprintf(
						...
						__( 'Unknown source type: %s', 'gohigh-seo' ),
						$source_type
					),
					[ 'status' => 400 ]
				);
	...
	public function get_preview( $batch_id, $page = 1, $per_page = 20 ) {
		...
		if ( empty( $snapshots ) ) {
			return new WP_Error(
				'no_snapshots_found',
				__( 'No rollback preview available for this batch.', 'gohigh-seo' ),
				[ 'status' => 404 ]
			);
		}
	...
	public function get_batch_changes( $batch_id ) {
		...
		if ( empty( $snapshots ) ) {
			return new WP_Error(
				'no_changes_found',
				__( 'No changes found for this batch.', 'gohigh-seo' ),
				[ 'status' => 404 ]
			);
		}
