...
class History_Service {
	...
	public function delete_history( $batch_id ) {
		...
		if ( ! $exists ) {
			return new WP_Error(
				'history_not_found',
				__( 'History record not found.', 'gohigh-seo' ),
				[ 'status' => 404 ]
			);
		}
		...
		if ( false === $deleted ) {
			return new WP_Error(
				'delete_failed',
				__( 'Failed to delete history record.', 'gohigh-seo' ),
				[ 'status' => 500 ]
			);
		}
		...
		return [
			'success' => true,
			'message' => __( 'History record deleted successfully.', 'gohigh-seo' ),
		];
	}
	...
	public function bulk_delete( $batch_ids ) {
		if ( empty( $batch_ids ) || ! is_array( $batch_ids ) ) {
			return [
				'success' => false,
				'deleted' => 0,
				'message' => __( 'No batch IDs provided.', 'gohigh-seo' ),
			];
		}
		...
		return [
			...
			'message' => sprintf(
				...
				__( '%d history records deleted.', 'gohigh-seo' ),
				$deleted_count
			),
		];
	}
	...
	private function build_summary( $item, $changes_summary ) {
		switch ( $item->source_type ) {
			case 'bulk_update':
				...
				if ( $from && $to ) {
					return sprintf(
						...
						__( 'Changed <span class="rank-math-history-from">"%1$s"</span> to <span class="rank-math-history-to">"%2$s"</span> in <span class="rank-math-history-count">%3$d</span> links', 'gohigh-seo' ),
						$from,
						$to,
			...
			case 'keyword_map':
				...
				if ( $keyword ) {
					return sprintf(
						...
						__( 'Added %2$d links for keyword <span class="rank-math-history-to">"%1$s"</span>', 'gohigh-seo' ),
						$keyword,
						(int) $item->affected_links_count
					);
				}
			...
			default:
				return sprintf(
					/* translators: %d: number of links */
					__( 'Affected %d links', 'gohigh-seo' ),
					(int) $item->affected_links_count
				);
		}
	}
