...
use GoHighSEO\Link_Genius\Features\BulkUpdate\Processor as Bulk_Update_Processor;
use GoHighSEO\Link_Genius\Features\BulkUpdate\Preview_Processor;
use GoHighSEO\Link_Genius\Features\KeywordMaps\Keyword_Map_Processor;
use GoHighSEO\Link_Genius\Features\KeywordMaps\Preview_Processor as KM_Preview_Processor;
...
class Operations_Rest extends Base_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'operation' => [
						'description' => esc_html__( 'Operation type.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
						'enum'        => array_keys( self::OPERATIONS ),
					],
					'id'        => [
						'description' => esc_html__( 'Optional operation ID (for keyword maps).', 'gohigh-seo' ),
						'type'        => 'integer',
					],
				],
			]
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'operation' => [
						'description' => esc_html__( 'Operation type to cancel.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
						'enum'        => array_keys( self::OPERATIONS ),
					],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'operation' => [
						'description' => esc_html__( 'Operation type to clear.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
						'enum'        => array_keys( self::OPERATIONS ),
					],
			...
			return new WP_REST_Response(
				[
					...
					'message' => sprintf(
						...
						__( 'No %s operation in progress.', 'gohigh-seo' ),
	...
	private function perform_cancel( $operation, $id ) {
		...
		if ( 'keyword_map_preview' === $operation ) {
			if ( ! $id ) {
				return new WP_Error(
					'missing_id',
					__( 'Keyword map ID is required for preview cancellation.', 'gohigh-seo' ),
					[ 'status' => 400 ]
				);
			}
		...
		if ( ! method_exists( $processor_class, 'get' ) ) {
			return new WP_Error(
				'invalid_processor',
				__( 'Invalid processor for this operation.', 'gohigh-seo' ),
				[ 'status' => 500 ]
			);
		}
	...
	private function perform_clear( $operation, $id ) {
		...
		if ( 'keyword_map_preview' === $operation ) {
			if ( ! $id ) {
				return new WP_Error(
					'missing_id',
					__( 'Keyword map ID is required.', 'gohigh-seo' ),
					[ 'status' => 400 ]
				);
			}
			KM_Preview_Processor::get()->cancel( $id );
		...
		if ( ! method_exists( $processor_class, 'clear_completion' ) ) {
			return new WP_Error(
				'not_supported',
				__( 'Clear operation not supported for this operation type.', 'gohigh-seo' ),
				[ 'status' => 400 ]
			);
		}
	...
	private function get_operation_label( $operation ) {
		$labels = [
			'regenerate'          => __( 'link regeneration', 'gohigh-seo' ),
			'audit'               => __( 'link audit', 'gohigh-seo' ),
			'bulk_delete'         => __( 'bulk delete', 'gohigh-seo' ),
			'bulk_mark_safe'      => __( 'bulk mark safe', 'gohigh-seo' ),
			'bulk_restore'        => __( 'bulk restore', 'gohigh-seo' ),
			'bulk_update_preview' => __( 'bulk update preview', 'gohigh-seo' ),
			'bulk_update_apply'   => __( 'bulk update', 'gohigh-seo' ),
			'keyword_map_preview' => __( 'keyword map preview', 'gohigh-seo' ),
			'keyword_map_apply'   => __( 'keyword map execution', 'gohigh-seo' ),
			'export'              => __( 'export', 'gohigh-seo' ),
		];
	...
	private function get_cancel_message( $operation ) {
		$messages = [
			'regenerate'          => __( 'Link regeneration cancelled successfully.', 'gohigh-seo' ),
			'audit'               => __( 'Link audit cancelled successfully.', 'gohigh-seo' ),
			'bulk_delete'         => __( 'Bulk delete operation cancelled successfully.', 'gohigh-seo' ),
			'bulk_mark_safe'      => __( 'Bulk mark safe operation cancelled successfully.', 'gohigh-seo' ),
			'bulk_restore'        => __( 'Bulk restore operation cancelled successfully.', 'gohigh-seo' ),
			'bulk_update_preview' => __( 'Bulk update preview cancelled successfully.', 'gohigh-seo' ),
			'bulk_update_apply'   => __( 'Bulk update cancelled successfully.', 'gohigh-seo' ),
			'keyword_map_preview' => __( 'Keyword map preview cancelled successfully.', 'gohigh-seo' ),
			'keyword_map_apply'   => __( 'Keyword map execution cancelled successfully.', 'gohigh-seo' ),
			'export'              => __( 'Export cancelled successfully.', 'gohigh-seo' ),
		];

		return $messages[ $operation ] ?? __( 'Operation cancelled successfully.', 'gohigh-seo' );
	}
	...
	private function get_clear_message( $operation ) {
		$messages = [
			'regenerate'          => __( 'Link regeneration completion data cleared.', 'gohigh-seo' ),
			'audit'               => __( 'Link audit completion data cleared.', 'gohigh-seo' ),
			'bulk_delete'         => __( 'Bulk delete completion data cleared.', 'gohigh-seo' ),
			'bulk_mark_safe'      => __( 'Bulk mark safe completion data cleared.', 'gohigh-seo' ),
			'bulk_restore'        => __( 'Bulk restore completion data cleared.', 'gohigh-seo' ),
			'bulk_update_preview' => __( 'Bulk update preview data cleared.', 'gohigh-seo' ),
			'bulk_update_apply'   => __( 'Bulk update completion data cleared.', 'gohigh-seo' ),
			'keyword_map_preview' => __( 'Keyword map preview data cleared.', 'gohigh-seo' ),
			'keyword_map_apply'   => __( 'Keyword map execution completion data cleared.', 'gohigh-seo' ),
			'export'              => __( 'Export completion data cleared.', 'gohigh-seo' ),
		];

		return $messages[ $operation ] ?? __( 'Operation completion data cleared.', 'gohigh-seo' );
	}
}
