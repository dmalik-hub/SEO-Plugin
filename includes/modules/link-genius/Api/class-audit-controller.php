...
class Audit_Controller extends Base_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'filters' => [
						'description' => esc_html__( 'Filters to apply (is_internal, unchecked_only, etc.).', 'gohigh-seo' ),
						'type'        => 'object',
						'required'    => false,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'link_ids' => [
						'description' => esc_html__( 'Array of link IDs to recheck.', 'gohigh-seo' ),
						'type'        => 'array',
						'required'    => false,
					],
					'filters'  => [
						'description' => esc_html__( 'Filter criteria for selecting links to recheck.', 'gohigh-seo' ),
						'type'        => 'object',
						'required'    => false,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'link_ids' => [
						'description' => esc_html__( 'Link IDs to mark as safe.', 'gohigh-seo' ),
						'type'        => 'array',
						'required'    => false,
					],
					'reason'   => [
						'description' => esc_html__( 'Reason for marking as safe.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => false,
					],
					'filters'  => [
						'description' => esc_html__( 'Filter criteria for selecting links to mark as safe.', 'gohigh-seo' ),
						'type'        => 'object',
						'required'    => false,
					],
				],
	...
	public function start_audit( $request ) {
		...
		if ( $crawler->is_active() ) {
			return $this->error(
				...
				__( 'An audit is already in progress.', 'gohigh-seo' ),
				409
			);
		}
		...
		return new WP_REST_Response(
			[
				'success' => true,
				'message' => __( 'Audit started successfully.', 'gohigh-seo' ),
	...
	public function recheck_links( $request ) {
		...
		if ( empty( $link_ids ) && empty( $filters ) ) {
			return $this->error(
				'invalid_params',
				__( 'Either link_ids or filters must be provided.', 'gohigh-seo' ),
				400
			);
		}
		...
		if ( ! empty( $filters ) ) {
			...
			return $this->error(
				'invalid_link_ids',
				__( 'Invalid link IDs provided.', 'gohigh-seo' ),
				400
			);
		}
		...
		if ( empty( $links ) ) {
			return $this->error(
				'links_not_found',
				__( 'No links found with the provided parameters.', 'gohigh-seo' ),
				404
			);
		}
		...
		if ( $check_succeeded ) {
			...
			return new WP_REST_Response(
				[
					'success' => true,
					'message' => __( 'Link rechecked successfully.', 'gohigh-seo' ),
				],
				200
			);
		}
		...
		return new WP_REST_Response(
			[
				'success' => false,
				'message' => __( 'Failed to recheck link. The URL may be unreachable.', 'gohigh-seo' ),
			],
			200
		);
	}
	...
	public function mark_link_safe( $request ) {
		...
		if ( $is_bulk ) {
			...
			if ( $processor->is_active() ) {
				return $this->error(
					...
					__( 'A bulk mark safe operation is already in progress.', 'gohigh-seo' ),
					409
				);
			}
			...
			$processor->start(
				...
				[
					'filters'        => $filters,
					'link_ids'       => $link_ids,
					'reason'         => ! empty( $reason ) ? $reason : __( 'Bulk marked as safe', 'gohigh-seo' ),
			...
			return rest_ensure_response(
				[
					'success' => true,
					'queued'  => true,
					'message' => __( 'Bulk mark safe queued for background processing.', 'gohigh-seo' ),
				]
			);
		}
		...
		if ( $updated_count > 0 ) {
			$message = $is_marked_safe
				? __( 'Link marked as not broken successfully.', 'gohigh-seo' )
				: __( 'Link status reverted successfully.', 'gohigh-seo' );
			...
			return $this->error(
				'update_failed',
				__( 'No status record found for this link.', 'gohigh-seo' ),
				404
			);
		}
