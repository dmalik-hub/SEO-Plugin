...
namespace GoHighSEO\Link_Genius\Api;
...
use GoHighSEO\Link_Genius\Data\Link_Updater;
use GoHighSEO\Link_Genius\Services\Batch_Helper;
...
class Links_Controller extends Base_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'id' => [
						'description' => esc_html__( 'Link ID.', 'gohigh-seo' ),
						'type'        => 'integer',
						'required'    => true,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'link_ids' => [
						'description' => esc_html__( 'Link IDs to delete.', 'gohigh-seo' ),
						'type'        => 'array',
						'required'    => false,
					],
					'filters'  => [
						'description' => esc_html__( 'Filter criteria for selecting links to delete.', 'gohigh-seo' ),
						'type'        => 'object',
						'required'    => false,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'post_id'          => [
						'description' => esc_html__( 'Post ID to restore.', 'gohigh-seo' ),
						'type'        => 'integer',
						'required'    => true,
					],
					'original_content' => [
						'description' => esc_html__( 'Original post content to restore.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'link_id' => [
						'description' => esc_html__( 'Link ID to remove nofollow from.', 'gohigh-seo' ),
						'type'        => 'integer',
						'required'    => true,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'link_id'      => [
						'description' => esc_html__( 'Link ID to update.', 'gohigh-seo' ),
						'type'        => 'integer',
						'required'    => true,
					],
					'new_url'      => [
						'description' => esc_html__( 'New URL for the link.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
					],
					'new_anchor'   => [
						'description' => esc_html__( 'New anchor text for the link.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
					],
					'is_nofollow'  => [
						'description' => esc_html__( 'Whether the link should have nofollow attribute.', 'gohigh-seo' ),
						'type'        => 'integer',
						'required'    => false,
						'default'     => 0,
					],
					'target_blank' => [
						'description' => esc_html__( 'Whether the link should open in a new tab.', 'gohigh-seo' ),
						'type'        => 'integer',
						'required'    => false,
						'default'     => 0,
					],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'batch_id' => [
						'description' => esc_html__( 'Batch ID to restore from snapshots.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
					],
				],
	...
	public function get_link_details( $request ) {
		...
		if ( $link_id <= 0 ) {
			return $this->error( 'invalid_link_id', __( 'Invalid link ID.', 'gohigh-seo' ), 400 );
		}
		...
		if ( ! $link ) {
			return $this->error( 'link_not_found', __( 'Link not found.', 'gohigh-seo' ), 404 );
		}
	...
	public function delete_links( $request ) {
		...
		if ( ! empty( $link_ids ) ) {
			...
			return $this->error( 'invalid_params', __( 'No link ID or filters provided.', 'gohigh-seo' ), 400 );
		}
		...
		if ( ( ! empty( $link_ids ) && count( $link_ids ) > 1 ) || ! empty( $filters ) ) {
			...
			if ( $processor->is_active() ) {
				return $this->error(
					'delete_in_progress',
					__( 'A bulk delete operation is already in progress.', 'gohigh-seo' ),
					409
				);
			}
			...
			return rest_ensure_response(
				[
					'success' => true,
					'queued'  => true,
					'message' => __( 'Bulk delete queued for background processing.', 'gohigh-seo' ),
				]
			);
		}
		...
		if ( empty( $link_ids ) ) {
			return $this->error( 'invalid_params', __( 'No link ID provided.', 'gohigh-seo' ), 400 );
		}
	...
	public function undo_delete_link( $request ) {
		...
		if ( empty( $posts[ $post_id ] ) ) {
			return $this->error( 'post_not_found', __( 'Post not found.', 'gohigh-seo' ), 404 );
		}
		...
		if ( ! $result ) {
			return $this->error( 'restore_failed', __( 'Failed to restore post content.', 'gohigh-seo' ), 500 );
		}
		...
		return rest_ensure_response(
			[
				'success' => true,
				'message' => __( 'Link restored successfully.', 'gohigh-seo' ),
			]
		);
	}
	...
	public function remove_nofollow( $request ) {
		...
		if ( empty( $link->is_nofollow ) || 0 === (int) $link->is_nofollow ) {
			return $this->error( 'not_nofollow', __( 'Link is not set to nofollow.', 'gohigh-seo' ), 400 );
		}

		if ( empty( $link->url ) || ( empty( $link->anchor_text ) && 'IMAGE' !== $link->anchor_type ) ) {
			return $this->error( 'invalid_link_data', __( 'Link data is incomplete.', 'gohigh-seo' ), 400 );
		}
		...
		if ( null === $new_content ) {
			return $this->error(
				...
				__( 'Could not find the link in post content. It may have been modified or deleted.', 'gohigh-seo' ),
				404
			);
		}
		...
		return rest_ensure_response(
			[
				'success' => true,
				'message' => __( 'Nofollow attribute removed successfully.', 'gohigh-seo' ),
			]
		);
	}
	...
	public function update_link( $request ) {
		...
		if ( empty( $new_url ) ) {
			return $this->error( 'invalid_params', __( 'URL cannot be empty.', 'gohigh-seo' ), 400 );
		}
		...
		if ( empty( $new_anchor ) && 'IMAGE' !== $link->anchor_type ) {
			return $this->error( 'invalid_params', __( 'Anchor text cannot be empty.', 'gohigh-seo' ), 400 );
		}
		...
		return rest_ensure_response(
			[
				'success'     => true,
				'message'     => __( 'Link updated successfully.', 'gohigh-seo' ),
	...
	public function restore_bulk_delete( $request ) {
		...
		return new WP_REST_Response(
			[
				'success' => true,
				'message' => __( 'Restore started. Processing in background.', 'gohigh-seo' ),
			],
			200
		);
	}
	...
	private function delete_single_link( $link_id ) {
		...
		if ( ! $pattern ) {
			return $this->error( 'pattern_failed', __( 'Failed to build delete pattern for the link.', 'gohigh-seo' ), 500 );
		}
		...
		if ( $new_content === $original_content ) {
			return $this->error( 'link_not_found_in_content', __( 'Link not found in post content.', 'gohigh-seo' ), 404 );
		}
		...
		return rest_ensure_response(
			[
				...
				'failed_count'  => 0,
				'total_count'   => 1,
				'message'       => __( 'Link deleted successfully.', 'gohigh-seo' ),
				'undo_data'     => [
	...
	private function get_links_args() {
		return [
			'page'            => [
				'description' => esc_html__( 'Page number.', 'gohigh-seo' ),
				'type'        => 'integer',
				'default'     => 1,
			],
			'per_page'        => [
				'description' => esc_html__( 'Items per page.', 'gohigh-seo' ),
				'type'        => 'integer',
				'default'     => 50,
				'maximum'     => 100,
			],
			'source_id'       => [
				'description' => esc_html__( 'Filter by source post ID.', 'gohigh-seo' ),
				'type'        => 'integer',
				'default'     => 0,
			],
			'target_post_id'  => [
				'description' => esc_html__( 'Filter by target post ID.', 'gohigh-seo' ),
				'type'        => 'integer',
				'default'     => 0,
			],
			'is_internal'     => [
				'description' => esc_html__( 'Filter by link type.', 'gohigh-seo' ),
				'type'        => 'string',
				'default'     => '',
			],
			'is_nofollow'     => [
				'description' => esc_html__( 'Filter by nofollow status.', 'gohigh-seo' ),
				'type'        => 'string',
				'default'     => '',
			],
			'anchor_type'     => [
				'description' => esc_html__( 'Filter by anchor type (HPLNK, CNCL, HLANG, IMAGE).', 'gohigh-seo' ),
				'type'        => 'string',
				'default'     => '',
			],
			'target_blank'    => [
				'description' => esc_html__( 'Filter by target blank status.', 'gohigh-seo' ),
				'type'        => 'string',
				'default'     => '',
			],
			'search'          => [
				'description' => esc_html__( 'Search in anchor text or URL.', 'gohigh-seo' ),
				'type'        => 'string',
				'default'     => '',
			],
			'orderby'         => [
				'description' => esc_html__( 'Order by field.', 'gohigh-seo' ),
				'type'        => 'string',
				'default'     => 'created_at',
			],
			'order'           => [
				'description' => esc_html__( 'Order direction.', 'gohigh-seo' ),
				'type'        => 'string',
				'default'     => 'DESC',
			],
			'status_code'     => [
				'description' => esc_html__( 'Filter by HTTP status code (e.g., 404). Supports multiple values.', 'gohigh-seo' ),
				'type'        => 'array',
				'items'       => [ 'type' => 'string' ],
				'default'     => [],
			],
			'status_category' => [
				'description' => esc_html__( 'Filter by status category (2xx, 3xx, 4xx, 5xx, timeout, error).', 'gohigh-seo' ),
				'type'        => 'string',
				'default'     => '',
			],
			'is_broken'       => [
				'description' => esc_html__( 'Filter broken links (4xx, 5xx, timeout, error).', 'gohigh-seo' ),
				'type'        => 'boolean',
				'default'     => false,
			],
			'is_success'      => [
				'description' => esc_html__( 'Filter successful links (2xx).', 'gohigh-seo' ),
				'type'        => 'boolean',
				'default'     => false,
			],
			'is_redirect'     => [
				'description' => esc_html__( 'Filter redirect links (3xx).', 'gohigh-seo' ),
				'type'        => 'string',
				'default'     => '',
			],
			'robots_blocked'  => [
				'description' => esc_html__( 'Filter links blocked by robots.txt.', 'gohigh-seo' ),
				'type'        => 'string',
				'default'     => '',
			],
			'unchecked'       => [
				'description' => esc_html__( 'Filter unchecked links (no status data).', 'gohigh-seo' ),
				'type'        => 'string',
				'default'     => '',
			],
			'is_marked_safe'  => [
				'description' => esc_html__( 'Filter links marked as safe.', 'gohigh-seo' ),
				'type'        => 'string',
				'default'     => '',
			],
		];
