...
class Export_Controller extends Base_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'type'    => [
						'description' => esc_html__( 'Export type (links or posts).', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
					],
					'format'  => [
						'description' => esc_html__( 'Export format (csv, json, excel).', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
					],
					'filters' => [
						'description' => esc_html__( 'Filters to apply to the export.', 'gohigh-seo' ),
						'type'        => 'object',
						'required'    => false,
					],
					'columns' => [
						'description' => esc_html__( 'Column definitions.', 'gohigh-seo' ),
						'type'        => 'array',
						'required'    => true,
					],
					'total'   => [
						'description' => esc_html__( 'Total number of records to export.', 'gohigh-seo' ),
						'type'        => 'integer',
						'required'    => true,
					],
				],
	...
	public function schedule_export( $request ) {
		...
		if ( ! in_array( $type, [ 'links', 'posts' ], true ) ) {
			return $this->error( 'invalid_type', __( 'Invalid export type.', 'gohigh-seo' ), 400 );
		}

		// Validate format.
		if ( ! in_array( $format, [ 'csv', 'json', 'excel' ], true ) ) {
			return $this->error( 'invalid_format', __( 'Invalid export format.', 'gohigh-seo' ), 400 );
		}
