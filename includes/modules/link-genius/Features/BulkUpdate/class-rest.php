...
class Rest extends Base_Operation_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'filters'           => [
						'description' => esc_html__( 'Search filters for finding links.', 'gohigh-seo' ),
						'type'        => 'object',
						'required'    => true,
					],
					'update_config'     => [
						'description' => esc_html__( 'Update configuration.', 'gohigh-seo' ),
						'type'        => 'object',
						'required'    => true,
					],
					'selected_link_ids' => [
						'description' => esc_html__( 'Optional array of specific link IDs to update.', 'gohigh-seo' ),
						'type'        => 'array',
						'required'    => false,
						'items'       => [
							'type' => 'integer',
					...
					'is_rollback'       => [
						'description' => esc_html__( 'Whether this is a rollback operation (prevents history creation).', 'gohigh-seo' ),
						'type'        => 'boolean',
						'required'    => false,
						'default'     => false,
					],
					'rollback_batch_id' => [
						'description' => esc_html__( 'Batch ID being rolled back (for status update).', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => false,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'batch_id'          => [
						'description' => esc_html__( 'Batch ID to rollback.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
					],
					'selected_post_ids' => [
						'description' => esc_html__( 'Optional array of specific post IDs to rollback.', 'gohigh-seo' ),
						'type'        => 'array',
						'required'    => false,
						'items'       => [
							'type' => 'integer',
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'batch_id' => [
						'description' => esc_html__( 'Batch ID.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
					],
				],
	...
	public function get_preview_results( $request ) {
		...
		if ( ! $progress ) {
			return new WP_Error(
				'preview_not_found',
				__( 'No preview found.', 'gohigh-seo' ),
				[ 'status' => 404 ]
			);
		}
		...
		if ( 'completed' !== $progress['status'] ) {
			return new WP_Error(
				...
				__( 'Preview generation is still in progress.', 'gohigh-seo' ),
				[ 'status' => 400 ]
			);
		}
