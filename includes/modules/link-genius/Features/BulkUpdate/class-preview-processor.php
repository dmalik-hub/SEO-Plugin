...
class Preview_Processor extends \WP_Background_Process {
	...
	public function start( $filters, $update_config ) {
		...
		if ( $existing_progress && 'processing' === $existing_progress['status'] ) {
			return new WP_Error(
				'preview_in_progress',
				__( 'A preview generation is already in progress. Please wait for it to complete or cancel it first.', 'gohigh-seo' ),
				[ 'status' => 409 ]
			);
		}
		...
		return [
			...
			'preview_id'  => 'current',
			'total_links' => $total_links,
			'message'     => __( 'Preview generation started. Processing in background.', 'gohigh-seo' ),
		];
	}
