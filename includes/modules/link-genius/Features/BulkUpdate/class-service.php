...
namespace GoHighSEO\Link_Genius\Features\BulkUpdate;

use WP_Error;
use GoHighSEO\Helpers\DB;
use GoHighSEO\Link_Genius\Data\Query_Builder;
use GoHighSEO\Link_Genius\Features\KeywordMaps\Utils\Content_Analyzer;
use GoHighSEO\Link_Genius\Services\Batch_Helper;
use GoHighSEO\Link_Genius\Services\Link_Processor;
use GoHighSEO\Link_Genius\Services\History_Service;
use GoHighSEO\Link_Genius\Services\Rollback_Service;
use GoHighSEO\Link_Genius\Services\Utils;
	...
	public function start_bulk_update( $filters, $update_config, $selected_link_ids = null, $is_rollback = false, $rollback_batch_id = null ) {
		...
		if ( ! $is_rollback ) {
			...
			if ( empty( $affected_links ) ) {
				return new WP_Error(
					'no_links_found',
					__( 'No links found matching the specified filters.', 'gohigh-seo' ),
					[ 'status' => 404 ]
				);
			}
		...
		if ( ! $is_rollback && $total_links > self::MAX_LINKS_PER_BATCH ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Cannot update more than %d links at once. Please refine your filters.', 'gohigh-seo' ),
		...
		return [
			...
			'message'       => $is_rollback
				? __( 'Rollback started. Processing in background.', 'gohigh-seo' )
				: __( 'Bulk update started. Processing in background.', 'gohigh-seo' ),
		];
	}
	...
	private function validate_update_config( $config ) {
		...
		if ( empty( $config['operation_type'] ) || ! in_array( $config['operation_type'], [ 'anchor', 'url', 'both' ], true ) ) {
			return new WP_Error(
				...
				__( 'Invalid operation type. Must be "anchor", "url", or "both".', 'gohigh-seo' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( in_array( $config['operation_type'], [ 'anchor', 'both' ], true ) ) {
			if ( empty( $config['anchor_update']['search_value'] ) ) {
				return new WP_Error(
					...
					__( 'Anchor search value is required.', 'gohigh-seo' ),
					[ 'status' => 400 ]
				);
			}
			...
			if ( isset( $config['anchor_update']['search_type'] ) && 'regex' === $config['anchor_update']['search_type'] ) {
				...
				if ( preg_match( $pattern, '' ) === false && PREG_NO_ERROR !== preg_last_error() ) {
					return new WP_Error(
						'invalid_regex',
						__( 'Invalid regex pattern for anchor search.', 'gohigh-seo' ),
						[ 'status' => 400 ]
					);
				}
			}
		...
		if ( in_array( $config['operation_type'], [ 'url', 'both' ], true ) ) {
			if ( empty( $config['url_update']['search_value'] ) ) {
				return new WP_Error(
					'missing_url_search',
					__( 'URL search value is required.', 'gohigh-seo' ),
					[ 'status' => 400 ]
				);
			}
			...
			if ( isset( $config['url_update']['search_type'] ) && 'exact' === $config['url_update']['search_type'] ) {
				...
				if ( ! filter_var( $url, FILTER_VALIDATE_URL ) ) {
					return new WP_Error(
						'invalid_url_format',
						__( 'Invalid URL format for exact URL search.', 'gohigh-seo' ),
						[ 'status' => 400 ]
					);
				}
			}
