...
use GoHighSEO\Link_Genius\Background\Process_Coordinator;
use GoHighSEO\Link_Genius\Background\Processor_Utils;
...
class Processor extends \WP_Background_Process {
	...
	use \GoHighSEO\Link_Genius\Background\Background_Processor_Error_Handler;
	use \GoHighSEO\Link_Genius\Background\Progress_Helper;
	...
	public function start( $filters, $update_config, $link_ids = null, $is_rollback = false, $rollback_batch_id = null ) {
		...
		return [
			...
			'total_posts' => $total_posts,
			'total_links' => $total_links,
			'message'     => __( 'Bulk update scheduled. Processing in background.', 'gohigh-seo' ),
		];
	}
	...
	private function queue_affected_links_for_crawler( $post_ids ) {
		...
		if ( ! empty( $links ) ) {
			...
			\GoHighSEO\Link_Genius\Background\Link_Status_Crawler::queue_links_batch( $links );
		}
	}
	...
	public function cancel() {
		...
		$wpdb->update(
			...
			[
				'status'        => 'failed',
				'error_message' => __( 'Cancelled by user', 'gohigh-seo' ),
