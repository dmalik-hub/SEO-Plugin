	...
	public function start( $keyword_map, $selected_items = null, $preview_results = null, $is_rollback = false, $rollback_batch_id = null ) {
		...
		if ( $is_rollback && $rollback_batch_id ) {
			...
			if ( empty( $preview_results ) || ! is_array( $preview_results ) ) {
				return new WP_Error(
					'no_preview_data',
					__( 'Preview data is required to execute keyword map.', 'gohigh-seo' ),
					[ 'status' => 400 ]
				);
			}
			...
			if ( empty( $all_changes ) ) {
				return new WP_Error(
					'no_changes_found',
					__( 'No changes found to apply.', 'gohigh-seo' )
				);
			}
		...
		$message = $is_rollback
			...
			: __( 'Keyword map execution started. Processing in background.', 'gohigh-seo' );
