...
use GoHighSEO\Link_Genius\Features\KeywordMaps\Utils\Link_Applier;
...
class Executor {
	...
	public function preview( $keyword_map ) {
		...
		if ( empty( $variations ) ) {
			return new \WP_Error(
				'no_keywords',
				__( 'No keywords or variations defined for this keyword map.', 'gohigh-seo' )
			);
		}
		...
		if ( empty( $posts ) ) {
			return [
				...
				'sample_changes' => [],
				'warnings'       => [],
				'message'        => __( 'No posts found containing the specified keywords.', 'gohigh-seo' ),
			];
		}
		...
		if ( 0 === $total_links ) {
			return [
				...
				'sample_changes' => [],
				'warnings'       => [],
				'message'        => __( 'No new link opportunities found. All keyword matches are already linked or in content that cannot be modified.', 'gohigh-seo' ),
			];
		}
