...
class AI_Client {
	...
	public function generate_keyword_variations( $keyword, $context = '' ) {
		...
		if ( empty( $this->registered_data['username'] ) || empty( $this->registered_data['api_key'] ) ) {
			return new WP_Error(
				'not_registered',
				__( 'Please connect your Rank Math account to use AI features.', 'gohigh-seo' )
			);
		}
		...
		if ( empty( $keyword ) ) {
			return new WP_Error(
				'empty_keyword',
				__( 'Keyword cannot be empty.', 'gohigh-seo' )
			);
		}
		...
		if ( is_wp_error( $response ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'API request failed: %s', 'gohigh-seo' ),
		...
		if ( 429 === $status_code ) {
			return new WP_Error(
				'rate_limited',
				__( 'Rate limit exceeded. Please try again later.', 'gohigh-seo' )
			);
		}
		...
		if ( 401 === $status_code || 403 === $status_code ) {
			return new WP_Error(
				'auth_failed',
				__( 'Authentication failed. Please check your Rank Math account connection.', 'gohigh-seo' )
			);
		}
		...
		if ( 402 === $status_code ) {
			return new WP_Error(
				...
				__( 'Insufficient Content AI credits. Please upgrade your plan.', 'gohigh-seo' )
			);
		}

		// Handle other errors.
		if ( $status_code < 200 || $status_code >= 300 ) {
			$error_message = isset( $data['message'] ) ? $data['message'] : __( 'Unknown API error.', 'gohigh-seo' );
			return new WP_Error(
				'api_error',
				$error_message
			);
		...
		if ( ! $has_valid_category ) {
			return new WP_Error(
				'invalid_response',
				__( 'Invalid API response format.', 'gohigh-seo' )
			);
		}
