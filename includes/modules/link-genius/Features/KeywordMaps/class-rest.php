...
namespace GoHighSEO\Link_Genius\Features\KeywordMaps;
...
use GoHighSEO\Rest\Rest_Helper;
use GoHighSEO\Link_Genius\Api\Base_Operation_Controller;
use GoHighSEO\Link_Genius\Services\History_Service;
use GoHighSEO\Link_Genius\Features\KeywordMaps\Utils\Variation_Expander;
...
class Rest extends Base_Operation_Controller {
	...
	public function register_routes() {
		...
		$this->register_preview_endpoints(
			...
			[
				...
				'results_args'     => [
					'keyword_map_id' => [
						'description' => esc_html__( 'Keyword map ID.', 'gohigh-seo' ),
						'type'        => 'integer',
						'required'    => true,
					],
				],
			...
			[
				[
					...
					'args'                => [
						...
						'per_page' => [
							'description' => esc_html__( 'Number of items per page.', 'gohigh-seo' ),
							'type'        => 'integer',
							'default'     => 20,
							'minimum'     => 1,
							'maximum'     => 100,
						...
						'orderby'  => [
							'description' => esc_html__( 'Column to order by.', 'gohigh-seo' ),
							'type'        => 'string',
							'default'     => 'created_at',
						...
						'enabled'  => [
							'description' => esc_html__( 'Filter by enabled status.', 'gohigh-seo' ),
							'type'        => 'boolean',
						],
						'search'   => [
							'description' => esc_html__( 'Search term.', 'gohigh-seo' ),
							'type'        => 'string',
						],
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'ids'     => [
						'description' => esc_html__( 'Array of keyword map IDs.', 'gohigh-seo' ),
						'type'        => 'array',
						'required'    => true,
						'items'       => [ 'type' => 'integer' ],
					],
					'enabled' => [
						'description' => esc_html__( 'New enabled status.', 'gohigh-seo' ),
						'type'        => 'boolean',
						'required'    => true,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				[
					...
					'args'                => [
						'variation' => [
							'description' => esc_html__( 'Variation text.', 'gohigh-seo' ),
							'type'        => 'string',
							'required'    => true,
						],
						'source'    => [
							'description' => esc_html__( 'Variation source.', 'gohigh-seo' ),
							'type'        => 'string',
							'default'     => 'manual',
							'enum'        => [ 'manual', 'ai' ],
						],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'keyword' => [
						'description' => esc_html__( 'Keyword to generate variations for.', 'gohigh-seo' ),
						'type'        => 'string',
						'required'    => true,
					],
					'context' => [
						'description' => esc_html__( 'Optional context to help generate better variations.', 'gohigh-seo' ),
						'type'        => 'string',
					],
				],
			]
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'variations' => [
						'description' => esc_html__( 'Array of variations to accept.', 'gohigh-seo' ),
						'type'        => 'array',
						'required'    => true,
					],
				],
	...
	private function get_keyword_map_args( $required = true ) {
		return [
			'name'                 => [
				'description' => esc_html__( 'Keyword map name.', 'gohigh-seo' ),
				'type'        => 'string',
				'required'    => $required,
			],
			'description'          => [
				'description' => esc_html__( 'Keyword map description.', 'gohigh-seo' ),
				'type'        => 'string',
			],
			'target_url'           => [
				'description' => esc_html__( 'Target URL.', 'gohigh-seo' ),
				'type'        => 'string',
				'required'    => $required,
			],
			'is_enabled'           => [
				'description' => esc_html__( 'Whether the keyword map is enabled.', 'gohigh-seo' ),
				'type'        => 'boolean',
				'default'     => true,
			],
			'max_links_per_post'   => [
				'description' => esc_html__( 'Maximum links per post.', 'gohigh-seo' ),
				'type'        => 'integer',
				'default'     => 3,
				'minimum'     => 1,
			],
			'auto_link_on_publish' => [
				'description' => esc_html__( 'Enable auto-linking on publish.', 'gohigh-seo' ),
				'type'        => 'boolean',
				'default'     => false,
			],
			'case_sensitive'       => [
				'description' => esc_html__( 'Enable case-sensitive keyword matching.', 'gohigh-seo' ),
				'type'        => 'boolean',
				'default'     => false,
			],
		];
	...
	public function get_keyword_map( $request ) {
		...
		if ( ! $map ) {
			return new WP_Error(
				'not_found',
				__( 'Keyword map not found.', 'gohigh-seo' ),
				[ 'status' => 404 ]
			);
		}
	...
	public function create_keyword_map( $request ) {
		...
		if ( ! $id ) {
			return new WP_Error(
				'create_failed',
				__( 'Failed to create keyword map.', 'gohigh-seo' ),
				[ 'status' => 500 ]
			);
		}
		...
		$message = $this->build_success_message(
			__( 'Keyword map created successfully.', 'gohigh-seo' ),
			$skipped_variations
		);
	...
	public function update_keyword_map( $request ) {
		...
		if ( ! $result ) {
			return new WP_Error(
				'update_failed',
				__( 'Failed to update keyword map.', 'gohigh-seo' ),
				[ 'status' => 500 ]
			);
		}
		...
		return new WP_REST_Response(
			[
				...
				'message' => __( 'Keyword map updated successfully.', 'gohigh-seo' ),
			],
			200
		);
	}
	...
	private function validate_keyword_map_name( $name, $exclude_id = null ) {
		...
		if ( $duplicate_name ) {
			return new WP_Error(
				'duplicate_name',
				__( 'A keyword map with this name already exists. Please use a different name.', 'gohigh-seo' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( $duplicate_keyword ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'This keyword already exists in the keyword map "%s". Consider adding your variations there instead.', 'gohigh-seo' ),
	...
	private function build_success_message( $base_message, $skipped_variations ) {
		...
		return sprintf(
			...
			__( '%1$s The following variations were skipped because they already exist in other keyword maps: %2$s', 'gohigh-seo' ),
			$base_message,
	...
	public function delete_keyword_map( $request ) {
		...
		if ( ! $result ) {
			return new WP_Error(
				'delete_failed',
				__( 'Failed to delete keyword map.', 'gohigh-seo' ),
				[ 'status' => 500 ]
			);
		}
		...
		return new WP_REST_Response(
			[
				'success' => true,
				'message' => __( 'Keyword map deleted successfully.', 'gohigh-seo' ),
			],
			200
		);
	}
		...
		return new WP_REST_Response(
			[
				...
				'message' => sprintf(
					...
					__( '%d keyword maps updated.', 'gohigh-seo' ),
					$updated
				),
			],
			200
		...
		return new WP_REST_Response(
			[
				...
				'message' => sprintf(
					...
					__( '%d keyword maps deleted.', 'gohigh-seo' ),
					$deleted
				),
			],
			200
	...
	public function execute_keyword_map( $request ) {
		...
		if ( ! $is_rollback && ! $map->is_enabled ) {
			return new WP_Error(
				'disabled',
				__( 'This keyword map is disabled. Enable it first to execute.', 'gohigh-seo' ),
				[ 'status' => 400 ]
			);
		}
	...
	public function preview_keyword_map( $request ) {
		...
		if ( ! $map->is_enabled ) {
			return new WP_Error(
				'disabled',
				__( 'This keyword map is disabled. Enable it first to preview.', 'gohigh-seo' ),
				[ 'status' => 400 ]
			);
		}
	...
	public function add_variation( $request ) {
		...
		if ( $this->storage->variation_exists( $id, $variation ) ) {
			return new WP_Error(
				'duplicate_variation',
				__( 'This variation already exists in this keyword map.', 'gohigh-seo' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( $duplicate_keyword ) {
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: existing keyword map name */
					__( 'This keyword already exists in the keyword map "%s".', 'gohigh-seo' ),
					$duplicate_keyword['keyword_map']->name
				),
				[
					'status'          => 400,
		...
		if ( ! $variation_id ) {
			return new WP_Error(
				'add_failed',
				__( 'Failed to add variation.', 'gohigh-seo' ),
				[ 'status' => 500 ]
			);
		}
		...
		return new WP_REST_Response(
			[
				...
				'variations'   => $variations,
				'message'      => __( 'Variation added successfully.', 'gohigh-seo' ),
			],
			201
		);
	}
	...
	public function delete_variation( $request ) {
		...
		if ( ! $result ) {
			return new WP_Error(
				'delete_failed',
				__( 'Failed to delete variation.', 'gohigh-seo' ),
				[ 'status' => 500 ]
			);
		}
		...
		return new WP_REST_Response(
			[
				'success' => true,
				'message' => __( 'Variation deleted successfully.', 'gohigh-seo' ),
			],
			200
		);
	}
	...
	public function generate_variations( $request ) {
		...
		if ( ! $plan || $plan === 'free' ) {
			return new WP_Error(
				'upgrade_required',
				esc_html__( 'This feature is only available for Content AI subscribers.', 'gohigh-seo' ),
				[ 'status' => 426 ]
			);
		}
		...
		$total_suggestions = 0;
		$category_labels   = [
			'synonyms'             => __( 'Synonyms', 'gohigh-seo' ),
			'related_phrases'      => __( 'Related Phrases', 'gohigh-seo' ),
			'long_tail_variations' => __( 'Long-tail Variations', 'gohigh-seo' ),
			'common_misspellings'  => __( 'Common Misspellings', 'gohigh-seo' ),
		];
		...
		return new WP_REST_Response(
			[
				...
				'keyword'     => $keyword,
				'message'     => __( 'AI variations generated. Review and accept the ones you want to add.', 'gohigh-seo' ),
			],
			200
		);
	}
	...
	public function generate_variations_by_keyword( $request ) {
		...
		if ( empty( $keyword ) ) {
			return new WP_Error(
				'missing_keyword',
				__( 'Keyword is required.', 'gohigh-seo' ),
				[ 'status' => 400 ]
			);
		}
	...
	public function accept_variations( $request ) {
		...
		if ( empty( $variations ) || ! is_array( $variations ) ) {
			return new WP_Error(
				'invalid_variations',
				__( 'No variations provided.', 'gohigh-seo' ),
				[ 'status' => 400 ]
			);
		}
		...
		return new WP_REST_Response(
			[
				...
				'message'    => sprintf(
					...
					__( '%d variations added.', 'gohigh-seo' ),
					$added
				),
			],
			200
	...
	public function preview( $request ) {
		return new WP_Error(
			'not_implemented',
			__( 'Use preview_keyword_map endpoint instead.', 'gohigh-seo' ),
			[ 'status' => 400 ]
		);
	}
	...
	public function delete_history( $request ) {
		return new WP_Error(
			'not_implemented',
			__( 'Keyword maps do not support history deletion.', 'gohigh-seo' ),
			[ 'status' => 400 ]
		);
	}
}
