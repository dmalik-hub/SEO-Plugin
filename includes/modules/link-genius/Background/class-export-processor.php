...
class Export_Processor extends \WP_Background_Process {
	...
	public function start( $type, $format, $filters, $columns, $total_count ) {
		...
		Helper::add_notification(
			sprintf(
				...
				__( 'Export started for %s records. It might take a few minutes to complete.', 'gohigh-seo' ),
		...
		return [
			'success' => true,
			'job_id'  => $job_id,
			'message' => __( 'Export scheduled. You will receive an email when it\'s ready.', 'gohigh-seo' ),
		];
	}
		...
		foreach ( $jobs as $option_name ) {
			...
			Helper::add_notification(
				sprintf(
					...
					__( 'Export completed: %1$s records. %2$s', 'gohigh-seo' ),
					number_format_i18n( $job['processed'] ),
					'<a href="' . esc_url( $job['file_url'] ) . '" target="_blank">' . __( 'Download', 'gohigh-seo' ) . '</a>'
				),
				[
					'type'    => 'success',
	...
	private static function send_export_email( $job ) {
		$to      = $job['user_email'];
		$subject = __( 'Rank Math: Your export is ready', 'gohigh-seo' );
		...
		$message = sprintf(
			...
			__( 'Your Link Genius export is ready for download.%1$s%1$sTotal records: %2$d%1$s%1$sDownload link: %3$s%1$s%1$sNote: This link will expire in 24 hours.', 'gohigh-seo' ),
			"\n",
			$job['total_count'],
			$job['file_url']
		);
