...
namespace GoHighSEO\Link_Genius\Background;
...
class Bulk_Link_Modifier extends \WP_Background_Process {
	...
	public function start( $operation, $params = [] ) {
		...
		if ( ! in_array( $operation, [ 'delete', 'mark_safe', 'restore' ], true ) ) {
			return new WP_Error(
				'invalid_operation',
				__( 'Invalid operation type.', 'gohigh-seo' ),
				[ 'status' => 400 ]
			);
		}
	...
	private function start_restore( $params ) {
		...
		if ( empty( $batch_id ) ) {
			return new WP_Error(
				'missing_batch_id',
				__( 'Batch ID is required for restore operation.', 'gohigh-seo' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( 'delete' !== $batch->source_type ) {
			return new WP_Error(
				'invalid_batch_type',
				__( 'This batch is not a delete operation.', 'gohigh-seo' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( 'completed' !== $batch->status ) {
			return new WP_Error(
				...
				__( 'Only completed delete operations can be restored.', 'gohigh-seo' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( empty( $post_ids ) ) {
			return new WP_Error(
				'no_snapshots_found',
				__( 'No snapshots found for this batch. Restore not possible.', 'gohigh-seo' ),
				[ 'status' => 404 ]
			);
		}
