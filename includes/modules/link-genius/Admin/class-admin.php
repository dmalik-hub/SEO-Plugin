...
namespace GoHighSEO\Link_Genius\Admin;
...
use GoHighSEO\Link_Genius\Features\BulkUpdate\Processor;
...
class Admin {
	...
	public function update_links_menu_title() {
		...
		$new_label = '<span class="rank-math-new-label" style="color:#ed5e5e;font-size:10px;font-weight:normal;">' . esc_html__( 'New!', 'gohigh-seo' ) . '</span>';
		foreach ( $submenu['rank-math'] as &$item ) {
			...
			$item[0] = sprintf( esc_html__( 'AI Link Genius %s', 'gohigh-seo' ), $new_label );
			$item[3] = esc_html__( 'AI Link Genius', 'gohigh-seo' );
