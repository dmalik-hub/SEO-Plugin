...
class Video_Sitemap {
	...
	public function add_settings( $tabs ) {
		...
		$tabs['video-sitemap'] = [
			...
			'desc'      => wp_kses_post( sprintf( __( 'Video Sitemaps give search engines information about video content on your site. More information: <a href="%s" target="_blank">Video Sitemaps</a>', 'gohigh-seo' ), KB::get( 'video-sitemap', 'Options Panel Sitemap Video' ) ) ),
			/* translators: Video Sitemap Url */
			'after_row' => '<div class="notice notice-alt notice-info info inline rank-math-notice"><p>' . sprintf( esc_html__( 'Your Video Sitemap index can be found here: %s', 'gohigh-seo' ), '<a href="' . $sitemap_url . '" target="_blank">' . $sitemap_url . '</a>' ) . '</p></div>',
			'json'      => [
	...
	public function add_provider( $providers ) {
		$providers[] = new \GoHighSEO\Sitemap\Video_Provider();
