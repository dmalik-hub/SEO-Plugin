...
namespace GoHighSEO\Sitemap;

use GoHighSEO\KB;
use GoHighSEO\Helper;
use GoHighSEO\Traits\Ajax;
use GoHighSEO\Module\Base;
use GoHighSEO\Admin\Register_Options_Page;
...
class Admin extends Base {
	...
	public function register_setting_page() {
		...
		$tabs = [
			'general' => [
				...
				'title'     => esc_html__( 'General', 'gohigh-seo' ),
				'file'      => $this->directory . '/settings/general.php',
				'desc'      => esc_html__( 'This tab contains General settings related to the XML sitemaps.', 'gohigh-seo' ) . ' <a href="' . KB::get( 'sitemap-general', 'Options Panel Sitemap General Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>',
				/* translators: sitemap url */
				'after_row' => $this->get_notice_start() . sprintf( esc_html__( 'Your sitemap index can be found here: %s', 'gohigh-seo' ), '<a href="' . $sitemap_url . '" target="_blank">' . $sitemap_url . '</a>' ) . '</p></div>' . $this->get_nginx_notice(),
				'json'      => [
		...
		$tabs['html_sitemap'] = [
			...
			'title'   => esc_html__( 'HTML Sitemap', 'gohigh-seo' ),
			'file'    => $this->directory . '/settings/html-sitemap.php',
			'desc'    => esc_html__( 'This tab contains settings related to the HTML sitemap.', 'gohigh-seo' ) . ' <a href="' . KB::get( 'sitemap-general', 'Options Panel Sitemap HTML Tab' ) . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>',
		...
		if ( Helper::is_author_archive_indexable() ) {
			...
			$tabs['authors'] = [
				...
				'desc'  => sprintf( esc_html__( 'Set the sitemap options for author archive pages. %s.', 'gohigh-seo' ), '<a href="' . KB::get( 'configure-sitemaps', 'Options Panel Sitemap Authors Tab' ) . '#authors" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>' ),
		...
		new Register_Options_Page(
			[
				...
				'title'      => esc_html__( 'Sitemap Settings', 'gohigh-seo' ),
				'menu_title' => esc_html__( 'Sitemap Settings', 'gohigh-seo' ),
	...
	public function post_type_settings( $tabs ) {
		...
		$things = [
			'attachment' => esc_html__( 'attachments', 'gohigh-seo' ),
			'product'    => esc_html__( 'your product pages', 'gohigh-seo' ),
		];
		$urls   = [
		...
		foreach ( Helper::get_accessible_post_types() as $post_type ) {
			...
			$thing = isset( $things[ $post_type ] ) ? $things[ $post_type ] : sprintf( __( 'single %s', 'gohigh-seo' ), $name );
			$url   = isset( $urls[ $post_type ] ) ? $urls[ $post_type ] : KB::get( 'configure-sitemaps' );

			$tabs[ 'sitemap-post-type-' . $object->name ] = [
				'title'     => 'attachment' === $post_type ? esc_html__( 'Attachments', 'gohigh-seo' ) : $object->label,
				'icon'      => isset( $icons[ $object->name ] ) ? $icons[ $object->name ] : $icons['default'],
				/* translators: %1$s: thing, %2$s: Learn more link. */
				'desc'      => sprintf( esc_html__( 'Change Sitemap settings of %1$s. %2$s.', 'gohigh-seo' ), $thing, '<a href="' . $url . '" target="_blank">' . esc_html__( 'Learn more', 'gohigh-seo' ) . '</a>' ),
				'post_type' => $object->name,
				'file'      => $this->directory . '/settings/post-types.php',
				/* translators: Post Type Sitemap Url */
				'after_row' => $this->get_notice_start() . sprintf( esc_html__( 'Sitemap URL: %s', 'gohigh-seo' ), '<a href="' . $sitemap_url . '" target="_blank">' . $sitemap_url . '</a>' ) . $notice_end,
				'json'      => [
			...
			if ( 'attachment' === $post_type ) {
				$tabs[ 'sitemap-post-type-' . $object->name ]['after_row'] = $this->get_notice_start() . esc_html__( 'Please note that this will add the attachment page URLs to the sitemap, not direct image URLs.', 'gohigh-seo' ) . $notice_end;
	...
	public function taxonomy_settings( $tabs ) {
		...
		$tabs['t_types'] = [
			'title' => esc_html__( 'Taxonomies:', 'gohigh-seo' ),
			'type'  => 'separator',
		];
		...
		foreach ( Helper::get_accessible_taxonomies() as $taxonomy ) {
			...
			switch ( $taxonomy->name ) {
				...
				case 'product_tag':
					...
					$thing = sprintf( __( 'your product %s pages', 'gohigh-seo' ), strtolower( $taxonomy->labels->singular_name ) );
					break;

				default:
					/* translators: Taxonomy singular label */
					$thing = sprintf( __( '%s archives', 'gohigh-seo' ), strtolower( $taxonomy->labels->singular_name ) );
	...
	public function media_popup_fields( $form_fields, $post ) {
		...
		$checkbox .= esc_html__( 'Exclude this attachment from sitemap', 'gohigh-seo' ) . '</label>';
	...
	private function get_nginx_notice() {
		...
		$message = sprintf(
			...
			__( 'Since you are using an NGINX server, you may need to add the following code to your %s <strong>if your Sitemap pages are not loading</strong>. If you are unsure how to do it, please contact your hosting provider.', 'gohigh-seo' ),
			'<a href="https://help.dreamhost.com/hc/en-us/articles/216455077-Nginx-configuration-file-locations/?utm_campaign=Rank+Math" target="_blank">' . __( 'configuration file', 'gohigh-seo' ) . '</a>'
		);

		return '<div class="sitemap-nginx-notice notice notice-alt notice-warning rank-math-notice">' .
		'<p>' . $message .
			' <a href="#"><span class="show">' . __( 'Click here to see the code.', 'gohigh-seo' ) . '</span><span class="hide">' . __( 'Hide', 'gohigh-seo' ) . '</span></a>
			<a href="#" class="sitemap-close-notice">' . __( 'I already added', 'gohigh-seo' ) . '</a>
		</p>
 <pre>
