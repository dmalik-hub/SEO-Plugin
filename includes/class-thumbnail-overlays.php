...
class Thumbnail_Overlays {
	...
	public function sanitize_fields( $sanitized_value, $value, $field_id ) {
		...
		foreach ( $value as $key => $overlay ) {
			if ( empty( $overlay['image'] ) ) {
				...
				Helper::add_notification( esc_html__( 'A Custom Watermark item could not be saved because the name field is empty.', 'gohigh-seo' ), [ 'type' => 'error' ] );
				unset( $value[ $key ] );
	...
	public function enqueue() {
		...
		wp_enqueue_style(
			'gohigh-seo-title-options',
			RANK_MATH_PRO_URL . 'assets/admin/css/title-options.css',
			null,
			rank_math()->version
		);

		wp_enqueue_script( 'gohigh-seo-redirections', RANK_MATH_PRO_URL . 'assets/admin/js/title-options.js', [], RANK_MATH_PRO_VERSION, true );
	}
	...
	private function get_position_choices() {
		return [
			'top_left'      => __( 'Top Left', 'gohigh-seo' ),
			'top_center'    => __( 'Top Center', 'gohigh-seo' ),
			'top_right'     => __( 'Top Right', 'gohigh-seo' ),

			'middle_left'   => __( 'Middle Left', 'gohigh-seo' ),
			'middle_center' => __( 'Middle Center', 'gohigh-seo' ),
			'middle_right'  => __( 'Middle Right', 'gohigh-seo' ),

			'bottom_left'   => __( 'Bottom Left', 'gohigh-seo' ),
			'bottom_center' => __( 'Bottom Center', 'gohigh-seo' ),
			'bottom_right'  => __( 'Bottom Right', 'gohigh-seo' ),
		];
	}
}
