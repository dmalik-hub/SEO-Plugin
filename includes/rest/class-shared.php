...
use GoHighSEO\Redirections\Metabox;
...
class Shared extends WP_REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			[
				...
				'permission_callback' => [ '\\GoHighSEO\\Rest\\Rest_Helper', 'get_redirection_permissions_check' ],
			]
		);
		...
		register_rest_route(
			...
			[
				...
				'permission_callback' => [ '\\GoHighSEO\\Rest\\Rest_Helper', 'get_object_permissions_check' ],
			]
		);
		...
		register_rest_route(
			...
			[
				...
				'permission_callback' => [ '\\GoHighSEO\\Rest\\Rest_Helper', 'get_schema_permissions_check' ],
			]
		);
	}
	...
	private function get_update_redirection_args() {
		return [
			'objectID'        => [
				'type'              => 'integer',
				'required'          => true,
				'description'       => esc_html__( 'Object unique id', 'gohigh-seo' ),
			...
			'objectType'      => [
				...
				'default'           => 'post',
				'required'          => true,
				'description'       => esc_html__( 'Object Type i.e. post, term, user', 'gohigh-seo' ),
				'sanitize_callback' => 'rest_sanitize_request_arg',
				'validate_callback' => [ '\\GoHighSEO\\Rest\\Rest_Helper', 'is_valid_string' ],
			],
			'hasRedirect'     => [
				'type'        => 'boolean',
				'required'    => true,
				'description' => esc_html__( 'Whether the object has a redirect or not', 'gohigh-seo' ),
			],
			'redirectionID'   => [
				'type'        => 'string',
				'required'    => false,
				'description' => esc_html__( 'Redirection ID', 'gohigh-seo' ),
			],
			'redirectionUrl'  => [
				'type'              => 'string',
				'required'          => false,
				'description'       => esc_html__( 'Redirection URL', 'gohigh-seo' ),
				'sanitize_callback' => 'rest_sanitize_request_arg',
			...
			'redirectionType' => [
				...
				'default'           => '301',
				'required'          => true,
				'description'       => esc_html__( 'Redirection Type', 'gohigh-seo' ),
	...
	private function get_update_metadata_args() {
		return [
			...
			'objectID'   => [
				...
				'validate_callback' => function ( $param ) {
					if ( empty( $param ) && 0 !== $param ) {
						return new WP_Error(
							'param_value_empty',
							esc_html__( 'Sorry, field is empty which is not allowed.', 'gohigh-seo' )
						);
					}
			...
			'meta'       => [
				'required'          => true,
				'description'       => esc_html__( 'Meta to add or update data.', 'gohigh-seo' ),
				'validate_callback' => [ '\\GoHighSEO\\Rest\\Rest_Helper', 'is_param_empty' ],
			],
		];
	}
	...
	private function get_update_schemas_args() {
		return [
			...
			'schemas'    => [
				'required'          => true,
				'description'       => esc_html__( 'schemas to add or update data.', 'gohigh-seo' ),
				'validate_callback' => [ '\\GoHighSEO\\Rest\\Rest_Helper', 'is_param_empty' ],
			],
		];
	}
