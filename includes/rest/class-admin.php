...
use GoHighSEO\Role_Manager\Capability_Manager;
...
class Admin extends WP_REST_Controller {
	...
	public function __construct() {
		$this->namespace = \GoHighSEO\Rest\Rest_Helper::BASE;
	}
	...
	public function register_routes() {
		register_rest_route(
			...
			[
				...
				'permission_callback' => [ '\\GoHighSEO\\Rest\\Rest_Helper', 'can_manage_options' ],
		...
		register_rest_route(
			...
			[
				...
				'permission_callback' => [ '\\GoHighSEO\\Rest\\Rest_Helper', 'can_manage_settings' ],
			]
		);
	...
	public function update_settings( WP_REST_Request $request ) {
		...
		if ( $type === 'redirections' ) {
			...
			if ( $redirection->is_infinite_loop() ) {
				if ( ! $redirection->get_id() ) {
					...
					return rest_ensure_response(
						[
							'error' => __( 'The redirection you are trying to create may cause an infinite loop. Please check the source and destination URLs. The redirection has been deactivated.', 'gohigh-seo' ),
						]
					);
				}
				...
				return rest_ensure_response(
					[
						'error' => __( 'The redirection you are trying to update may cause an infinite loop. Please check the source and destination URLs.', 'gohigh-seo' ),
					]
				);
			...
			if ( false === $redirection->save() ) {
				return __( 'Please add at least one valid source URL.', 'gohigh-seo' );
			}
		...
		if ( ! in_array( $type, [ 'general', 'titles', 'sitemap' ], true ) ) {
			return __( 'Invalid type.', 'gohigh-seo' );
		}

		return \GoHighSEO\Admin\Option_Center::save_settings(
			$type,
			$settings,
	...
	private function get_save_module_args() {
		return [
			'module' => [
				'type'              => 'string',
				'required'          => true,
				'description'       => esc_html__( 'Module slug', 'gohigh-seo' ),
				'sanitize_callback' => 'rest_sanitize_request_arg',
				'validate_callback' => function ( $param, $request, $key ) {
					$modules = array_keys( rank_math()->manager->modules );
					if ( ! in_array( $param, $modules, true ) ) {
						return new WP_Error( 'invalid_module', esc_html__( 'Invalid module', 'gohigh-seo' ), [ 'status' => 400 ] );
					}
			...
			'state'  => [
				'type'              => 'string',
				'required'          => true,
				'description'       => esc_html__( 'Module state either on or off', 'gohigh-seo' ),
				'enum'              => [ 'on', 'off' ],
				'sanitize_callback' => 'rest_sanitize_request_arg',
	...
	private function get_update_seo_score_args() {
		return [
			'postScores' => [
				'type'              => 'object',
				'required'          => true,
				'description'       => esc_html__( 'Post scores', 'gohigh-seo' ),
				'sanitize_callback' => 'rest_sanitize_request_arg',
				'validate_callback' => 'rest_validate_request_arg',
			],
		];
	...
	private function get_tools_action_args() {
		return [
			'action' => [
				'type'              => 'string',
				'required'          => true,
				'description'       => esc_html__( 'Action to perform', 'gohigh-seo' ),
				'sanitize_callback' => 'rest_sanitize_request_arg',
				'validate_callback' => 'rest_validate_request_arg',
			],
		];
	...
	private function get_update_mode_args() {
		return [
			'mode' => [
				'type'              => 'string',
				'required'          => true,
				'description'       => esc_html__( 'Mode to set', 'gohigh-seo' ),
