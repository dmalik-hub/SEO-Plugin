...
class Rest_Helper {
	...
	public static function get_redirection_permissions_check() {
		if ( ! Helper::is_module_active( 'redirections' ) || ! Helper::has_cap( 'redirections' ) ) {
			return new WP_Error(
				'rest_cannot_edit',
				__( 'Sorry, you are not allowed to create/update redirection.', 'gohigh-seo' ),
	...
	public static function get_schema_permissions_check( $request ) {
		if ( ! Helper::is_module_active( 'rich-snippet' ) || ! Helper::has_cap( 'onpage_snippet' ) ) {
			return new WP_Error(
				'rest_cannot_edit',
				__( 'Sorry, you are not allowed to create/update schema.', 'gohigh-seo' ),
				[ 'status' => rest_authorization_required_code() ]
			);
		}
	...
	public static function get_post_permissions_check( $request ) {
		...
		if ( $object_id === 0 ) {
			if ( ! Helper::has_cap( 'titles' ) ) {
				return new WP_Error(
					'rest_cannot_edit',
					__( 'Sorry, you are not allowed to edit homepage SEO settings.', 'gohigh-seo' ),
					[ 'status' => rest_authorization_required_code() ]
				);
			}
			return true;
		...
		if ( ! Helper::is_post_type_accessible( $post->post_type ) && 'rank_math_schema' !== $post->post_type ) {
			return new WP_Error(
				'rest_cannot_edit',
				__( 'Sorry, you are not allowed to edit this post type.', 'gohigh-seo' ),
				[ 'status' => rest_authorization_required_code() ]
			);
		}
		...
		return new WP_Error(
			'rest_cannot_edit',
			__( 'Sorry, you are not allowed to edit this post.', 'gohigh-seo' ),
			[ 'status' => rest_authorization_required_code() ]
		);
	}
	...
	public static function get_term_permissions_check( $request ) {
		...
		if (
			...
			return new WP_Error(
				'rest_cannot_edit',
				__( 'Sorry, you are not allowed to edit this term.', 'gohigh-seo' ),
				[ 'status' => rest_authorization_required_code() ]
			);
		}
	...
	public static function is_alphanumerical( $param ) {
		if ( ! preg_match( '/^[a-zA-Z0-9]+$/', $param ) ) {
			return new WP_Error(
				'param_value_empty',
				esc_html__( 'Sorry, the field contains invalid characters.', 'gohigh-seo' )
			);
		}
		return true;
	}
