...
class Setup_Wizard extends WP_REST_Controller {
	...
	public function __construct() {
		$this->namespace = \GoHighSEO\Rest\Rest_Helper::BASE . '/setupWizard';
	}
	...
	public function get_step_data( WP_REST_Request $request ) {
		...
		return \GoHighSEO\Admin\Setup_Wizard::get_localized_data( $step );
	}
	...
	public function update_step_data( WP_REST_Request $request ) {
		...
		return \GoHighSEO\Admin\Setup_Wizard::save_data( $step, $values );
	}
	...
	private function get_step_args() {
		return [
			'step' => [
				'type'              => 'string',
				'required'          => true,
				'description'       => esc_html__( 'Current Step', 'gohigh-seo' ),
	...
	private function update_step_args() {
		return [
			...
			'value' => [
				'type'              => 'object',
				'required'          => true,
				'description'       => esc_html__( 'Current Step Data', 'gohigh-seo' ),
				'sanitize_callback' => 'rest_sanitize_request_arg',
				'validate_callback' => 'rest_validate_request_arg',
			],
		];
	...
	private function tracking_optin_args() {
		return [
			'enable_tracking' => [
				'type'              => 'string',
				'required'          => true,
				'description'       => esc_html__( 'Enable Tracking', 'gohigh-seo' ),
				'sanitize_callback' => 'rest_sanitize_request_arg',
				'validate_callback' => 'rest_validate_request_arg',
			],
		];
