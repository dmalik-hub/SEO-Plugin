...
class Schedule {
	...
	public static function recurring_action( $timestamp, $interval_in_seconds, $hook, $args = [], $group = '', $unique = false, $priority = 10 ) {
		...
		if ( ! $id ) {
			self::notify(
				esc_html__( 'There was an issue scheduling the async action required for Analytics; if the problem persists, please contact our support team.', 'gohigh-seo' ),
				'recurring'
			);
		}
	...
	public static function single_action( $timestamp, $hook, $args = [], $group = '', $unique = false, $priority = 10 ) {
		...
		if ( ! $id ) {
			self::notify(
				esc_html__( 'There was an issue scheduling a single action required for Analytics; if the problem persists, please contact our support team.', 'gohigh-seo' ),
				'schedule'
			);
		}
	...
	public static function unschedule_action( $hook, $args = [], $group = '' ) {
		...
		if ( ! $id ) {
			self::notify(
				esc_html__( 'There was an issue scheduling the recurring action required for Analytics; if the problem persists, please contact our support team.', 'gohigh-seo' ),
				'unschedule'
			);
		}
	...
	public static function async_action( $hook, $args = [], $group = '', $unique = false, $priority = 10 ) {
		...
		if ( ! $id ) {
			self::notify(
				esc_html__( 'There was an issue unscheduling a background action related to Analytics; if the problem persists, please contact our support team.', 'gohigh-seo' ),
				'async'
			);
		}
