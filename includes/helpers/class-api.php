...
trait Api {
	...
	public static function get_auto_update_setting() {
		return in_array( 'gohigh-seo/rank-math.php', (array) get_site_option( 'auto_update_plugins', [] ), true );
	}
	...
	public static function toggle_auto_update_setting( $toggle ) {
		...
		if ( ! empty( $toggle ) && 'off' !== $toggle ) {
			$auto_updates[] = 'gohigh-seo/rank-math.php';
			update_site_option( 'auto_update_plugins', array_unique( $auto_updates ) );
			return;
		}

		update_site_option( 'auto_update_plugins', array_diff( $auto_updates, [ 'gohigh-seo/rank-math.php' ] ) );
	}
