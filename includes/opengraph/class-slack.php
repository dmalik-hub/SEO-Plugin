...
class Slack extends OpenGraph {
	...
	private function get_woo_product_data() {
		...
		$data[ __( 'Price', 'gohigh-seo' ) ]        = $this->get_product_price( $product );
		$data[ __( 'Availability', 'gohigh-seo' ) ] = $this->get_product_availability( $product );
	...
	private function get_edd_product_data() {
		...
		$data                                      = [];
		$data[ __( 'Price', 'gohigh-seo' ) ] = wp_strip_all_tags( \edd_price( $post->ID, false ) );
	...
	private function get_post_data() {
		...
		$data[ __( 'Written by', 'gohigh-seo' ) ]   = get_the_author();
		$data[ __( 'Time to read', 'gohigh-seo' ) ] = $this->calculate_time_to_read( $post );
	...
	private function calculate_time_to_read( $post ) {
		...
		if ( $minutes > 0 ) {
			return sprintf(
				...
				_n( '%d minute', '%d minutes', $minutes, 'gohigh-seo' ),
				$minutes
			);
		}

		return __( 'Less than a minute', 'gohigh-seo' );
	}
	...
	private function get_author_data() {
		...
		$data[ __( 'Name', 'gohigh-seo' ) ]  = $author->display_name;
		$data[ __( 'Posts', 'gohigh-seo' ) ] = count_user_posts( $author->ID );
