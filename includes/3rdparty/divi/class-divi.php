...
class Divi {
	...
	public function add_divi_scripts() {
		...
		wp_dequeue_script( 'gohigh-seo-metabox' );
		wp_enqueue_style(
			'gohigh-seo-editor',
	...
	private function filter_module_et_pb_accordion( $module ) {
		...
		if (
			...
			$toggles_list['general']['toggles']['rank_math_faq_schema_toggle'] = [
				'title'    => wp_strip_all_tags( __( 'Rank Math FAQ Schema', 'gohigh-seo' ) ),
				'priority' => 220,
			];
		...
		if ( ! empty( $fields_list ) ) {
			$fields_list['rank_math_faq_schema'] = [
				'label'       => wp_strip_all_tags( __( 'Add FAQ Schema Markup', 'gohigh-seo' ) ),
				'description' => wp_strip_all_tags( __( 'Added by the Rank Math SEO Plugin.', 'gohigh-seo' ) ),
				'toggle_slug' => 'rank_math_faq_schema_toggle',
				'tab_slug'    => 'general',
				'type'        => 'yes_no_button',
				'default'     => 'off',
				'options'     => [
					'on'  => wp_strip_all_tags( __( 'Yes', 'gohigh-seo' ) ),
					'off' => wp_strip_all_tags( __( 'No', 'gohigh-seo' ) ),
				],
			];
