...
class Update_Email {
	...
	public function maybe_send_update_notification_email( $transient ) {
		...
		$subject = __( '[%s] An update is available for Rank Math', 'gohigh-seo' );
		$subject = sprintf( $subject, wp_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ) );

		$body   = [];
		$body[] = __( 'Hello,', 'gohigh-seo' ) . "\n";

		// Translators: placeholder is the site URL.
		$body[] = sprintf( __( 'This is an automated email to let you know that there is an update available for the Rank Math SEO plugin installed on: %s', 'gohigh-seo' ), get_home_url() ) . "\n";
		...
		$body[] = sprintf( __( 'To ensure your site is always on the latest, most up-to-date version of Rank Math - we recommend logging into the admin area to update the plugin as soon as possible: %s', 'gohigh-seo' ), admin_url( 'update-core.php' ) ) . "\n";

		// Add a note about the support forums.
		$body[] = __( 'If you have any questions or experience any issues – our support team is at your disposal:', 'gohigh-seo' );
		$body[] = __( 'https://support.rankmath.com/', 'gohigh-seo' );
		$body[] = "\n" . '-';
		$body[] = __( 'Rank Math Team', 'gohigh-seo' );
	...
	public function should_send_email( $transient ) {
		...
		if ( ! is_object( $transient )
			...
			|| empty( $transient->response['gohigh-seo/rank-math.php']->new_version )
		) {
			return false;
		}

		$new_version = $transient->response['gohigh-seo/rank-math.php']->new_version;
	...
	public function get_updatable_products( $transient ) {
		...
		$products = [
			'free' => [
				'name'        => __( 'Rank Math Free', 'gohigh-seo' ),
	...
	public function get_products_list( $products_array ) {
		...
		foreach ( $products_array as $product_data ) {
			...
			$list .= sprintf( __( '%1$s: Old %2$s -> New %3$s | Changelog: %4$s', 'gohigh-seo' ), $product_data['name'], $product_data['old_version'], $product_data['new_version'], $product_data['changelog'] ) . "\n";
		}
