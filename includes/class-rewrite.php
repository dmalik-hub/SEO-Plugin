...
class Rewrite {
	...
	public function __construct() {
		...
		if ( Helper::get_settings( 'general.strip_category_base' ) ) {
			...
			add_action( 'created_category', 'GoHighSEO\\Helper::schedule_flush_rewrite' );
			add_action( 'delete_category', 'GoHighSEO\\Helper::schedule_flush_rewrite' );
			add_action( 'edited_category', 'GoHighSEO\\Helper::schedule_flush_rewrite' );
		}

		if ( ! Helper::get_settings( 'titles.disable_author_archives' ) ) {
			if ( ! empty( Helper::get_settings( 'titles.url_author_base' ) ) ) {
				add_action( 'init', 'GoHighSEO\\Rewrite::change_author_base', 4 );
			}
	...
	public function remove_rules() {
		...
		remove_action( 'init', 'GoHighSEO\\Rewrite::change_author_base', 4 );
	}
